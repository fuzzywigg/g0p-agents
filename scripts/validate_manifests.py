...
- AGENT-PROMPTS.md role-blurbs / escalation-format / living-docs leftover locks
...
- AGENT-PROMPTS.md metrics-detail / orch-metrics / communication-detail leftover locks
- AGENT-PROMPTS.md principles-detail / responsibilities-residual / expertise-residual leftover locks
- AGENT-PROMPTS.md vision-context / context-residual / monthly-detail leftover locks
- AGENT-PROMPTS.md docs-residual / matrix-resolutions / usage-detail leftover locks
- AGENT-PROMPTS.md matrix-rows / monthly-checkboxes / usage-numbered leftover locks
- AGENT-PROMPTS.md example-steps / living-bullets / quantum-resp-numbered leftover locks
- AGENT-PROMPTS.md bc/edge/orch-resp-numbered / user-input leftover locks
- AGENT-PROMPTS.md cannot-delegate-numbered / decision-bullets leftover locks
...
PROMPT_RESPONSIBILITIES_REQUIRED_PHRASES: tuple[str, ...] = (
    ...
    "Log all decisions in postmortem.md",
    "Update AGENTS.md risk tolerance (quarterly)",
)
...
PROMPT_INTEGRATION_REQUIRED_PHRASES: tuple[str, ...] = (
...
PROMPT_ROLE_BLURBS_REQUIRED_PHRASES: tuple[str, ...] = (
    'You design quantum algorithms, optimize quantum circuits, and ensure quantum-safe '
    'cryptographic properties for all FUZZYWIGG operations.',
    'You design multi-chain smart contract architecture, implement quantum-resistant consensus '
    'logic, and manage cross-chain state synchronization.',
    'You implement quantum-safe cryptography on mobile devices, design "walled garden" data '
    'isolation, and optimize for device constraints.',
    'You orchestrate the three specialist agents (Quantum, Blockchain, On-Device), resolve '
    'conflicts, make final architectural decisions, and keep the team aligned on vision.',
...
PROMPT_LIVING_DOCS_REQUIRED_PHRASES: tuple[str, ...] = (
    ...
    'Use these prompts when instantiating each specialist agent. Customize with project-specific '
    'context.',
    'New tools become available',
    'AGENTS.md (the constitution)',
    'Task.md (current sprint)',
    'Keep prompts synchronized with AGENTS.md Section 22 (Quantum-Blockchain Integration '
    'Standards).',
)

PROMPT_CONSTRAINTS_DETAIL_REQUIRED_PHRASES: tuple[str, ...] = (
...
PROMPT_TRIGGERS_DETAIL_REQUIRED_PHRASES: tuple[str, ...] = (
...
PROMPT_HUMAN_FIELDS_REQUIRED_PHRASES: tuple[str, ...] = (
...
PROMPT_CONTEXT_DETAIL_REQUIRED_PHRASES: tuple[str, ...] = (
...
PROMPT_EXPERTISE_DETAIL_REQUIRED_PHRASES: tuple[str, ...] = (
...
PROMPT_RELATED_HEALTH_REQUIRED_PHRASES: tuple[str, ...] = (
...
PROMPT_TOOLS_DETAIL_REQUIRED_PHRASES: tuple[str, ...] = (
...
PROMPT_RESPONSIBILITIES_DETAIL_REQUIRED_PHRASES: tuple[str, ...] = (
    'Optimize Cirq circuits for target hardware',
    'Simulate quantum timelines and collective consciousness models (your "Mickey 18" concept)',
    'Provide hardware requirement estimates',
    'Implement quantum-resistant consensus logic',
    'Define cross-chain state synchronization protocols',
    'Audit contracts for quantum vulnerabilities',
    'Design data isolation ("walled garden") architecture',
    'Manage secure key lifecycle (storage, rotation, deletion)',
    'Validate health data privacy architecture (HIPAA, GDPR)',
    'Escalate unsolvable conflicts to human',
)
...
PROMPT_METRICS_DETAIL_REQUIRED_PHRASES: tuple[str, ...] = (
...
PROMPT_ORCH_METRICS_DETAIL_REQUIRED_PHRASES: tuple[str, ...] = (
    'Battery drain < 2% per transaction',
...
PROMPT_RESPONSIBILITIES_RESIDUAL_REQUIRED_PHRASES: tuple[str, ...] = (
...
PROMPT_USAGE_DETAIL_REQUIRED_PHRASES: tuple[str, ...] = (
    'Agent Action:',
    '**Fill in the [INSERT PROJECT-SPECIFIC INFO HERE] sections**',
    '(Claude, GPT, Gemini, etc.)',
    '**Copy the relevant prompt template** (above)',
    'Check Cirq optimization, validate quantum-safety, provide gate count '
    'and error rate estimates. Use the scratchpad to track progress.',
)

PROMPT_MATRIX_ROWS_REQUIRED_PHRASES: tuple[str, ...] = (
    '| **Algorithm Complexity** (Quantum says "too complex", Blockchain says "necessary") | '
    'Weigh risk tolerance. Choose testnet approach to validate. |',
    '| **Gas Cost** (Blockchain says "over budget", On-Device says "can\'t afford") | '
    'Redesign contract interface or reduce scope. |',
    '| **Crypto Algorithm** (On-Device says "RSA too slow", Quantum says "must be RSA") | '
    'Use hybrid (post-quantum + RSA), implement staged migration. |',
    '| **Timeline** (All agents say "2 weeks", business needs "2 days") | Reduce scope, '
    'increase risk, escalate to stakeholder. |',
)

PROMPT_MONTHLY_CHECKBOXES_REQUIRED_PHRASES: tuple[str, ...] = (
    '- [ ] Review all recent decisions in postmortem.md',
    '- [ ] Check agent success metrics (are they meeting targets?)',
    '- [ ] Identify any emerging conflicts (before they escalate)',
    '- [ ] Update risk register (quarterly, minimum)',
...
PROMPT_USAGE_NUMBERED_REQUIRED_PHRASES: tuple[str, ...] = (
...
PROMPT_EXAMPLE_STEPS_REQUIRED_PHRASES: tuple[str, ...] = (
...
PROMPT_LIVING_BULLETS_REQUIRED_PHRASES: tuple[str, ...] = (
...
PROMPT_QUANTUM_RESP_NUMBERED_REQUIRED_PHRASES: tuple[str, ...] = (
    ...
    '4. Simulate quantum timelines and collective consciousness models (your "Mickey 18" '
    'concept)',
    '5. Provide hardware requirement estimates',
    '6. Escalate when constraints cannot be met',
)

PROMPT_BC_RESP_NUMBERED_REQUIRED_PHRASES: tuple[str, ...] = (
...
PROMPT_EDGE_RESP_NUMBERED_REQUIRED_PHRASES: tuple[str, ...] = (
...
PROMPT_ORCH_RESP_NUMBERED_REQUIRED_PHRASES: tuple[str, ...] = (
...
PROMPT_USER_INPUT_REQUIRED_PHRASES: tuple[str, ...] = (
    'User Input: "Review the proposed factorization circuit for our NFT mint operation. '
    'Check Cirq optimization, validate quantum-safety, provide gate count and error rate '
    'estimates. Use the scratchpad to track progress."',
)

PROMPT_CANNOT_DELEGATE_NUMBERED_REQUIRED_PHRASES: tuple[str, ...] = (
...
PROMPT_DECISION_BULLETS_REQUIRED_PHRASES: tuple[str, ...] = (
    '- Trade-offs between security, performance, and usability',
    '- Prioritization (what gets built first)',
    '- Risk acceptance (can we deploy with this vulnerability?)',
    '- Timeline adjustments (can we ship on schedule?)',
...
def validate_packaging_inventory(root: Path) -> list[Finding]:
    ...
    if (
        ...
        findings.append(
            _lock_mismatch(
                schema_path, "prompt_decision_authority_required_phrases"
            )
        )

    if (
        tuple(inventory.get("prompt_integration_required_phrases", ()))
        != PROMPT_INTEGRATION_REQUIRED_PHRASES
    ):
        findings.append(
            _lock_mismatch(schema_path, "prompt_integration_required_phrases")
        )
    ...
    if (
        ...
        findings.append(_lock_mismatch(schema_path, "prompt_context_required_phrases"))
    ...
    if (
        tuple(inventory.get("prompt_constraints_detail_required_phrases", ()))
        != PROMPT_CONSTRAINTS_DETAIL_REQUIRED_PHRASES
    ):
        findings.append(
            _lock_mismatch(schema_path, "prompt_constraints_detail_required_phrases")
        )

    if (
        tuple(inventory.get("prompt_triggers_detail_required_phrases", ()))
        != PROMPT_TRIGGERS_DETAIL_REQUIRED_PHRASES
    ):
        findings.append(
            _lock_mismatch(schema_path, "prompt_triggers_detail_required_phrases")
        )

    if (
        tuple(inventory.get("prompt_human_fields_required_phrases", ()))
        != PROMPT_HUMAN_FIELDS_REQUIRED_PHRASES
    ):
        findings.append(
            _lock_mismatch(schema_path, "prompt_human_fields_required_phrases")
        )

    if (
        tuple(inventory.get("prompt_context_detail_required_phrases", ()))
        != PROMPT_CONTEXT_DETAIL_REQUIRED_PHRASES
    ):
        findings.append(
            _lock_mismatch(schema_path, "prompt_context_detail_required_phrases")
        )

    if (
        tuple(inventory.get("prompt_expertise_detail_required_phrases", ()))
        != PROMPT_EXPERTISE_DETAIL_REQUIRED_PHRASES
    ):
        findings.append(
            _lock_mismatch(schema_path, "prompt_expertise_detail_required_phrases")
        )

    if (
        tuple(inventory.get("prompt_related_health_required_phrases", ()))
        != PROMPT_RELATED_HEALTH_REQUIRED_PHRASES
    ):
        findings.append(
            _lock_mismatch(schema_path, "prompt_related_health_required_phrases")
        )

    if (
        tuple(inventory.get("prompt_tools_detail_required_phrases", ()))
        != PROMPT_TOOLS_DETAIL_REQUIRED_PHRASES
    ):
        findings.append(
            _lock_mismatch(schema_path, "prompt_tools_detail_required_phrases")
        )

    if (
        tuple(inventory.get("prompt_responsibilities_detail_required_phrases", ()))
        != PROMPT_RESPONSIBILITIES_DETAIL_REQUIRED_PHRASES
    ):
        findings.append(
            _lock_mismatch(schema_path, "prompt_responsibilities_detail_required_phrases")
        )
    ...
    if (
        tuple(inventory.get("prompt_metrics_detail_required_phrases", ()))
        != PROMPT_METRICS_DETAIL_REQUIRED_PHRASES
    ):
        findings.append(
            _lock_mismatch(schema_path, "prompt_metrics_detail_required_phrases")
        )

    if (
        tuple(inventory.get("prompt_orch_metrics_detail_required_phrases", ()))
        != PROMPT_ORCH_METRICS_DETAIL_REQUIRED_PHRASES
    ):
        findings.append(
            _lock_mismatch(schema_path, "prompt_orch_metrics_detail_required_phrases")
        )
    ...
    if (
        tuple(inventory.get("prompt_responsibilities_residual_required_phrases", ()))
        != PROMPT_RESPONSIBILITIES_RESIDUAL_REQUIRED_PHRASES
    ):
        findings.append(
            _lock_mismatch(schema_path, "prompt_responsibilities_residual_required_phrases")
        )
    ...
    if (
        tuple(inventory.get("prompt_vision_context_required_phrases", ()))
        != PROMPT_VISION_CONTEXT_REQUIRED_PHRASES
    ):
        findings.append(
            _lock_mismatch(schema_path, "prompt_vision_context_required_phrases")
        )

    if (
        tuple(inventory.get("prompt_context_residual_required_phrases", ()))
        != PROMPT_CONTEXT_RESIDUAL_REQUIRED_PHRASES
    ):
        findings.append(
            _lock_mismatch(schema_path, "prompt_context_residual_required_phrases")
        )

    if (
        tuple(inventory.get("prompt_monthly_detail_required_phrases", ()))
        != PROMPT_MONTHLY_DETAIL_REQUIRED_PHRASES
    ):
        findings.append(
            _lock_mismatch(schema_path, "prompt_monthly_detail_required_phrases")
        )

    if (
        tuple(inventory.get("prompt_docs_residual_required_phrases", ()))
        != PROMPT_DOCS_RESIDUAL_REQUIRED_PHRASES
    ):
        findings.append(
            _lock_mismatch(schema_path, "prompt_docs_residual_required_phrases")
        )

    if (
        tuple(inventory.get("prompt_matrix_resolutions_required_phrases", ()))
        != PROMPT_MATRIX_RESOLUTIONS_REQUIRED_PHRASES
    ):
        findings.append(
            _lock_mismatch(schema_path, "prompt_matrix_resolutions_required_phrases")
        )

    if (
        tuple(inventory.get("prompt_usage_detail_required_phrases", ()))
        != PROMPT_USAGE_DETAIL_REQUIRED_PHRASES
    ):
        findings.append(
            _lock_mismatch(schema_path, "prompt_usage_detail_required_phrases")
        )

    if (
        tuple(inventory.get("prompt_matrix_rows_required_phrases", ()))
        != PROMPT_MATRIX_ROWS_REQUIRED_PHRASES
    ):
        findings.append(
            _lock_mismatch(schema_path, "prompt_matrix_rows_required_phrases")
        )

    if (
        tuple(inventory.get("prompt_monthly_checkboxes_required_phrases", ()))
        != PROMPT_MONTHLY_CHECKBOXES_REQUIRED_PHRASES
    ):
        findings.append(
            _lock_mismatch(schema_path, "prompt_monthly_checkboxes_required_phrases")
        )

    if (
        tuple(inventory.get("prompt_usage_numbered_required_phrases", ()))
        != PROMPT_USAGE_NUMBERED_REQUIRED_PHRASES
    ):
        findings.append(
            _lock_mismatch(schema_path, "prompt_usage_numbered_required_phrases")
        )

    if (
        tuple(inventory.get("prompt_example_steps_required_phrases", ()))
        != PROMPT_EXAMPLE_STEPS_REQUIRED_PHRASES
    ):
        findings.append(
            _lock_mismatch(schema_path, "prompt_example_steps_required_phrases")
        )

    if (
        tuple(inventory.get("prompt_living_bullets_required_phrases", ()))
        != PROMPT_LIVING_BULLETS_REQUIRED_PHRASES
    ):
        findings.append(
            _lock_mismatch(schema_path, "prompt_living_bullets_required_phrases")
        )

    if (
        tuple(inventory.get("prompt_quantum_resp_numbered_required_phrases", ()))
        != PROMPT_QUANTUM_RESP_NUMBERED_REQUIRED_PHRASES
    ):
        findings.append(
            _lock_mismatch(schema_path, "prompt_quantum_resp_numbered_required_phrases")
        )

    if (
        tuple(inventory.get("prompt_bc_resp_numbered_required_phrases", ()))
        != PROMPT_BC_RESP_NUMBERED_REQUIRED_PHRASES
    ):
        findings.append(
            _lock_mismatch(schema_path, "prompt_bc_resp_numbered_required_phrases")
        )

    if (
        tuple(inventory.get("prompt_edge_resp_numbered_required_phrases", ()))
        != PROMPT_EDGE_RESP_NUMBERED_REQUIRED_PHRASES
    ):
        findings.append(
            _lock_mismatch(schema_path, "prompt_edge_resp_numbered_required_phrases")
        )

    if (
        tuple(inventory.get("prompt_orch_resp_numbered_required_phrases", ()))
        != PROMPT_ORCH_RESP_NUMBERED_REQUIRED_PHRASES
    ):
        findings.append(
            _lock_mismatch(schema_path, "prompt_orch_resp_numbered_required_phrases")
        )

    if (
        tuple(inventory.get("prompt_user_input_required_phrases", ()))
        != PROMPT_USER_INPUT_REQUIRED_PHRASES
    ):
        findings.append(
            _lock_mismatch(schema_path, "prompt_user_input_required_phrases")
        )

    if (
        tuple(inventory.get("prompt_cannot_delegate_numbered_required_phrases", ()))
        != PROMPT_CANNOT_DELEGATE_NUMBERED_REQUIRED_PHRASES
    ):
        findings.append(
            _lock_mismatch(schema_path, "prompt_cannot_delegate_numbered_required_phrases")
        )

    if (
        tuple(inventory.get("prompt_decision_bullets_required_phrases", ()))
        != PROMPT_DECISION_BULLETS_REQUIRED_PHRASES
    ):
        findings.append(
            _lock_mismatch(schema_path, "prompt_decision_bullets_required_phrases")
        )
...
def _inventory_lock_consistency(
    ...
    for phrase in prompt_orchestration_matrix:
        ...
        if prompt_orchestration_matrix and not required_prompt_orchestration_matrix <= set(
    ...
    for phrase in prompt_usage_example:
        ...
        if prompt_usage_example and not required_prompt_usage_example <= set(
    ...
    for phrase in prompt_responsibilities:
        ...
        if prompt_responsibilities and not required_prompt_responsibilities <= set(
    ...
    for phrase in prompt_decision_authority:
        ...
        if prompt_decision_authority and not required_prompt_decision_authority <= set(
    ...
    prompt_integration = list(inventory.get("prompt_integration_required_phrases", ()))
    if len(prompt_integration) != len(set(prompt_integration)):
        findings.append(
            Finding(schema_path, "prompt_integration_required_phrases must be unique")
        )
    if not prompt_integration:
        findings.append(
            Finding(
                schema_path, "prompt_integration_required_phrases must not be empty"
            )
        )
    for phrase in prompt_integration:
        if not isinstance(phrase, str) or not phrase.strip():
            findings.append(
                Finding(
                    schema_path,
                    "prompt_integration_required_phrases entries must be "
                    "non-empty strings",
                )
            )
            break
    else:
        required_prompt_integration = {
            "## Integration with AGENTS.md",
            "These prompts are **living documents**",
            "## When You Escalate to Human",
            "🚨 ESCALATION TO HUMAN REQUIRED",
        }
        if prompt_integration and not required_prompt_integration <= set(
            prompt_integration
        ):
            findings.append(
                Finding(
                    schema_path,
                    "prompt_integration_required_phrases must include "
    ...
    if len(prompt_cannot_delegate) != len(set(prompt_cannot_delegate)):
        findings.append(
            Finding(
                schema_path, "prompt_cannot_delegate_required_phrases must be unique"
            )
        )
    if not prompt_cannot_delegate:
        findings.append(
            Finding(
                schema_path, "prompt_cannot_delegate_required_phrases must not be empty"
            )
        )
    for phrase in prompt_cannot_delegate:
        ...
        if prompt_cannot_delegate and not required_prompt_cannot_delegate <= set(
    ...
    for phrase in prompt_escalation_authority:
        ...
        if prompt_escalation_authority and not required_prompt_escalation_authority <= set(
    ...
    prompt_role_blurbs = list(inventory.get("prompt_role_blurbs_required_phrases", ()))
    ...
    if not prompt_role_blurbs:
        findings.append(
            Finding(schema_path, "prompt_role_blurbs_required_phrases must not be empty")
        )
    ...
    prompt_escalation_format = list(inventory.get("prompt_escalation_format_required_phrases", ()))
    if len(prompt_escalation_format) != len(set(prompt_escalation_format)):
        findings.append(
            Finding(schema_path, "prompt_escalation_format_required_phrases must be unique")
        )
    if not prompt_escalation_format:
        findings.append(
            Finding(schema_path, "prompt_escalation_format_required_phrases must not be empty")
        )
    ...
    if not prompt_living_docs:
        findings.append(
            Finding(schema_path, "prompt_living_docs_required_phrases must not be empty")
        )
    ...
    prompt_constraints_detail = list(
        inventory.get('prompt_constraints_detail_required_phrases', ())
    )
    if len(prompt_constraints_detail) != len(set(prompt_constraints_detail)):
        findings.append(
            Finding(schema_path, "prompt_constraints_detail_required_phrases must be unique")
        )
    if not prompt_constraints_detail:
        findings.append(
            Finding(schema_path, "prompt_constraints_detail_required_phrases must not be empty")
        )
    for phrase in prompt_constraints_detail:
        if not isinstance(phrase, str) or not phrase.strip():
            findings.append(
                Finding(
                    schema_path,
                    "prompt_constraints_detail_required_phrases entries must be "
                    "non-empty strings",
                )
            )
            break
    else:
        required_prompt_constraints_detail = {
            'Always validate data isolation (no leaks to system logs)',
            'Never allow critical vulnerabilities (Slither must pass)',
            'Never optimize beyond device constraints (memory, runtime)',
        }
        if prompt_constraints_detail and not required_prompt_constraints_detail <= set(
            prompt_constraints_detail
        ):
            findings.append(
                Finding(
                    schema_path,
                    "prompt_constraints_detail_required_phrases must include "
                    "Never optimize/Slither must pass/Apple/Google/data isolation",
                )
            )

    prompt_triggers_detail = list(inventory.get("prompt_triggers_detail_required_phrases", ()))
    if len(prompt_triggers_detail) != len(set(prompt_triggers_detail)):
        findings.append(
            Finding(schema_path, "prompt_triggers_detail_required_phrases must be unique")
        )
    if not prompt_triggers_detail:
        findings.append(
            Finding(schema_path, "prompt_triggers_detail_required_phrases must not be empty")
        )
    for phrase in prompt_triggers_detail:
        if not isinstance(phrase, str) or not phrase.strip():
            findings.append(
                Finding(
                    schema_path,
                    "prompt_triggers_detail_required_phrases entries must be "
                    "non-empty strings",
                )
            )
            break
    else:
        required_prompt_triggers_detail = {
            'Battery drain > 2% per transaction',
            'Multi-chain sync time > 10 minutes',
            'Quantum advantage deadline < 6 months and algorithm not quantum-safe',
        }
        if prompt_triggers_detail and not required_prompt_triggers_detail <= set(
            prompt_triggers_detail
        ):
            findings.append(
                Finding(
                    schema_path,
                    "prompt_triggers_detail_required_phrases must include "
                    "Quantum advantage/Multi-chain sync/Battery drain/API design conflicts",
                )
            )

    prompt_human_fields = list(inventory.get("prompt_human_fields_required_phrases", ()))
    if len(prompt_human_fields) != len(set(prompt_human_fields)):
        findings.append(
            Finding(schema_path, "prompt_human_fields_required_phrases must be unique")
        )
    if not prompt_human_fields:
        findings.append(
            Finding(schema_path, "prompt_human_fields_required_phrases must not be empty")
        )
    for phrase in prompt_human_fields:
        if not isinstance(phrase, str) or not phrase.strip():
            findings.append(
                Finding(
                    schema_path,
                    "prompt_human_fields_required_phrases entries must be "
                    "non-empty strings",
                )
            )
            break
    else:
        required_prompt_human_fields = {
            'Agent Input: [What did specialist agents recommend?]',
            'Risk Assessment: [What could go wrong with each option?]',
            'Situation: [What decision needs human input?]',
        }
        if prompt_human_fields and not required_prompt_human_fields <= set(prompt_human_fields):
            findings.append(
                Finding(
                    schema_path,
                    "prompt_human_fields_required_phrases must include "
                    "Situation/Agent Input/Risk Assessment/caveats",
                )
            )

    prompt_context_detail = list(inventory.get("prompt_context_detail_required_phrases", ()))
    if len(prompt_context_detail) != len(set(prompt_context_detail)):
        findings.append(
            Finding(schema_path, "prompt_context_detail_required_phrases must be unique")
        )
    if not prompt_context_detail:
        findings.append(
            Finding(schema_path, "prompt_context_detail_required_phrases must not be empty")
        )
    for phrase in prompt_context_detail:
        if not isinstance(phrase, str) or not phrase.strip():
            findings.append(
                Finding(
                    schema_path,
                    "prompt_context_detail_required_phrases entries must be "
                    "non-empty strings",
                )
            )
            break
    else:
        required_prompt_context_detail = {
            'GDPR compliance required: [YES / NO]',
            'HIPAA compliance required: [YES / NO]',
            'Token standards: ERC-20 (if applicable), ERC-721 (NFT)',
        }
        if prompt_context_detail and not required_prompt_context_detail <= set(
            prompt_context_detail
        ):
            findings.append(
                Finding(
                    schema_path,
                    "prompt_context_detail_required_phrases must include "
                    "Token standards/HIPAA/GDPR/Deployment target/Key Constraint",
                )
            )

    prompt_expertise_detail = list(inventory.get("prompt_expertise_detail_required_phrases", ()))
    if len(prompt_expertise_detail) != len(set(prompt_expertise_detail)):
        findings.append(
            Finding(schema_path, "prompt_expertise_detail_required_phrases must be unique")
        )
    if not prompt_expertise_detail:
        findings.append(
            Finding(schema_path, "prompt_expertise_detail_required_phrases must not be empty")
        )
    for phrase in prompt_expertise_detail:
        if not isinstance(phrase, str) or not phrase.strip():
            findings.append(
                Finding(
                    schema_path,
                    "prompt_expertise_detail_required_phrases entries must be "
                    "non-empty strings",
                )
            )
            break
    else:
        required_prompt_expertise_detail = {
            'Circuit optimization (gate reduction, error mitigation)',
            'Multi-chain bridges (Connext, LayerZero, IBC)',
            'On-device cryptography (post-quantum)',
        }
        if prompt_expertise_detail and not required_prompt_expertise_detail <= set(
            prompt_expertise_detail
        ):
            findings.append(
                Finding(
                    schema_path,
                    "prompt_expertise_detail_required_phrases must include "
                    "Circuit optimization/Connext/On-device cryptography/HIPAA applicability",
                )
            )

    prompt_related_health = list(inventory.get("prompt_related_health_required_phrases", ()))
    if len(prompt_related_health) != len(set(prompt_related_health)):
        findings.append(
            Finding(schema_path, "prompt_related_health_required_phrases must be unique")
        )
    if not prompt_related_health:
        findings.append(
            Finding(schema_path, "prompt_related_health_required_phrases must not be empty")
        )
    for phrase in prompt_related_health:
        if not isinstance(phrase, str) or not phrase.strip():
            findings.append(
                Finding(
                    schema_path,
                    "prompt_related_health_required_phrases entries must be "
                    "non-empty strings",
                )
            )
            break
    else:
        required_prompt_related_health = {
            'See AGENTS.md Section 12.4.1 (Risk Tolerance Review)',
            'See AGENTS.md Section 5.3 (Knowledge & Health Agents)',
            'See AGENTS.md Section 5.3.1 (Health Data Privacy Roadmap)',
        }
        if prompt_related_health and not required_prompt_related_health <= set(
            prompt_related_health
        ):
            findings.append(
                Finding(
                    schema_path,
                    "prompt_related_health_required_phrases must include "
                    "Section 5.3/5.3.1/22.3/12.4.1",
                )
            )

    prompt_tools_detail = list(inventory.get("prompt_tools_detail_required_phrases", ()))
    if len(prompt_tools_detail) != len(set(prompt_tools_detail)):
        findings.append(
            Finding(schema_path, "prompt_tools_detail_required_phrases must be unique")
        )
    if not prompt_tools_detail:
        findings.append(
            Finding(schema_path, "prompt_tools_detail_required_phrases must not be empty")
        )
    for phrase in prompt_tools_detail:
        if not isinstance(phrase, str) or not phrase.strip():
            findings.append(
                Finding(
                    schema_path,
                    "prompt_tools_detail_required_phrases entries must be "
                    "non-empty strings",
                )
            )
            break
    else:
        required_prompt_tools_detail = {
            'Etherscan (contract verification)',
            'IBM Quantum Experience (real hardware validation)',
            'React Native + Expo (cross-platform prototyping)',
        }
        if prompt_tools_detail and not required_prompt_tools_detail <= set(prompt_tools_detail):
            findings.append(
                Finding(
                    schema_path,
                    "prompt_tools_detail_required_phrases must include "
                    "IBM Quantum/Etherscan/React Native/Ledger/libsodium",
                )
            )

    prompt_responsibilities_detail = list(
        inventory.get('prompt_responsibilities_detail_required_phrases', ())
    )
    if len(prompt_responsibilities_detail) != len(set(prompt_responsibilities_detail)):
        findings.append(
            Finding(schema_path, "prompt_responsibilities_detail_required_phrases must be unique")
        )
    if not prompt_responsibilities_detail:
        findings.append(
            Finding(
                schema_path, "prompt_responsibilities_detail_required_phrases must not be empty"
            )
        )
    for phrase in prompt_responsibilities_detail:
        if not isinstance(phrase, str) or not phrase.strip():
            findings.append(
                Finding(
                    schema_path,
                    "prompt_responsibilities_detail_required_phrases entries must be "
                    "non-empty strings",
                )
            )
            break
    else:
        required_prompt_responsibilities_detail = {
            'Design data isolation ("walled garden") architecture',
            'Escalate unsolvable conflicts to human',
            'Optimize Cirq circuits for target hardware',
        }
        if prompt_responsibilities_detail and not required_prompt_responsibilities_detail <= set(
            prompt_responsibilities_detail
        ):
            findings.append(
                Finding(
                    schema_path,
                    "prompt_responsibilities_detail_required_phrases must include "
                    "Optimize Cirq/Mickey 18/walled garden/Escalate unsolvable",
                )
            )

    prompt_instantiation = list(inventory.get("prompt_instantiation_required_phrases", ()))
    if len(prompt_instantiation) != len(set(prompt_instantiation)):
        findings.append(
            Finding(schema_path, "prompt_instantiation_required_phrases must be unique")
        )
    if not prompt_instantiation:
        findings.append(
            Finding(schema_path, "prompt_instantiation_required_phrases must not be empty")
        )
    for phrase in prompt_instantiation:
        if not isinstance(phrase, str) or not phrase.strip():
            findings.append(
                Finding(
                    schema_path,
    ...
    metrics_detail = list(inventory.get("prompt_metrics_detail_required_phrases", ()))
    if len(metrics_detail) != len(set(metrics_detail)):
        findings.append(
            Finding(schema_path, "prompt_metrics_detail_required_phrases must be unique")
        )
    if not metrics_detail:
        findings.append(
            Finding(schema_path, "prompt_metrics_detail_required_phrases must not be empty")
        )
    for phrase in metrics_detail:
        if not isinstance(phrase, str) or not phrase.strip():
            findings.append(
                Finding(
                    schema_path,
                    "prompt_metrics_detail_required_phrases entries must be "
                    "non-empty strings",
                )
            )
            break
    else:
        required_metrics_detail = {
            'All smart contracts quantum-safe audited',
            'Gate count optimized for target hardware',
            'Multi-chain sync < 2 minutes',
        }
        if metrics_detail and not required_metrics_detail <= set(metrics_detail):
            findings.append(
                Finding(
                    schema_path,
                    "prompt_metrics_detail_required_phrases must include "
                    "Gate count/Multi-chain sync/quantum-safe audited",
                )
            )

    orch_metrics_detail = list(inventory.get("prompt_orch_metrics_detail_required_phrases", ()))
    if len(orch_metrics_detail) != len(set(orch_metrics_detail)):
        findings.append(
            Finding(schema_path, "prompt_orch_metrics_detail_required_phrases must be unique")
        )
    if not orch_metrics_detail:
        findings.append(
            Finding(schema_path, "prompt_orch_metrics_detail_required_phrases must not be empty")
        )
    for phrase in orch_metrics_detail:
        if not isinstance(phrase, str) or not phrase.strip():
            findings.append(
                Finding(
                    schema_path,
                    "prompt_orch_metrics_detail_required_phrases entries must be "
                    "non-empty strings",
                )
            )
            break
    else:
        required_orch_metrics_detail = {
            'Apple security review: PASS',
            'Google security review: PASS',
            'Team consensus on direction (if possible)',
        }
        if orch_metrics_detail and not required_orch_metrics_detail <= set(orch_metrics_detail):
            findings.append(
                Finding(
                    schema_path,
                    "prompt_orch_metrics_detail_required_phrases must include "
                    "Apple/Google PASS/Team consensus/On-time/Stakeholder",
                )
            )

    communication_detail = list(inventory.get("prompt_communication_detail_required_phrases", ()))
    if len(communication_detail) != len(set(communication_detail)):
        findings.append(
            Finding(schema_path, "prompt_communication_detail_required_phrases must be unique")
        )
    if not communication_detail:
        findings.append(
            Finding(schema_path, "prompt_communication_detail_required_phrases must not be empty")
        )
    for phrase in communication_detail:
        if not isinstance(phrase, str) or not phrase.strip():
            findings.append(
                Finding(
                    schema_path,
        ...
        required_communication_detail = {
            'Acknowledge risks clearly',
            'Conservative on claims ("this is theoretically possible, but...")',
            'Explain via architecture diagrams',
        }
        if communication_detail and not required_communication_detail <= set(communication_detail):
            findings.append(
                Finding(
                    schema_path,
    ...
    principles_detail = list(inventory.get("prompt_principles_detail_required_phrases", ()))
    if len(principles_detail) != len(set(principles_detail)):
        findings.append(
            Finding(schema_path, "prompt_principles_detail_required_phrases must be unique")
        )
    if not principles_detail:
        findings.append(
            Finding(schema_path, "prompt_principles_detail_required_phrases must not be empty")
        )
    for phrase in principles_detail:
        if not isinstance(phrase, str) or not phrase.strip():
            findings.append(
                Finding(
                    schema_path,
        ...
        required_principles_detail = {
            'Recommend testing on multiple backends',
            'Validate Apple/Google security guidelines compliance',
            'Validate circuit correctness before integration',
        }
        if principles_detail and not required_principles_detail <= set(principles_detail):
            findings.append(
                Finding(
                    schema_path,
    ...
    responsibilities_residual = list(
        inventory.get("prompt_responsibilities_residual_required_phrases", ())
    )
    if len(responsibilities_residual) != len(set(responsibilities_residual)):
        findings.append(
            Finding(schema_path, "prompt_responsibilities_residual_required_phrases must be unique")
        )
    if not responsibilities_residual:
        findings.append(
            Finding(
                schema_path, "prompt_responsibilities_residual_required_phrases must not be empty"
            )
        )
    for phrase in responsibilities_residual:
        if not isinstance(phrase, str) or not phrase.strip():
            findings.append(
                Finding(
                    schema_path,
                    "prompt_responsibilities_residual_required_phrases entries must be "
                    "non-empty strings",
                )
            )
            break
    else:
        required_responsibilities_residual = {
            'Identify conflicts (if any)',
            'Make final go/no-go decision',
            'Validate quantum-safe properties (post-quantum crypto)',
        }
        if responsibilities_residual and not (
            required_responsibilities_residual <= set(responsibilities_residual)
        ):
            findings.append(
                Finding(
                    schema_path,
                    "prompt_responsibilities_residual_required_phrases must include "
                    "Validate quantum-safe/Identify conflicts/Make final go/no-go",
                )
            )

    expertise_residual = list(inventory.get("prompt_expertise_residual_required_phrases", ()))
    if len(expertise_residual) != len(set(expertise_residual)):
        findings.append(
            Finding(schema_path, "prompt_expertise_residual_required_phrases must be unique")
        )
    if not expertise_residual:
        findings.append(
            Finding(schema_path, "prompt_expertise_residual_required_phrases must not be empty")
        )
    for phrase in expertise_residual:
        if not isinstance(phrase, str) or not phrase.strip():
            findings.append(
                Finding(
                    schema_path,
        ...
        required_expertise_residual = {
            'Data lifecycle management',
            'Qualtran (algorithm analysis)',
            'Qualtran (resource analysis)',
        }
        if expertise_residual and not required_expertise_residual <= set(expertise_residual):
            findings.append(
                Finding(
                    schema_path,
    ...
    vision_context = list(inventory.get("prompt_vision_context_required_phrases", ()))
    if len(vision_context) != len(set(vision_context)):
        findings.append(
            Finding(schema_path, "prompt_vision_context_required_phrases must be unique")
        )
    if not vision_context:
        findings.append(
            Finding(schema_path, "prompt_vision_context_required_phrases must not be empty")
        )
    for phrase in vision_context:
        if not isinstance(phrase, str) or not phrase.strip():
            findings.append(
                Finding(
                    schema_path,
                    "prompt_vision_context_required_phrases entries must be "
                    "non-empty strings",
                )
            )
            break
    else:
        required_vision_context = {
            '## Current Project Vision',
            'Budget: [INSERT IF APPLICABLE]',
            'Timeline: [INSERT TARGET DATE]',
        }
        if vision_context and not required_vision_context <= set(vision_context):
            findings.append(
                Finding(
                    schema_path,
                    "prompt_vision_context_required_phrases must include "
                    "Current Project Vision/Timeline/Budget INSERT",
                )
            )

    context_residual = list(inventory.get("prompt_context_residual_required_phrases", ()))
    if len(context_residual) != len(set(context_residual)):
        findings.append(
            Finding(schema_path, "prompt_context_residual_required_phrases must be unique")
        )
    if not context_residual:
        findings.append(
            Finding(schema_path, "prompt_context_residual_required_phrases must not be empty")
        )
    for phrase in context_residual:
        if not isinstance(phrase, str) or not phrase.strip():
            findings.append(
                Finding(
                    schema_path,
                    "prompt_context_residual_required_phrases entries must be "
                    "non-empty strings",
                )
            )
            break
    else:
        required_context_residual = {
            'Deadline: [INSERT DATE]',
            'Error tolerance: < 1%',
            'Gas budget: [INSERT LIMIT]',
        }
        if context_residual and not required_context_residual <= set(context_residual):
            findings.append(
                Finding(
                    schema_path,
                    "prompt_context_residual_required_phrases must include "
                    "Error tolerance/Gas budget/Deadline INSERT",
                )
            )

    monthly_detail = list(inventory.get("prompt_monthly_detail_required_phrases", ()))
    if len(monthly_detail) != len(set(monthly_detail)):
        findings.append(
            Finding(schema_path, "prompt_monthly_detail_required_phrases must be unique")
        )
    if not monthly_detail:
        findings.append(
            Finding(schema_path, "prompt_monthly_detail_required_phrases must not be empty")
        )
    for phrase in monthly_detail:
        if not isinstance(phrase, str) or not phrase.strip():
            findings.append(
                Finding(
                    schema_path,
                    "prompt_monthly_detail_required_phrases entries must be "
                    "non-empty strings",
                )
            )
            break
    else:
        required_monthly_detail = {
            'Communicate progress to stakeholder',
            'Update risk register (quarterly, minimum)',
        }
        if monthly_detail and not required_monthly_detail <= set(monthly_detail):
            findings.append(
                Finding(
                    schema_path,
                    "prompt_monthly_detail_required_phrases must include "
                    "Update risk register/Communicate progress",
                )
            )

    docs_residual = list(inventory.get("prompt_docs_residual_required_phrases", ()))
    if len(docs_residual) != len(set(docs_residual)):
        findings.append(
            Finding(schema_path, "prompt_docs_residual_required_phrases must be unique")
        )
    if not docs_residual:
        findings.append(
            Finding(schema_path, "prompt_docs_residual_required_phrases must not be empty")
        )
    for phrase in docs_residual:
        if not isinstance(phrase, str) or not phrase.strip():
            findings.append(
                Finding(
                    schema_path,
                    "prompt_docs_residual_required_phrases entries must be "
                    "non-empty strings",
                )
            )
            break
    else:
        required_docs_residual = {
            'See postmortem.md (incident log)',
            'Update them whenever:',
            'postmortem.md (incident log)',
        }
        if docs_residual and not required_docs_residual <= set(docs_residual):
            findings.append(
                Finding(
                    schema_path,
                    "prompt_docs_residual_required_phrases must include "
                    "See postmortem.md/postmortem incident log/Update them whenever",
                )
            )

    matrix_resolutions = list(inventory.get("prompt_matrix_resolutions_required_phrases", ()))
    if len(matrix_resolutions) != len(set(matrix_resolutions)):
        findings.append(
            Finding(schema_path, "prompt_matrix_resolutions_required_phrases must be unique")
        )
    if not matrix_resolutions:
        findings.append(
            Finding(schema_path, "prompt_matrix_resolutions_required_phrases must not be empty")
        )
    for phrase in matrix_resolutions:
        if not isinstance(phrase, str) or not phrase.strip():
            findings.append(
                Finding(
                    schema_path,
                    "prompt_matrix_resolutions_required_phrases entries must be "
                    "non-empty strings",
                )
            )
            break
    else:
        required_matrix_resolutions = {
            'Redesign contract interface or reduce scope.',
            'Reduce scope, increase risk, escalate to stakeholder.',
            'Weigh risk tolerance. Choose testnet approach to validate.',
        }
        if matrix_resolutions and not required_matrix_resolutions <= set(matrix_resolutions):
            findings.append(
                Finding(
                    schema_path,
                    "prompt_matrix_resolutions_required_phrases must include "
                    "Weigh risk tolerance/Redesign contract/hybrid RSA/Reduce scope",
                )
            )

    usage_detail = list(inventory.get("prompt_usage_detail_required_phrases", ()))
    if len(usage_detail) != len(set(usage_detail)):
        findings.append(
            Finding(schema_path, "prompt_usage_detail_required_phrases must be unique")
        )
    if not usage_detail:
        findings.append(
            Finding(schema_path, "prompt_usage_detail_required_phrases must not be empty")
        )
    for phrase in usage_detail:
        if not isinstance(phrase, str) or not phrase.strip():
            findings.append(
                Finding(
                    schema_path,
                    "prompt_usage_detail_required_phrases entries must be "
                    "non-empty strings",
                )
            )
            break
    else:
        required_usage_detail = {
            '(Claude, GPT, Gemini, etc.)',
            '**Fill in the [INSERT PROJECT-SPECIFIC INFO HERE] sections**',
            'Agent Action:',
        }
        if usage_detail and not required_usage_detail <= set(usage_detail):
            findings.append(
                Finding(
                    schema_path,
                    "prompt_usage_detail_required_phrases must include "
                    "Agent Action/Fill in INSERT/Claude GPT Gemini/Copy template",
                )
            )

    prompt_matrix_rows = list(
        inventory.get("prompt_matrix_rows_required_phrases", ())
    )
    if len(prompt_matrix_rows) != len(set(prompt_matrix_rows)):
        findings.append(
            Finding(schema_path, "prompt_matrix_rows_required_phrases must be unique")
        )
    if not prompt_matrix_rows:
        findings.append(
            Finding(
                schema_path,
                "prompt_matrix_rows_required_phrases must not be empty",
            )
        )
    for phrase in prompt_matrix_rows:
        if not isinstance(phrase, str) or not phrase.strip():
            findings.append(
                Finding(
                    schema_path,
                    "prompt_matrix_rows_required_phrases entries must be "
                    "non-empty strings",
                )
            )
            break
    else:
        required_prompt_matrix_rows = {
            '| **Algorithm Complexity** (Quantum says "too complex", Blockchain says '
            '"necessary") | Weigh risk tolerance. Choose testnet approach to validate. |',
            '| **Gas Cost** (Blockchain says "over budget", On-Device says "can\'t afford") | '
            'Redesign contract interface or reduce scope. |',
            '| **Crypto Algorithm** (On-Device says "RSA too slow", Quantum says "must be RSA") '
            '| Use hybrid (post-quantum + RSA), implement staged migration. |',
        ...
        if prompt_matrix_rows and not required_prompt_matrix_rows <= set(
            prompt_matrix_rows
        ):
            findings.append(
                Finding(
                    schema_path,
                    "prompt_matrix_rows_required_phrases must include "
                    "Algorithm Complexity/Gas Cost/Crypto Algorithm/Timeline rows",
                )
            )

    prompt_monthly_checkboxes = list(
        inventory.get("prompt_monthly_checkboxes_required_phrases", ())
    )
    if len(prompt_monthly_checkboxes) != len(set(prompt_monthly_checkboxes)):
        findings.append(
            Finding(schema_path, "prompt_monthly_checkboxes_required_phrases must be unique")
        )
    if not prompt_monthly_checkboxes:
        findings.append(
            Finding(
                schema_path,
                "prompt_monthly_checkboxes_required_phrases must not be empty",
            )
        )
    for phrase in prompt_monthly_checkboxes:
        if not isinstance(phrase, str) or not phrase.strip():
            findings.append(
                Finding(
                    schema_path,
                    "prompt_monthly_checkboxes_required_phrases entries must be "
                    "non-empty strings",
                )
            )
            break
    else:
        required_prompt_monthly_checkboxes = {
            '- [ ] Review all recent decisions in postmortem.md',
            '- [ ] Check agent success metrics (are they meeting targets?)',
            '- [ ] Identify any emerging conflicts (before they escalate)',
            '- [ ] Update risk register (quarterly, minimum)',
        }
        if prompt_monthly_checkboxes and not required_prompt_monthly_checkboxes <= set(
            prompt_monthly_checkboxes
        ):
            findings.append(
                Finding(
                    schema_path,
                    "prompt_monthly_checkboxes_required_phrases must include "
                    "checkbox Review/Check/Identify/Update/Communicate",
                )
            )

    prompt_usage_numbered = list(
        inventory.get("prompt_usage_numbered_required_phrases", ())
    )
    if len(prompt_usage_numbered) != len(set(prompt_usage_numbered)):
        findings.append(
            Finding(schema_path, "prompt_usage_numbered_required_phrases must be unique")
        )
    if not prompt_usage_numbered:
        findings.append(
            Finding(
                schema_path,
                "prompt_usage_numbered_required_phrases must not be empty",
            )
        )
    for phrase in prompt_usage_numbered:
        if not isinstance(phrase, str) or not phrase.strip():
            findings.append(
                Finding(
                    schema_path,
                    "prompt_usage_numbered_required_phrases entries must be "
                    "non-empty strings",
                )
            )
            break
    else:
        required_prompt_usage_numbered = {
            '1. **Copy the relevant prompt template** (above)',
            '2. **Fill in the [INSERT PROJECT-SPECIFIC INFO HERE] sections**',
            "3. **Paste into the LLM's system prompt** (Claude, GPT, Gemini, etc.)",
            '4. **Provide the agent with access to**:',
        }
        if prompt_usage_numbered and not required_prompt_usage_numbered <= set(
            prompt_usage_numbered
        ):
            findings.append(
                Finding(
                    schema_path,
                    "prompt_usage_numbered_required_phrases must include "
                    "Copy/Fill/Paste/Provide/Run numbered steps",
                )
            )

    prompt_example_steps = list(
        inventory.get("prompt_example_steps_required_phrases", ())
    )
    if len(prompt_example_steps) != len(set(prompt_example_steps)):
        findings.append(
            Finding(schema_path, "prompt_example_steps_required_phrases must be unique")
        )
    if not prompt_example_steps:
        findings.append(
            Finding(
                schema_path,
                "prompt_example_steps_required_phrases must not be empty",
            )
        )
    for phrase in prompt_example_steps:
        if not isinstance(phrase, str) or not phrase.strip():
            findings.append(
                Finding(
                    schema_path,
                    "prompt_example_steps_required_phrases entries must be "
                    "non-empty strings",
                )
            )
            break
    else:
        required_prompt_example_steps = {
            '1. Reads ./agentic_flows/scratchpad.txt (finds pending quantum mint task)',
            '2. Designs Cirq circuit for factorization',
            '3. Optimizes for mobile constraints (2MB RAM max)',
            '4. Validates using CRYSTALS-Kyber (NIST post-quantum standard)',
        }
        if prompt_example_steps and not required_prompt_example_steps <= set(
            prompt_example_steps
        ):
            findings.append(
                Finding(
                    schema_path,
                    "prompt_example_steps_required_phrases must include "
                    "Reads scratchpad/Designs Cirq/Optimizes/Validates/Updates/approves",
                )
            )

    prompt_living_bullets = list(
        inventory.get("prompt_living_bullets_required_phrases", ())
    )
    if len(prompt_living_bullets) != len(set(prompt_living_bullets)):
        findings.append(
            Finding(schema_path, "prompt_living_bullets_required_phrases must be unique")
        )
    if not prompt_living_bullets:
        findings.append(
            Finding(
                schema_path,
                "prompt_living_bullets_required_phrases must not be empty",
            )
        )
    for phrase in prompt_living_bullets:
        if not isinstance(phrase, str) or not phrase.strip():
            findings.append(
                Finding(
                    schema_path,
                    "prompt_living_bullets_required_phrases entries must be "
                    "non-empty strings",
                )
            )
            break
    else:
        required_prompt_living_bullets = {
            'These prompts are **living documents**. Update them whenever:',
            '- AGENTS.md constraints change',
            '- A new escalation pattern emerges',
            '- Risk tolerance thresholds change (quarterly)',
        }
        if prompt_living_bullets and not required_prompt_living_bullets <= set(
            prompt_living_bullets
        ):
            findings.append(
                Finding(
                    schema_path,
                    "prompt_living_bullets_required_phrases must include "
                    "Update them whenever/constraints change/escalation pattern/tools",
                )
            )

    prompt_quantum_resp_numbered = list(
        inventory.get("prompt_quantum_resp_numbered_required_phrases", ())
    )
    if len(prompt_quantum_resp_numbered) != len(set(prompt_quantum_resp_numbered)):
        findings.append(
            Finding(schema_path, "prompt_quantum_resp_numbered_required_phrases must be unique")
        )
    if not prompt_quantum_resp_numbered:
        findings.append(
            Finding(
                schema_path,
                "prompt_quantum_resp_numbered_required_phrases must not be empty",
            )
        )
    for phrase in prompt_quantum_resp_numbered:
        if not isinstance(phrase, str) or not phrase.strip():
            findings.append(
                Finding(
                    schema_path,
                    "prompt_quantum_resp_numbered_required_phrases entries must be "
                    "non-empty strings",
                )
            )
            break
    else:
        required_prompt_quantum_resp_numbered = {
            '1. Design quantum algorithms for cryptographic operations',
            '2. Optimize Cirq circuits for target hardware',
            '3. Validate quantum-safe properties (post-quantum crypto)',
            '4. Simulate quantum timelines and collective consciousness models (your "Mickey '
            '18" concept)',
        }
        if prompt_quantum_resp_numbered and not required_prompt_quantum_resp_numbered <= set(
            prompt_quantum_resp_numbered
        ):
            findings.append(
                Finding(
                    schema_path,
                    "prompt_quantum_resp_numbered_required_phrases must include "
                    "Design quantum/Optimize Cirq/Validate/Mickey 18/Escalate",
                )
            )

    prompt_bc_resp_numbered = list(
        inventory.get("prompt_bc_resp_numbered_required_phrases", ())
    )
    if len(prompt_bc_resp_numbered) != len(set(prompt_bc_resp_numbered)):
        findings.append(
            Finding(schema_path, "prompt_bc_resp_numbered_required_phrases must be unique")
        )
    if not prompt_bc_resp_numbered:
        findings.append(
            Finding(
                schema_path,
                "prompt_bc_resp_numbered_required_phrases must not be empty",
            )
        )
    for phrase in prompt_bc_resp_numbered:
        if not isinstance(phrase, str) or not phrase.strip():
            findings.append(
                Finding(
                    schema_path,
                    "prompt_bc_resp_numbered_required_phrases entries must be "
                    "non-empty strings",
                )
            )
            break
    else:
        required_prompt_bc_resp_numbered = {
            '1. Design multi-chain smart contract architecture',
            '2. Implement quantum-resistant consensus logic',
            '3. Define cross-chain state synchronization protocols',
            '4. Audit contracts for quantum vulnerabilities',
        }
        if prompt_bc_resp_numbered and not required_prompt_bc_resp_numbered <= set(
            prompt_bc_resp_numbered
        ):
            findings.append(
                Finding(
                    schema_path,
                    "prompt_bc_resp_numbered_required_phrases must include "
                    "Design multi-chain/Implement/Define/Audit/Estimate gas/Escalate",
                )
            )

    prompt_edge_resp_numbered = list(
        inventory.get("prompt_edge_resp_numbered_required_phrases", ())
    )
    if len(prompt_edge_resp_numbered) != len(set(prompt_edge_resp_numbered)):
        findings.append(
            Finding(schema_path, "prompt_edge_resp_numbered_required_phrases must be unique")
        )
    if not prompt_edge_resp_numbered:
        findings.append(
            Finding(
                schema_path,
                "prompt_edge_resp_numbered_required_phrases must not be empty",
            )
        )
    for phrase in prompt_edge_resp_numbered:
        if not isinstance(phrase, str) or not phrase.strip():
            findings.append(
                Finding(
                    schema_path,
                    "prompt_edge_resp_numbered_required_phrases entries must be "
                    "non-empty strings",
                )
            )
            break
    else:
        required_prompt_edge_resp_numbered = {
            '1. Implement quantum-safe cryptography on Android/iOS',
            '2. Design data isolation ("walled garden") architecture',
            '3. Manage secure key lifecycle (storage, rotation, deletion)',
            '4. Optimize for mobile constraints (battery, memory, CPU)',
        }
        if prompt_edge_resp_numbered and not required_prompt_edge_resp_numbered <= set(
            prompt_edge_resp_numbered
        ):
            findings.append(
                Finding(
                    schema_path,
                    "prompt_edge_resp_numbered_required_phrases must include "
                    "Implement Android/iOS/walled garden/key lifecycle/HIPAA/Escalate",
                )
            )

    prompt_orch_resp_numbered = list(
        inventory.get("prompt_orch_resp_numbered_required_phrases", ())
    )
    if len(prompt_orch_resp_numbered) != len(set(prompt_orch_resp_numbered)):
        findings.append(
            Finding(schema_path, "prompt_orch_resp_numbered_required_phrases must be unique")
        )
    if not prompt_orch_resp_numbered:
        findings.append(
            Finding(
                schema_path,
                "prompt_orch_resp_numbered_required_phrases must not be empty",
            )
        )
    for phrase in prompt_orch_resp_numbered:
        if not isinstance(phrase, str) or not phrase.strip():
            findings.append(
                Finding(
                    schema_path,
                    "prompt_orch_resp_numbered_required_phrases entries must be "
                    "non-empty strings",
                )
            )
            break
    else:
        required_prompt_orch_resp_numbered = {
            '1. Collect outputs from all three specialist agents',
            '2. Identify conflicts (if any)',
            '3. Make final go/no-go decision',
            '4. Escalate unsolvable conflicts to human',
        }
        if prompt_orch_resp_numbered and not required_prompt_orch_resp_numbered <= set(
            prompt_orch_resp_numbered
        ):
            findings.append(
                Finding(
                    schema_path,
                    "prompt_orch_resp_numbered_required_phrases must include "
                    "Collect outputs/Identify conflicts/go-no-go/Log/risk tolerance",
                )
            )

    prompt_user_input = list(
        inventory.get("prompt_user_input_required_phrases", ())
    )
    if len(prompt_user_input) != len(set(prompt_user_input)):
        findings.append(
            Finding(schema_path, "prompt_user_input_required_phrases must be unique")
        )
    if not prompt_user_input:
        findings.append(
            Finding(
                schema_path,
                "prompt_user_input_required_phrases must not be empty",
            )
        )
    for phrase in prompt_user_input:
        if not isinstance(phrase, str) or not phrase.strip():
            findings.append(
                Finding(
                    schema_path,
                    "prompt_user_input_required_phrases entries must be "
                    "non-empty strings",
                )
            )
            break
    else:
        required_prompt_user_input = {
            'User Input: "Review the proposed factorization circuit for our NFT mint operation. '
        ...
        if prompt_user_input and not required_prompt_user_input <= set(
            prompt_user_input
        ):
            findings.append(
                Finding(
                    schema_path,
                    "prompt_user_input_required_phrases must include "
                    "User Input/factorization circuit/Cirq optimization",
                )
            )

    prompt_cannot_delegate_numbered = list(
        inventory.get("prompt_cannot_delegate_numbered_required_phrases", ())
    )
    if len(prompt_cannot_delegate_numbered) != len(set(prompt_cannot_delegate_numbered)):
        findings.append(
            Finding(schema_path, "prompt_cannot_delegate_numbered_required_phrases must be unique")
        )
    if not prompt_cannot_delegate_numbered:
        findings.append(
            Finding(
                schema_path,
                "prompt_cannot_delegate_numbered_required_phrases must not be empty",
            )
        )
    for phrase in prompt_cannot_delegate_numbered:
        if not isinstance(phrase, str) or not phrase.strip():
            findings.append(
                Finding(
                    schema_path,
                    "prompt_cannot_delegate_numbered_required_phrases entries must be "
                    "non-empty strings",
                )
            )
            break
    else:
        required_prompt_cannot_delegate_numbered = {
            '1. Final go/no-go decisions',
            '2. Risk acceptance (acknowledging consequences)',
            "3. Human escalation (when agents can't decide)",
            '4. Vision articulation (Mickey 18 → technical architecture)',
        }
        if prompt_cannot_delegate_numbered and not required_prompt_cannot_delegate_numbered <= set(
            prompt_cannot_delegate_numbered
        ):
            findings.append(
                Finding(
                    schema_path,
                    "prompt_cannot_delegate_numbered_required_phrases must include "
                    "Final go-no-go/Risk acceptance/Human escalation/Vision/Stakeholder",
                )
            )

    prompt_decision_bullets = list(
        inventory.get("prompt_decision_bullets_required_phrases", ())
    )
    if len(prompt_decision_bullets) != len(set(prompt_decision_bullets)):
        findings.append(
            Finding(schema_path, "prompt_decision_bullets_required_phrases must be unique")
        )
    if not prompt_decision_bullets:
        findings.append(
            Finding(
                schema_path,
                "prompt_decision_bullets_required_phrases must not be empty",
            )
        )
    for phrase in prompt_decision_bullets:
        if not isinstance(phrase, str) or not phrase.strip():
            findings.append(
                Finding(
                    schema_path,
                    "prompt_decision_bullets_required_phrases entries must be "
                    "non-empty strings",
                )
            )
            break
    else:
        required_prompt_decision_bullets = {
            '- Trade-offs between security, performance, and usability',
            '- Prioritization (what gets built first)',
            '- Risk acceptance (can we deploy with this vulnerability?)',
            '- Timeline adjustments (can we ship on schedule?)',
        }
        if prompt_decision_bullets and not required_prompt_decision_bullets <= set(
            prompt_decision_bullets
        ):
            findings.append(
                Finding(
                    schema_path,
                    "prompt_decision_bullets_required_phrases must include "
                    "Trade-offs/Prioritization/Risk acceptance/Timeline adjustments",
                )
            )
    ...
    for phrase in PROMPT_DECISION_AUTHORITY_REQUIRED_PHRASES:
        if phrase not in body:
            findings.append(
                Finding(
                    rel,
                    f"missing locked prompt-decision-authority phrase: {phrase}",
                )
            )
    return findings


def validate_prompt_integration(root: Path) -> list[Finding]:
    rel = "AGENT-PROMPTS.md"
    path = root / rel
    if not path.is_file():
        return [Finding(rel, "prompts missing")]
    body = path.read_text(encoding="utf-8")
    findings: list[Finding] = []
    if "## Integration with AGENTS.md" not in body:
    ...
    for phrase in PROMPT_INTEGRATION_REQUIRED_PHRASES:
        if phrase not in body:
            findings.append(
                Finding(rel, f"missing locked prompt-integration phrase: {phrase}")
            )
    return findings
...
def validate_prompt_cannot_delegate(root: Path) -> list[Finding]:
    ...
    for phrase in PROMPT_CANNOT_DELEGATE_REQUIRED_PHRASES:
        if phrase not in body:
            findings.append(
                Finding(rel, f"missing locked prompt-cannot-delegate phrase: {phrase}")
            )
    return findings
...
def validate_prompt_role_blurbs(root: Path) -> list[Finding]:
    ...
    body = path.read_text(encoding="utf-8")
    findings: list[Finding] = []
    if 'System Prompt' not in body:
        findings.append(Finding(rel, 'missing System Prompt / role-blurb anchors'))
...
def validate_prompt_escalation_format(root: Path) -> list[Finding]:
    ...
    for phrase in PROMPT_ESCALATION_FORMAT_REQUIRED_PHRASES:
        if phrase not in body:
            findings.append(
                Finding(rel, f"missing locked prompt-escalation-format phrase: {phrase}")
            )
    return findings
...
def validate_prompt_constraints_detail(root: Path) -> list[Finding]:
    rel = "AGENT-PROMPTS.md"
    path = root / rel
    if not path.is_file():
        return [Finding(rel, "prompts missing")]
    body = path.read_text(encoding="utf-8")
    findings: list[Finding] = []
    if 'Never optimize beyond device constraints' not in body:
        findings.append(Finding(rel, 'missing constraints-detail Never optimize lock'))
    for phrase in PROMPT_CONSTRAINTS_DETAIL_REQUIRED_PHRASES:
        if phrase not in body:
            findings.append(
                Finding(rel, f"missing locked prompt-constraints-detail phrase: {phrase}")
            )
    return findings


def validate_prompt_triggers_detail(root: Path) -> list[Finding]:
    rel = "AGENT-PROMPTS.md"
    path = root / rel
    if not path.is_file():
        return [Finding(rel, "prompts missing")]
    body = path.read_text(encoding="utf-8")
    findings: list[Finding] = []
    if 'Quantum advantage deadline' not in body:
        findings.append(Finding(rel, 'missing triggers-detail Quantum advantage lock'))
    for phrase in PROMPT_TRIGGERS_DETAIL_REQUIRED_PHRASES:
        if phrase not in body:
            findings.append(
                Finding(rel, f"missing locked prompt-triggers-detail phrase: {phrase}")
            )
    return findings


def validate_prompt_human_fields(root: Path) -> list[Finding]:
    rel = "AGENT-PROMPTS.md"
    path = root / rel
    if not path.is_file():
        return [Finding(rel, "prompts missing")]
    body = path.read_text(encoding="utf-8")
    findings: list[Finding] = []
    if 'Situation: [What decision needs human input?]' not in body:
        findings.append(Finding(rel, 'missing human-fields Situation lock'))
    for phrase in PROMPT_HUMAN_FIELDS_REQUIRED_PHRASES:
        if phrase not in body:
            findings.append(
                Finding(rel, f"missing locked prompt-human-fields phrase: {phrase}")
            )
    return findings


def validate_prompt_context_detail(root: Path) -> list[Finding]:
    rel = "AGENT-PROMPTS.md"
    path = root / rel
    if not path.is_file():
        return [Finding(rel, "prompts missing")]
    body = path.read_text(encoding="utf-8")
    findings: list[Finding] = []
    if 'Token standards: ERC-20' not in body:
        findings.append(Finding(rel, 'missing context-detail Token standards lock'))
    for phrase in PROMPT_CONTEXT_DETAIL_REQUIRED_PHRASES:
        if phrase not in body:
            findings.append(
                Finding(rel, f"missing locked prompt-context-detail phrase: {phrase}")
            )
    return findings


def validate_prompt_expertise_detail(root: Path) -> list[Finding]:
    rel = "AGENT-PROMPTS.md"
    path = root / rel
    if not path.is_file():
        return [Finding(rel, "prompts missing")]
    body = path.read_text(encoding="utf-8")
    findings: list[Finding] = []
    if 'Circuit optimization (gate reduction, error mitigation)' not in body:
        findings.append(Finding(rel, 'missing expertise-detail Circuit optimization lock'))
    for phrase in PROMPT_EXPERTISE_DETAIL_REQUIRED_PHRASES:
        if phrase not in body:
            findings.append(
                Finding(rel, f"missing locked prompt-expertise-detail phrase: {phrase}")
            )
    return findings


def validate_prompt_related_health(root: Path) -> list[Finding]:
    rel = "AGENT-PROMPTS.md"
    path = root / rel
    if not path.is_file():
        return [Finding(rel, "prompts missing")]
    body = path.read_text(encoding="utf-8")
    findings: list[Finding] = []
    if 'See AGENTS.md Section 5.3 (Knowledge & Health Agents)' not in body:
        findings.append(Finding(rel, 'missing related-health Section 5.3 lock'))
    for phrase in PROMPT_RELATED_HEALTH_REQUIRED_PHRASES:
        if phrase not in body:
            findings.append(
                Finding(rel, f"missing locked prompt-related-health phrase: {phrase}")
            )
    return findings


def validate_prompt_tools_detail(root: Path) -> list[Finding]:
    rel = "AGENT-PROMPTS.md"
    path = root / rel
    if not path.is_file():
        return [Finding(rel, "prompts missing")]
    body = path.read_text(encoding="utf-8")
    findings: list[Finding] = []
    if 'IBM Quantum Experience' not in body:
        findings.append(Finding(rel, 'missing tools-detail IBM Quantum lock'))
    for phrase in PROMPT_TOOLS_DETAIL_REQUIRED_PHRASES:
        if phrase not in body:
            findings.append(
                Finding(rel, f"missing locked prompt-tools-detail phrase: {phrase}")
            )
    return findings


def validate_prompt_responsibilities_detail(root: Path) -> list[Finding]:
    rel = "AGENT-PROMPTS.md"
    path = root / rel
    if not path.is_file():
        return [Finding(rel, "prompts missing")]
    body = path.read_text(encoding="utf-8")
    findings: list[Finding] = []
    if 'Optimize Cirq circuits for target hardware' not in body:
        findings.append(Finding(rel, 'missing responsibilities-detail Optimize Cirq lock'))
    for phrase in PROMPT_RESPONSIBILITIES_DETAIL_REQUIRED_PHRASES:
        if phrase not in body:
            findings.append(
                Finding(rel, f"missing locked prompt-responsibilities-detail phrase: {phrase}")
            )
    return findings
...
def validate_prompt_instantiation(root: Path) -> list[Finding]:
    ...
    body = path.read_text(encoding="utf-8")
    findings: list[Finding] = []
    if '**Provide the agent with access to**:' not in body:
        findings.append(Finding(rel, 'missing instantiation Provide access lock'))
...
def validate_prompt_metrics_detail(root: Path) -> list[Finding]:
    rel = "AGENT-PROMPTS.md"
    path = root / rel
    if not path.is_file():
        return [Finding(rel, "prompts missing")]
    ...
    if 'Gate count optimized for target hardware' not in body:
        findings.append(Finding(rel, 'missing metrics-detail Gate count lock'))
    for phrase in PROMPT_METRICS_DETAIL_REQUIRED_PHRASES:
        if phrase not in body:
            findings.append(
                Finding(rel, f"missing locked prompt-metrics-detail phrase: {phrase}")
            )
    return findings


def validate_prompt_orch_metrics_detail(root: Path) -> list[Finding]:
    rel = "AGENT-PROMPTS.md"
    path = root / rel
    if not path.is_file():
        return [Finding(rel, "prompts missing")]
    ...
    if 'Apple security review: PASS' not in body:
        findings.append(Finding(rel, 'missing orch-metrics-detail Apple security review lock'))
    for phrase in PROMPT_ORCH_METRICS_DETAIL_REQUIRED_PHRASES:
        if phrase not in body:
            findings.append(
                Finding(rel, f"missing locked prompt-orch-metrics-detail phrase: {phrase}")
            )
    return findings
...
def validate_prompt_communication_detail(root: Path) -> list[Finding]:
    ...
    body = path.read_text(encoding="utf-8")
    findings: list[Finding] = []
    if 'Conservative on claims ("this is theoretically possible, but...")' not in body:
        findings.append(Finding(rel, 'missing communication-detail Conservative on claims lock'))
...
def validate_prompt_responsibilities_residual(root: Path) -> list[Finding]:
    rel = "AGENT-PROMPTS.md"
    path = root / rel
    if not path.is_file():
        return [Finding(rel, "prompts missing")]
    body = path.read_text(encoding="utf-8")
    findings: list[Finding] = []
    if 'Validate quantum-safe properties (post-quantum crypto)' not in body:
        findings.append(
            Finding(rel, 'missing responsibilities-residual Validate quantum-safe lock')
        )
    for phrase in PROMPT_RESPONSIBILITIES_RESIDUAL_REQUIRED_PHRASES:
        if phrase not in body:
            findings.append(
                Finding(rel, f"missing locked prompt-responsibilities-residual phrase: {phrase}")
            )
    return findings
...
def validate_prompt_vision_context(root: Path) -> list[Finding]:
    rel = "AGENT-PROMPTS.md"
    path = root / rel
    if not path.is_file():
        return [Finding(rel, "prompts missing")]
    ...
    if '## Current Project Vision' not in body:
        findings.append(Finding(rel, 'missing vision-context Current Project Vision lock'))
    for phrase in PROMPT_VISION_CONTEXT_REQUIRED_PHRASES:
        if phrase not in body:
            findings.append(
                Finding(rel, f"missing locked prompt-vision-context phrase: {phrase}")
            )
    return findings


def validate_prompt_context_residual(root: Path) -> list[Finding]:
    rel = "AGENT-PROMPTS.md"
    path = root / rel
    if not path.is_file():
        return [Finding(rel, "prompts missing")]
    body = path.read_text(encoding="utf-8")
    findings: list[Finding] = []
    if 'Error tolerance: < 1%' not in body:
        findings.append(Finding(rel, 'missing context-residual Error tolerance lock'))
    for phrase in PROMPT_CONTEXT_RESIDUAL_REQUIRED_PHRASES:
        if phrase not in body:
            findings.append(
                Finding(rel, f"missing locked prompt-context-residual phrase: {phrase}")
            )
    return findings


def validate_prompt_monthly_detail(root: Path) -> list[Finding]:
    rel = "AGENT-PROMPTS.md"
    path = root / rel
    if not path.is_file():
        return [Finding(rel, "prompts missing")]
    ...
    if 'Update risk register (quarterly, minimum)' not in body:
        findings.append(Finding(rel, 'missing monthly-detail Update risk register lock'))
    for phrase in PROMPT_MONTHLY_DETAIL_REQUIRED_PHRASES:
        if phrase not in body:
            findings.append(
                Finding(rel, f"missing locked prompt-monthly-detail phrase: {phrase}")
            )
    return findings


def validate_prompt_docs_residual(root: Path) -> list[Finding]:
    rel = "AGENT-PROMPTS.md"
    path = root / rel
    if not path.is_file():
        return [Finding(rel, "prompts missing")]
    body = path.read_text(encoding="utf-8")
    findings: list[Finding] = []
    if 'See postmortem.md (incident log)' not in body:
        findings.append(Finding(rel, 'missing docs-residual See postmortem.md lock'))
    for phrase in PROMPT_DOCS_RESIDUAL_REQUIRED_PHRASES:
        if phrase not in body:
            findings.append(
                Finding(rel, f"missing locked prompt-docs-residual phrase: {phrase}")
            )
    return findings


def validate_prompt_matrix_resolutions(root: Path) -> list[Finding]:
    rel = "AGENT-PROMPTS.md"
    path = root / rel
    if not path.is_file():
        return [Finding(rel, "prompts missing")]
    body = path.read_text(encoding="utf-8")
    findings: list[Finding] = []
    if 'Weigh risk tolerance. Choose testnet approach to validate.' not in body:
        findings.append(Finding(rel, 'missing matrix-resolutions Weigh risk tolerance lock'))
    for phrase in PROMPT_MATRIX_RESOLUTIONS_REQUIRED_PHRASES:
        if phrase not in body:
            findings.append(
                Finding(rel, f"missing locked prompt-matrix-resolutions phrase: {phrase}")
            )
    return findings


def validate_prompt_usage_detail(root: Path) -> list[Finding]:
    rel = "AGENT-PROMPTS.md"
    path = root / rel
    if not path.is_file():
        return [Finding(rel, "prompts missing")]
    ...
    if 'Agent Action:' not in body:
        findings.append(Finding(rel, 'missing usage-detail Agent Action lock'))
    for phrase in PROMPT_USAGE_DETAIL_REQUIRED_PHRASES:
        if phrase not in body:
            findings.append(
                Finding(rel, f"missing locked prompt-usage-detail phrase: {phrase}")
            )
    return findings


def validate_prompt_matrix_rows(root: Path) -> list[Finding]:
    rel = "AGENT-PROMPTS.md"
    path = root / rel
    if not path.is_file():
        return [Finding(rel, "prompts missing")]
    body = path.read_text(encoding="utf-8")
    findings: list[Finding] = []
    if '| **Algorithm Complexity**' not in body:
        findings.append(
            Finding(rel, 'missing matrix-rows Algorithm Complexity lock')
        )
    for phrase in PROMPT_MATRIX_ROWS_REQUIRED_PHRASES:
        if phrase not in body:
            findings.append(
                Finding(
                    rel,
                    f"missing locked prompt-matrix-rows phrase: {phrase}",
                )
            )
    return findings


def validate_prompt_monthly_checkboxes(root: Path) -> list[Finding]:
    rel = "AGENT-PROMPTS.md"
    path = root / rel
    if not path.is_file():
        return [Finding(rel, "prompts missing")]
    body = path.read_text(encoding="utf-8")
    findings: list[Finding] = []
    if '- [ ] Review all recent decisions in postmortem.md' not in body:
        findings.append(
            Finding(rel, 'missing monthly-checkboxes Review postmortem lock')
        )
    for phrase in PROMPT_MONTHLY_CHECKBOXES_REQUIRED_PHRASES:
        if phrase not in body:
            findings.append(
                Finding(
                    rel,
                    f"missing locked prompt-monthly-checkboxes phrase: {phrase}",
                )
            )
    return findings


def validate_prompt_usage_numbered(root: Path) -> list[Finding]:
    rel = "AGENT-PROMPTS.md"
    path = root / rel
    if not path.is_file():
        return [Finding(rel, "prompts missing")]
    body = path.read_text(encoding="utf-8")
    findings: list[Finding] = []
    if '1. **Copy the relevant prompt template** (above)' not in body:
        findings.append(
            Finding(rel, 'missing usage-numbered Copy template lock')
        )
    for phrase in PROMPT_USAGE_NUMBERED_REQUIRED_PHRASES:
        if phrase not in body:
            findings.append(
                Finding(
                    rel,
                    f"missing locked prompt-usage-numbered phrase: {phrase}",
                )
            )
    return findings


def validate_prompt_example_steps(root: Path) -> list[Finding]:
    rel = "AGENT-PROMPTS.md"
    path = root / rel
    if not path.is_file():
        return [Finding(rel, "prompts missing")]
    body = path.read_text(encoding="utf-8")
    findings: list[Finding] = []
    if '1. Reads ./agentic_flows/scratchpad.txt (finds pending quantum mint task)' not in body:
        findings.append(
            Finding(rel, 'missing example-steps Reads scratchpad lock')
        )
    for phrase in PROMPT_EXAMPLE_STEPS_REQUIRED_PHRASES:
        if phrase not in body:
            findings.append(
                Finding(
                    rel,
                    f"missing locked prompt-example-steps phrase: {phrase}",
                )
            )
    return findings


def validate_prompt_living_bullets(root: Path) -> list[Finding]:
    rel = "AGENT-PROMPTS.md"
    path = root / rel
    if not path.is_file():
        return [Finding(rel, "prompts missing")]
    body = path.read_text(encoding="utf-8")
    findings: list[Finding] = []
    if 'These prompts are **living documents**. Update them whenever:' not in body:
        findings.append(
            Finding(rel, 'missing living-bullets Update them whenever lock')
        )
    for phrase in PROMPT_LIVING_BULLETS_REQUIRED_PHRASES:
        if phrase not in body:
            findings.append(
                Finding(
                    rel,
                    f"missing locked prompt-living-bullets phrase: {phrase}",
                )
            )
    return findings


def validate_prompt_quantum_resp_numbered(root: Path) -> list[Finding]:
    rel = "AGENT-PROMPTS.md"
    path = root / rel
    if not path.is_file():
        return [Finding(rel, "prompts missing")]
    body = path.read_text(encoding="utf-8")
    findings: list[Finding] = []
    if '1. Design quantum algorithms for cryptographic operations' not in body:
        findings.append(
            Finding(rel, 'missing quantum-resp-numbered Design quantum lock')
        )
    for phrase in PROMPT_QUANTUM_RESP_NUMBERED_REQUIRED_PHRASES:
        if phrase not in body:
            findings.append(
                Finding(
                    rel,
                    f"missing locked prompt-quantum-resp-numbered phrase: {phrase}",
                )
            )
    return findings


def validate_prompt_bc_resp_numbered(root: Path) -> list[Finding]:
    rel = "AGENT-PROMPTS.md"
    path = root / rel
    if not path.is_file():
        return [Finding(rel, "prompts missing")]
    body = path.read_text(encoding="utf-8")
    findings: list[Finding] = []
    if '1. Design multi-chain smart contract architecture' not in body:
        findings.append(
            Finding(rel, 'missing bc-resp-numbered Design multi-chain lock')
        )
    for phrase in PROMPT_BC_RESP_NUMBERED_REQUIRED_PHRASES:
        if phrase not in body:
            findings.append(
                Finding(
                    rel,
                    f"missing locked prompt-bc-resp-numbered phrase: {phrase}",
                )
            )
    return findings


def validate_prompt_edge_resp_numbered(root: Path) -> list[Finding]:
    rel = "AGENT-PROMPTS.md"
    path = root / rel
    if not path.is_file():
        return [Finding(rel, "prompts missing")]
    body = path.read_text(encoding="utf-8")
    findings: list[Finding] = []
    if '1. Implement quantum-safe cryptography on Android/iOS' not in body:
        findings.append(
            Finding(rel, 'missing edge-resp-numbered Implement quantum-safe lock')
        )
    for phrase in PROMPT_EDGE_RESP_NUMBERED_REQUIRED_PHRASES:
        if phrase not in body:
            findings.append(
                Finding(
                    rel,
                    f"missing locked prompt-edge-resp-numbered phrase: {phrase}",
                )
            )
    return findings


def validate_prompt_orch_resp_numbered(root: Path) -> list[Finding]:
    rel = "AGENT-PROMPTS.md"
    path = root / rel
    if not path.is_file():
        return [Finding(rel, "prompts missing")]
    body = path.read_text(encoding="utf-8")
    findings: list[Finding] = []
    if '1. Collect outputs from all three specialist agents' not in body:
        findings.append(
            Finding(rel, 'missing orch-resp-numbered Collect outputs lock')
        )
    for phrase in PROMPT_ORCH_RESP_NUMBERED_REQUIRED_PHRASES:
        if phrase not in body:
            findings.append(
                Finding(
                    rel,
                    f"missing locked prompt-orch-resp-numbered phrase: {phrase}",
                )
            )
    return findings


def validate_prompt_user_input(root: Path) -> list[Finding]:
    rel = "AGENT-PROMPTS.md"
    path = root / rel
    if not path.is_file():
        return [Finding(rel, "prompts missing")]
    body = path.read_text(encoding="utf-8")
    findings: list[Finding] = []
    if 'User Input: "Review the proposed factorization circuit' not in body:
        findings.append(
            Finding(rel, 'missing user-input Review factorization lock')
        )
    for phrase in PROMPT_USER_INPUT_REQUIRED_PHRASES:
        if phrase not in body:
            findings.append(
                Finding(
                    rel,
                    f"missing locked prompt-user-input phrase: {phrase}",
                )
            )
    return findings


def validate_prompt_cannot_delegate_numbered(root: Path) -> list[Finding]:
    rel = "AGENT-PROMPTS.md"
    path = root / rel
    if not path.is_file():
        return [Finding(rel, "prompts missing")]
    body = path.read_text(encoding="utf-8")
    findings: list[Finding] = []
    if '1. Final go/no-go decisions' not in body:
        findings.append(
            Finding(rel, 'missing cannot-delegate-numbered Final go/no-go lock')
        )
    for phrase in PROMPT_CANNOT_DELEGATE_NUMBERED_REQUIRED_PHRASES:
        if phrase not in body:
            findings.append(
                Finding(
                    rel,
                    f"missing locked prompt-cannot-delegate-numbered phrase: {phrase}",
                )
            )
    return findings


def validate_prompt_decision_bullets(root: Path) -> list[Finding]:
    rel = "AGENT-PROMPTS.md"
    path = root / rel
    if not path.is_file():
        return [Finding(rel, "prompts missing")]
    body = path.read_text(encoding="utf-8")
    findings: list[Finding] = []
    if '- Trade-offs between security, performance, and usability' not in body:
        findings.append(
            Finding(rel, 'missing decision-bullets Trade-offs lock')
        )
    for phrase in PROMPT_DECISION_BULLETS_REQUIRED_PHRASES:
        if phrase not in body:
            findings.append(
                Finding(
                    rel,
                    f"missing locked prompt-decision-bullets phrase: {phrase}",
                )
            )
    return findings
...
VALIDATORS: dict[str, ValidatorFn] = {
    ...
    "prompt-integration": validate_prompt_integration,
    ...
    "prompt-constraints-detail": validate_prompt_constraints_detail,
    "prompt-triggers-detail": validate_prompt_triggers_detail,
    "prompt-human-fields": validate_prompt_human_fields,
    "prompt-context-detail": validate_prompt_context_detail,
    "prompt-expertise-detail": validate_prompt_expertise_detail,
    "prompt-related-health": validate_prompt_related_health,
    ...
    "prompt-metrics-detail": validate_prompt_metrics_detail,
    "prompt-orch-metrics-detail": validate_prompt_orch_metrics_detail,
    ...
    "prompt-responsibilities-residual": validate_prompt_responsibilities_residual,
    "prompt-expertise-residual": validate_prompt_expertise_residual,
    "prompt-vision-context": validate_prompt_vision_context,
    "prompt-context-residual": validate_prompt_context_residual,
    "prompt-monthly-detail": validate_prompt_monthly_detail,
    "prompt-docs-residual": validate_prompt_docs_residual,
    "prompt-matrix-resolutions": validate_prompt_matrix_resolutions,
    "prompt-usage-detail": validate_prompt_usage_detail,
    "prompt-matrix-rows": validate_prompt_matrix_rows,
    "prompt-monthly-checkboxes": validate_prompt_monthly_checkboxes,
    "prompt-usage-numbered": validate_prompt_usage_numbered,
    "prompt-example-steps": validate_prompt_example_steps,
    "prompt-living-bullets": validate_prompt_living_bullets,
    "prompt-quantum-resp-numbered": validate_prompt_quantum_resp_numbered,
    "prompt-bc-resp-numbered": validate_prompt_bc_resp_numbered,
    "prompt-edge-resp-numbered": validate_prompt_edge_resp_numbered,
    "prompt-orch-resp-numbered": validate_prompt_orch_resp_numbered,
    "prompt-user-input": validate_prompt_user_input,
    "prompt-cannot-delegate-numbered": validate_prompt_cannot_delegate_numbered,
    "prompt-decision-bullets": validate_prompt_decision_bullets,
