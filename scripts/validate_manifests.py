...
- Hydration findings category / LIST A researchable / PHASE 4 issues locks
...
HYDRATION_ISSUES_REQUIRED_PHRASES: tuple[str, ...] = (
    ...
    "Add CONTRIBUTING.md",
    "Add SECURITY.md",
    "Scaffold agentic_flows/",
    "Scaffold quantum_circuits/",
    "Scaffold contracts/",
    "Scaffold mobile/",
)
...
def _inventory_lock_consistency(
    ...
    if not findings_phrases:
        findings.append(
            Finding(
                schema_path, "hydration_findings_required_phrases must not be empty"
            )
        )
    ...
    if not issues:
        findings.append(
            Finding(
                schema_path, "hydration_issues_required_phrases must not be empty"
            )
        )
    for phrase in issues:
        if not isinstance(phrase, str) or not phrase.strip():
