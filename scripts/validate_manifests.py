...
def _inventory_lock_consistency(
    ...
    if len(phases) != len(set(phases)):
        findings.append(
            Finding(schema_path, "implementation_phases_required_phrases must be unique")
        )
    if not phases:
        findings.append(
            Finding(
                schema_path, "implementation_phases_required_phrases must not be empty"
            )
        )
        ...
        if phases and not required_phases <= set(phases):
            findings.append(
                Finding(
                    ...
                    "Implementation/Phase 1/Phase 6/append-only rule",
                )
            )
    ...
    if len(tools) != len(set(tools)):
        findings.append(
            Finding(schema_path, "implementation_tools_required_phrases must be unique")
        )
    if not tools:
        findings.append(
            Finding(
                schema_path, "implementation_tools_required_phrases must not be empty"
            )
        )
    for phrase in tools:
        if not isinstance(phrase, str) or not phrase.strip():
        ...
        if tools and not required_tools <= set(tools):
            findings.append(
                Finding(
                    ...
                    "Checklist/Quantum/Orchestration/Goose",
                )
            )
        ...
        if success and not required_success <= set(success):
            findings.append(
                Finding(
                    ...
                    "Criteria/Week 1/Month/mainnet readiness",
                )
            )
...
def validate_implementation_tools(root: Path) -> list[Finding]:
    ...
    if "**Quantum (Days 2-3)**:" not in text:
        findings.append(Finding(rel, "missing Quantum tools section"))
    if "**Orchestration (Days 5-6)**:" not in text:
        findings.append(Finding(rel, "missing Orchestration tools section"))
...
def validate_implementation_success(root: Path) -> list[Finding]:
    ...
    if "**By End of Week 1**:" not in text:
        findings.append(Finding(rel, "missing By End of Week 1 criteria"))
    if "**By End of Month**:" not in text:
        findings.append(Finding(rel, "missing By End of Month criteria"))
