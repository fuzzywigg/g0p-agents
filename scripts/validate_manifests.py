...
IMPLEMENTATION_TIMEOUT_DETAIL_REQUIRED_PHRASES: tuple[str, ...] = (
...
def validate_packaging_inventory(root: Path) -> list[Finding]:
    ...
    if (
        tuple(inventory.get("goose_timeout_detail_required_phrases", ()))
        != GOOSE_TIMEOUT_DETAIL_REQUIRED_PHRASES
    ):
        findings.append(_lock_mismatch(schema_path, "goose_timeout_detail_required_phrases"))

    if (
        tuple(inventory.get("goose_deadline_detail_required_phrases", ()))
        != GOOSE_DEADLINE_DETAIL_REQUIRED_PHRASES
    ):
        findings.append(_lock_mismatch(schema_path, "goose_deadline_detail_required_phrases"))

    if (
        tuple(inventory.get("constitution_deadline_detail_required_phrases", ()))
        != CONSTITUTION_DEADLINE_DETAIL_REQUIRED_PHRASES
    ):
        findings.append(
            _lock_mismatch(schema_path, "constitution_deadline_detail_required_phrases")
        )

    if (
        tuple(inventory.get("implementation_timeout_detail_required_phrases", ()))
        != IMPLEMENTATION_TIMEOUT_DETAIL_REQUIRED_PHRASES
    ):
        findings.append(
            _lock_mismatch(schema_path, "implementation_timeout_detail_required_phrases")
        )
...
def _inventory_lock_consistency(
    ...
    goose_timeouts = list(inventory.get("goose_timeout_detail_required_phrases", ()))
    if len(goose_timeouts) != len(set(goose_timeouts)):
        findings.append(
            Finding(schema_path, "goose_timeout_detail_required_phrases must be unique")
        )
    if not goose_timeouts:
        findings.append(
            Finding(schema_path, "goose_timeout_detail_required_phrases must not be empty")
        )
    for phrase in goose_timeouts:
        if not isinstance(phrase, str) or not phrase.strip():
            findings.append(
                Finding(
                    schema_path,
                    "goose_timeout_detail_required_phrases entries must be "
                    "non-empty strings",
                )
            )
            break
        ...
        if goose_timeouts and not required_goose_timeouts <= set(goose_timeouts):
            findings.append(
                Finding(
                    schema_path,
                    "goose_timeout_detail_required_phrases must include Long timeout/"
                    "timeout 300/timeout 600",
                )
            )

    goose_deadlines = list(inventory.get("goose_deadline_detail_required_phrases", ()))
    if len(goose_deadlines) != len(set(goose_deadlines)):
        findings.append(
            Finding(schema_path, "goose_deadline_detail_required_phrases must be unique")
        )
    if not goose_deadlines:
        findings.append(
            Finding(schema_path, "goose_deadline_detail_required_phrases must not be empty")
        )
    for phrase in goose_deadlines:
        if not isinstance(phrase, str) or not phrase.strip():
            findings.append(
                Finding(
                    schema_path,
                    "goose_deadline_detail_required_phrases entries must be "
                    "non-empty strings",
                )
            )
            break
        ...
        if goose_deadlines and not required_goose_deadlines <= set(goose_deadlines):
            findings.append(
                Finding(
                    schema_path,
                    "goose_deadline_detail_required_phrases must include Deadline/"
                    "Created inheritance anchors",
                )
            )

    constitution_deadlines = list(
        inventory.get("constitution_deadline_detail_required_phrases", ())
    )
    if len(constitution_deadlines) != len(set(constitution_deadlines)):
        findings.append(
            Finding(
                schema_path, "constitution_deadline_detail_required_phrases must be unique"
            )
        )
    if not constitution_deadlines:
        findings.append(
            Finding(
                schema_path,
                "constitution_deadline_detail_required_phrases must not be empty",
            )
        )
    for phrase in constitution_deadlines:
        if not isinstance(phrase, str) or not phrase.strip():
            findings.append(
                Finding(
                    schema_path,
                    "constitution_deadline_detail_required_phrases entries must be "
                    "non-empty strings",
                )
            )
            break
        ...
        if constitution_deadlines and not required_constitution_deadlines <= set(
            ...
            findings.append(
                Finding(
                    schema_path,
                    "constitution_deadline_detail_required_phrases must include Nested/"
                    "Deadline/Quality vs. Deadline",
                )
            )

    implementation_timeouts = list(
        inventory.get("implementation_timeout_detail_required_phrases", ())
    )
    if len(implementation_timeouts) != len(set(implementation_timeouts)):
        findings.append(
            Finding(
                schema_path, "implementation_timeout_detail_required_phrases must be unique"
            )
        )
    if not implementation_timeouts:
        findings.append(
            Finding(
                schema_path,
                "implementation_timeout_detail_required_phrases must not be empty",
            )
        )
    for phrase in implementation_timeouts:
        if not isinstance(phrase, str) or not phrase.strip():
            findings.append(
                Finding(
                    schema_path,
                    "implementation_timeout_detail_required_phrases entries must be "
                    "non-empty strings",
                )
            )
            break
        ...
        if implementation_timeouts and not required_implementation_timeouts <= set(
            ...
            findings.append(
                Finding(
                    schema_path,
                    "implementation_timeout_detail_required_phrases must include Adjust "
...
def validate_goose_timeout_detail(root: Path) -> list[Finding]:
    rel = "GOOSE-RECIPES.md"
    path = root / rel
    if not path.is_file():
        return [Finding(rel, "GOOSE-RECIPES.md missing")]
    ...
    for phrase in GOOSE_TIMEOUT_DETAIL_REQUIRED_PHRASES:
        if phrase not in body:
            findings.append(
                Finding(rel, f"missing locked goose-timeout-detail phrase: {phrase}")
            )
    return findings


def validate_goose_deadline_detail(root: Path) -> list[Finding]:
    rel = "GOOSE-RECIPES.md"
    path = root / rel
    if not path.is_file():
        return [Finding(rel, "GOOSE-RECIPES.md missing")]
    ...
    for phrase in GOOSE_DEADLINE_DETAIL_REQUIRED_PHRASES:
        if phrase not in body:
            findings.append(
                Finding(rel, f"missing locked goose-deadline-detail phrase: {phrase}")
            )
    return findings


def validate_constitution_deadline_detail(root: Path) -> list[Finding]:
    rel = "AGENTS-v2.2.md"
    path = root / rel
    if not path.is_file():
        return [Finding(rel, "constitution missing")]
    ...
    for phrase in CONSTITUTION_DEADLINE_DETAIL_REQUIRED_PHRASES:
        if phrase not in body:
            findings.append(
                Finding(rel, f"missing locked constitution-deadline-detail phrase: {phrase}")
            )
    return findings


def validate_implementation_timeout_detail(root: Path) -> list[Finding]:
    rel = "IMPLEMENTATION-GUIDE.md"
    path = root / rel
    if not path.is_file():
        return [Finding(rel, "IMPLEMENTATION-GUIDE.md missing")]
    ...
    for phrase in IMPLEMENTATION_TIMEOUT_DETAIL_REQUIRED_PHRASES:
        if phrase not in body:
            findings.append(
                Finding(
                    rel, f"missing locked implementation-timeout-detail phrase: {phrase}"
                )
            )
    return findings
...
VALIDATORS: dict[str, ValidatorFn] = {
    ...
    "constitution-deadline-detail": validate_constitution_deadline_detail,
    ...
    "goose-timeout-detail": validate_goose_timeout_detail,
    "goose-deadline-detail": validate_goose_deadline_detail,
    ...
    "implementation-timeout-detail": validate_implementation_timeout_detail,
