...
CONTRIBUTING_LOCAL_REQUIRED_PHRASES: tuple[str, ...] = (
    ...
    "validate_manifests.py --list-validators",
    "pytest --cov=scripts",
    "Packaging inventory",
...
def _inventory_lock_consistency(
    ...
    if not local:
        findings.append(
            Finding(schema_path, "contributing_local_required_phrases must not be empty")
        )
    for phrase in local:
        if not isinstance(phrase, str) or not phrase.strip():
            findings.append(
        ...
        if local and not required_local <= set(local):
            findings.append(
                Finding(
                    ...
                    "validation/ruff/list-validators",
                )
            )
        ...
        if governance and not required_gov <= set(governance):
            findings.append(
                Finding(
                    ...
                    "Governance/Structural changes/routing matrix",
                )
            )
