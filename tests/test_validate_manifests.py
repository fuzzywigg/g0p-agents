...
def test_live_v8_validators() -> None:
    ...
    assert vm.INVENTORY_VERSION == 43
    assert len(vm.VALIDATORS) == 146
    assert vm.MIN_VALIDATOR_COUNT == 146
...
def test_live_v12_validators() -> None:
    ...
    assert inventory["min_validator_count"] == 146
...
def test_live_v43_hydration_deepener_validators() -> None:
    ...
    assert len(vm.VALIDATORS) == 146
    assert vm.MIN_VALIDATOR_COUNT == 146
    assert "hydration-meta" in vm.VALIDATORS
    assert "hydration-identity-detail" in vm.VALIDATORS
    assert "hydration-git-detail" in vm.VALIDATORS
    assert "hydration-phase2" in vm.VALIDATORS
    assert "hydration-phase5" in vm.VALIDATORS
    inventory = json.loads(
        (REPO_ROOT / "schemas" / "packaging-inventory.json").read_text(encoding="utf-8")
    )
    assert inventory["version"] == 43
    ...
    assert sorted(vm.VALIDATORS) == inventory["validator_names"]
    assert "Packaging inventory v43" in (
        REPO_ROOT / "CONTRIBUTING.md"
    ).read_text(encoding="utf-8")
    # prior v42 leftovers still green
    assert vm.validate_hydration_phase1(REPO_ROOT) == []
    assert vm.validate_hydration_list_a(REPO_ROOT) == []
    assert vm.validate_hydration_resolved(REPO_ROOT) == []
    assert vm.validate_hydration_phase4(REPO_ROOT) == []
...
def test_v43_hydration_deepener_edge_cases(tmp_path: Path) -> None:
    ...
    for key, phrases, include_msg in [
        (
            "hydration_meta_required_phrases",
            vm.HYDRATION_META_REQUIRED_PHRASES,
