...
def test_live_v5_validators() -> None:
    assert vm.validate_bug_report_template(REPO_ROOT) == []
    assert vm.validate_feature_request_template(REPO_ROOT) == []
    assert vm.INVENTORY_VERSION == 42
    assert len(vm.VALIDATORS) == 139
...
def test_live_v8_validators() -> None:
    ...
    assert vm.INVENTORY_VERSION == 42
    assert len(vm.VALIDATORS) == 139
    assert vm.MIN_VALIDATOR_COUNT == 139
...
def test_live_v12_validators() -> None:
    ...
    assert inventory["min_validator_count"] == 139
...
def test_v38_contributing_deepener_edge_cases(tmp_path: Path) -> None:
    ...
    assert any(
        "missing Packaging inventory v42 honesty lock" in f.message for f in findings
    )
...
def test_live_v42_agent_prompts_leftover_validators() -> None:
    ...
    assert len(vm.VALIDATORS) == 139
    assert vm.MIN_VALIDATOR_COUNT == 139
    assert "prompt-expertise" in vm.VALIDATORS
    assert "prompt-principles" in vm.VALIDATORS
    assert "prompt-metrics" in vm.VALIDATORS
    inventory = json.loads(
        (REPO_ROOT / "schemas" / "packaging-inventory.json").read_text(encoding="utf-8")
    )
    assert inventory["version"] == 42
...
def test_v42_agent_prompts_leftover_edge_cases(tmp_path: Path) -> None:
    assert any(
        "missing" in f.message for f in vm.validate_prompt_expertise(tmp_path)
    )
    assert any(
        "missing" in f.message for f in vm.validate_prompt_principles(tmp_path)
    )
    assert any("missing" in f.message for f in vm.validate_prompt_metrics(tmp_path))
    ...
    findings = vm.validate_prompt_principles(tmp_path)
    assert any(
        "missing Decision Making Principles section" in f.message for f in findings
    )
    for phrase in vm.PROMPT_PRINCIPLES_REQUIRED_PHRASES:
        assert any(phrase in f.message for f in findings)

    findings = vm.validate_prompt_metrics(tmp_path)
    assert any("missing Success Metrics section" in f.message for f in findings)
    for phrase in vm.PROMPT_METRICS_REQUIRED_PHRASES:
        assert any(phrase in f.message for f in findings)
    ...
    empty = _inventory_payload()
    empty["prompt_expertise_required_phrases"] = []
    findings = vm._inventory_lock_consistency(
        empty, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "prompt_expertise_required_phrases must not be empty" in f.message
        for f in findings
    )

    dup = _inventory_payload()
    dup["prompt_expertise_required_phrases"] = [
        vm.PROMPT_EXPERTISE_REQUIRED_PHRASES[0],
        vm.PROMPT_EXPERTISE_REQUIRED_PHRASES[0],
        *vm.PROMPT_EXPERTISE_REQUIRED_PHRASES[1:],
    ]
    findings = vm._inventory_lock_consistency(
        dup, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "prompt_expertise_required_phrases must be unique" in f.message
        for f in findings
    )

    blank = _inventory_payload()
    blank["prompt_expertise_required_phrases"] = ["ok", "  "]
    findings = vm._inventory_lock_consistency(
        blank, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
    ...
    missing = _inventory_payload()
    missing["prompt_expertise_required_phrases"] = ["## Your Expertise"]
    findings = vm._inventory_lock_consistency(
        missing, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "prompt_expertise_required_phrases must include" in f.message for f in findings
    )

    empty_p = _inventory_payload()
    empty_p["prompt_principles_required_phrases"] = []
    findings = vm._inventory_lock_consistency(
        empty_p, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "prompt_principles_required_phrases must not be empty" in f.message
        for f in findings
    )

    dup_p = _inventory_payload()
    dup_p["prompt_principles_required_phrases"] = [
        vm.PROMPT_PRINCIPLES_REQUIRED_PHRASES[0],
        vm.PROMPT_PRINCIPLES_REQUIRED_PHRASES[0],
        *vm.PROMPT_PRINCIPLES_REQUIRED_PHRASES[1:],
    ]
    findings = vm._inventory_lock_consistency(
        dup_p, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "prompt_principles_required_phrases must be unique" in f.message
        for f in findings
    )

    blank_p = _inventory_payload()
    blank_p["prompt_principles_required_phrases"] = ["ok", "  "]
    findings = vm._inventory_lock_consistency(
        blank_p, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "prompt_principles_required_phrases entries must be non-empty strings"
        in f.message
        for f in findings
    )

    missing_p = _inventory_payload()
    missing_p["prompt_principles_required_phrases"] = [
        "## Decision Making Principles"
    ]
    findings = vm._inventory_lock_consistency(
        missing_p, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "prompt_principles_required_phrases must include" in f.message for f in findings
    )

    empty_m = _inventory_payload()
    empty_m["prompt_metrics_required_phrases"] = []
    findings = vm._inventory_lock_consistency(
        empty_m, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "prompt_metrics_required_phrases must not be empty" in f.message
        for f in findings
    )

    dup_m = _inventory_payload()
    dup_m["prompt_metrics_required_phrases"] = [
        vm.PROMPT_METRICS_REQUIRED_PHRASES[0],
        vm.PROMPT_METRICS_REQUIRED_PHRASES[0],
        *vm.PROMPT_METRICS_REQUIRED_PHRASES[1:],
    ]
    findings = vm._inventory_lock_consistency(
        dup_m, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "prompt_metrics_required_phrases must be unique" in f.message for f in findings
    )

    blank_m = _inventory_payload()
    blank_m["prompt_metrics_required_phrases"] = ["ok", "  "]
    findings = vm._inventory_lock_consistency(
        blank_m, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "prompt_metrics_required_phrases entries must be non-empty strings"
        in f.message
        for f in findings
    )

    missing_m = _inventory_payload()
    missing_m["prompt_metrics_required_phrases"] = ["## Success Metrics"]
    findings = vm._inventory_lock_consistency(
        missing_m, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "prompt_metrics_required_phrases must include" in f.message for f in findings
    )
