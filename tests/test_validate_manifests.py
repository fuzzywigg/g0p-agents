...
def _inventory_payload(**overrides: object) -> dict:
    payload = {
        ...
        "dependabot_ecosystems_required_phrases": list(
            vm.DEPENDABOT_ECOSYSTEMS_REQUIRED_PHRASES
        ),
        "dependabot_groups_required_phrases": list(
            vm.DEPENDABOT_GROUPS_REQUIRED_PHRASES
        ),
        "dependabot_schedule_required_phrases": list(
            vm.DEPENDABOT_SCHEDULE_REQUIRED_PHRASES
        ),
        ...
        ("markdownlint_default", False),
            ...
            (
                ...
                (
                    ...
                    list(vm.DEPENDABOT_ECOSYSTEMS_REQUIRED_PHRASES)[:-1] + ["invented"],
                ),
                (
                    "dependabot_groups_required_phrases",
                    list(vm.DEPENDABOT_GROUPS_REQUIRED_PHRASES)[:-1] + ["invented"],
                ),
                (
                    "dependabot_schedule_required_phrases",
                    list(vm.DEPENDABOT_SCHEDULE_REQUIRED_PHRASES)[:-1] + ["invented"],
                ),
                (
...
def test_live_v8_validators() -> None:
    ...
    assert vm.INVENTORY_VERSION == 34
    assert len(vm.VALIDATORS) == 115
    assert vm.MIN_VALIDATOR_COUNT == 115
...
def test_live_v12_validators() -> None:
    ...
    assert inventory["min_validator_count"] == 115
...
def test_live_v34_dependabot_validators() -> None:
    assert vm.validate_dependabot(REPO_ROOT) == []
    assert vm.validate_dependabot_ecosystems(REPO_ROOT) == []
    assert vm.validate_dependabot_groups(REPO_ROOT) == []
    assert vm.validate_dependabot_schedule(REPO_ROOT) == []
    assert vm.INVENTORY_VERSION == 34
    assert len(vm.VALIDATORS) == 115
    assert vm.MIN_VALIDATOR_COUNT == 115
    assert "dependabot-ecosystems" in vm.VALIDATORS
    assert "dependabot-groups" in vm.VALIDATORS
    assert "dependabot-schedule" in vm.VALIDATORS
    inventory = json.loads(
        (REPO_ROOT / "schemas" / "packaging-inventory.json").read_text(encoding="utf-8")
    )
    assert inventory["version"] == 34
    assert inventory["dependabot_ecosystems_required_phrases"] == list(
        vm.DEPENDABOT_ECOSYSTEMS_REQUIRED_PHRASES
    )
    assert inventory["dependabot_groups_required_phrases"] == list(
        vm.DEPENDABOT_GROUPS_REQUIRED_PHRASES
    )
    assert inventory["dependabot_schedule_required_phrases"] == list(
        vm.DEPENDABOT_SCHEDULE_REQUIRED_PHRASES
    )
    assert inventory["min_validator_count"] == 115
    assert sorted(vm.VALIDATORS) == inventory["validator_names"]
    assert vm.VALIDATORS["dependabot"](REPO_ROOT) == []
    assert vm.validate_goose_recipe_headers(REPO_ROOT) == []
    assert set(vm.DEPENDABOT_GROUP_PATTERNS) == {"*"}
    assert tuple(vm.DEPENDABOT_ECOSYSTEMS_REQUIRED_PHRASES) == (
        "version: 2",
        'package-ecosystem: "github-actions"',
        'package-ecosystem: "pip"',
        'directory: "/"',
...
def test_v34_dependabot_edge_cases(tmp_path: Path) -> None:
    assert any(
        "missing" in f.message for f in vm.validate_dependabot_ecosystems(tmp_path)
    )
    assert any("missing" in f.message for f in vm.validate_dependabot_groups(tmp_path))
    assert any(
        "missing" in f.message for f in vm.validate_dependabot_schedule(tmp_path)
    )

    dep = tmp_path / ".github" / "dependabot.yml"
    dep.parent.mkdir(parents=True, exist_ok=True)
    _write(dep, "version: 1\nupdates: []\n")
    findings = vm.validate_dependabot_ecosystems(tmp_path)
    assert any("missing Dependabot version: 2" in f.message for f in findings)
    for phrase in vm.DEPENDABOT_ECOSYSTEMS_REQUIRED_PHRASES:
        if phrase != "version: 2":
            assert any(phrase in f.message for f in findings)

    findings = vm.validate_dependabot_groups(tmp_path)
    assert any("missing github_actions group" in f.message for f in findings)
    for phrase in vm.DEPENDABOT_GROUPS_REQUIRED_PHRASES:
        assert any(phrase in f.message for f in findings)

    findings = vm.validate_dependabot_schedule(tmp_path)
    assert any("missing weekly schedule interval" in f.message for f in findings)
    for phrase in vm.DEPENDABOT_SCHEDULE_REQUIRED_PHRASES:
        assert any(phrase in f.message for f in findings)
    ...
    assert vm.validate_dependabot_ecosystems(tmp_path) == []
    assert vm.validate_dependabot_groups(tmp_path) == []
    assert vm.validate_dependabot_schedule(tmp_path) == []

    bad = tmp_path / "badbot"
    bad.mkdir()
    (bad / ".github").mkdir()
    _copy_schemas(bad)
    live = (REPO_ROOT / ".github" / "dependabot.yml").read_text(encoding="utf-8")
    broken = live.replace('- "*"', '- "npm"')
    _write(bad / ".github" / "dependabot.yml", broken)
    findings = vm.validate_dependabot(bad)
    assert any("patterns must equal" in f.message for f in findings)

    empty = _inventory_payload()
    empty["dependabot_ecosystems_required_phrases"] = []
    findings = vm._inventory_lock_consistency(
        empty, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "dependabot_ecosystems_required_phrases must not be empty" in f.message
        for f in findings
    )

    dup = _inventory_payload()
    dup["dependabot_ecosystems_required_phrases"] = [
        vm.DEPENDABOT_ECOSYSTEMS_REQUIRED_PHRASES[0],
        vm.DEPENDABOT_ECOSYSTEMS_REQUIRED_PHRASES[0],
        *vm.DEPENDABOT_ECOSYSTEMS_REQUIRED_PHRASES[1:],
    ]
    findings = vm._inventory_lock_consistency(
        dup, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "dependabot_ecosystems_required_phrases must be unique" in f.message
        for f in findings
    )

    blank = _inventory_payload()
    blank["dependabot_ecosystems_required_phrases"] = ["ok", "  "]
    findings = vm._inventory_lock_consistency(
        blank, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "dependabot_ecosystems_required_phrases entries must be non-empty strings"
        in f.message
        for f in findings
    )

    missing = _inventory_payload()
    missing["dependabot_ecosystems_required_phrases"] = ["version: 2"]
    findings = vm._inventory_lock_consistency(
        missing, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "dependabot_ecosystems_required_phrases must include version 2" in f.message
        for f in findings
    )

    empty_g = _inventory_payload()
    empty_g["dependabot_groups_required_phrases"] = []
    findings = vm._inventory_lock_consistency(
        empty_g, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "dependabot_groups_required_phrases must not be empty" in f.message
        for f in findings
    )

    dup_g = _inventory_payload()
    dup_g["dependabot_groups_required_phrases"] = [
        vm.DEPENDABOT_GROUPS_REQUIRED_PHRASES[0],
        vm.DEPENDABOT_GROUPS_REQUIRED_PHRASES[0],
        *vm.DEPENDABOT_GROUPS_REQUIRED_PHRASES[1:],
    ]
    findings = vm._inventory_lock_consistency(
        dup_g, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "dependabot_groups_required_phrases must be unique" in f.message
        for f in findings
    )

    blank_g = _inventory_payload()
    blank_g["dependabot_groups_required_phrases"] = ["ok", "  "]
    findings = vm._inventory_lock_consistency(
        blank_g, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "dependabot_groups_required_phrases entries must be non-empty strings"
        in f.message
        for f in findings
    )

    missing_g = _inventory_payload()
    missing_g["dependabot_groups_required_phrases"] = ["github_actions:"]
    findings = vm._inventory_lock_consistency(
        missing_g, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "dependabot_groups_required_phrases must include github_actions" in f.message
        for f in findings
    )

    empty_s = _inventory_payload()
    empty_s["dependabot_schedule_required_phrases"] = []
    findings = vm._inventory_lock_consistency(
        empty_s, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "dependabot_schedule_required_phrases must not be empty" in f.message
        for f in findings
    )

    dup_s = _inventory_payload()
    dup_s["dependabot_schedule_required_phrases"] = [
        vm.DEPENDABOT_SCHEDULE_REQUIRED_PHRASES[0],
        vm.DEPENDABOT_SCHEDULE_REQUIRED_PHRASES[0],
        *vm.DEPENDABOT_SCHEDULE_REQUIRED_PHRASES[1:],
    ]
    findings = vm._inventory_lock_consistency(
        dup_s, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "dependabot_schedule_required_phrases must be unique" in f.message
        for f in findings
    )

    blank_s = _inventory_payload()
    blank_s["dependabot_schedule_required_phrases"] = ["ok", "  "]
    findings = vm._inventory_lock_consistency(
        blank_s, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "dependabot_schedule_required_phrases entries must be non-empty strings"
        in f.message
        for f in findings
    )

    missing_s = _inventory_payload()
    missing_s["dependabot_schedule_required_phrases"] = ["schedule:"]
    findings = vm._inventory_lock_consistency(
        missing_s, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "dependabot_schedule_required_phrases must include schedule" in f.message
        for f in findings
    )
    ...
    findings = vm.validate_packaging_inventory(tmp_path)
    assert any(
        "dependabot_ecosystems_required_phrases" in f.message for f in findings
    )
    assert any("dependabot_groups_required_phrases" in f.message for f in findings)
    assert any("dependabot_schedule_required_phrases" in f.message for f in findings)
