...
def _inventory_payload(**overrides: object) -> dict:
    payload = {
        ...
        "implementation_common_issues_required_phrases": list(
            vm.IMPLEMENTATION_COMMON_ISSUES_REQUIRED_PHRASES
        ),
...
def test_live_v41_implementation_guide_leftover_validators() -> None:
    assert vm.validate_implementation_common_issues(REPO_ROOT) == []
    assert vm.validate_implementation_faq(REPO_ROOT) == []
    assert vm.validate_implementation_support(REPO_ROOT) == []
    assert vm.INVENTORY_VERSION == 41
    assert len(vm.VALIDATORS) == 136
    assert vm.MIN_VALIDATOR_COUNT == 136
    assert "implementation-common-issues" in vm.VALIDATORS
    ...
    assert inventory["version"] == 41
    assert inventory["implementation_common_issues_required_phrases"] == list(
        vm.IMPLEMENTATION_COMMON_ISSUES_REQUIRED_PHRASES
    )
...
def test_v41_implementation_guide_leftover_edge_cases(tmp_path: Path) -> None:
    assert any(
        "missing" in f.message
        for f in vm.validate_implementation_common_issues(tmp_path)
    )
    assert any("missing" in f.message for f in vm.validate_implementation_faq(tmp_path))
    assert any(
        "missing" in f.message for f in vm.validate_implementation_support(tmp_path)
    )

    _write(tmp_path / "IMPLEMENTATION-GUIDE.md", "# Guide\n\nNo locked content.\n")
    findings = vm.validate_implementation_common_issues(tmp_path)
    assert any(
        "missing Common Issues & Solutions section" in f.message for f in findings
    )
    for phrase in vm.IMPLEMENTATION_COMMON_ISSUES_REQUIRED_PHRASES:
        assert any(phrase in f.message for f in findings)
    ...
    assert vm.validate_implementation_common_issues(tmp_path) == []
    assert vm.validate_implementation_faq(tmp_path) == []
    assert vm.validate_implementation_support(tmp_path) == []

    empty = _inventory_payload()
    empty["implementation_common_issues_required_phrases"] = []
    findings = vm._inventory_lock_consistency(
        empty, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "implementation_common_issues_required_phrases must not be empty" in f.message
        for f in findings
    )

    dup = _inventory_payload()
    dup["implementation_common_issues_required_phrases"] = [
        vm.IMPLEMENTATION_COMMON_ISSUES_REQUIRED_PHRASES[0],
        vm.IMPLEMENTATION_COMMON_ISSUES_REQUIRED_PHRASES[0],
        *vm.IMPLEMENTATION_COMMON_ISSUES_REQUIRED_PHRASES[1:],
    ]
    findings = vm._inventory_lock_consistency(
        dup, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "implementation_common_issues_required_phrases must be unique" in f.message
        for f in findings
    )

    blank = _inventory_payload()
    blank["implementation_common_issues_required_phrases"] = ["ok", "  "]
    findings = vm._inventory_lock_consistency(
        blank, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "implementation_common_issues_required_phrases entries must be "
        "non-empty strings"
        in f.message
        for f in findings
    )
    ...
    missing = _inventory_payload()
    missing["implementation_common_issues_required_phrases"] = [
    ...
    assert any(
        "implementation_common_issues_required_phrases must include" in f.message
        for f in findings
    )
    ...
    missing_s = _inventory_payload()
    missing_s["implementation_support_required_phrases"] = ["## Support & Resources"]
    findings = vm._inventory_lock_consistency(
        missing_s, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
    ...
    findings = vm.validate_packaging_inventory(tmp_path)
    assert any(
        "implementation_common_issues_required_phrases" in f.message for f in findings
    )
    assert any("implementation_faq_required_phrases" in f.message for f in findings)
    assert any(
