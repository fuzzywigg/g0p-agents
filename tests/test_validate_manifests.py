...
def _inventory_payload(**overrides: object) -> dict:
    payload = {
        ...
        "prompt_decision_required_phrases": list(
            vm.PROMPT_DECISION_REQUIRED_PHRASES
        ),
...
def test_live_v8_validators() -> None:
    ...
    assert vm.INVENTORY_VERSION == 43
    assert len(vm.VALIDATORS) == 147
    assert vm.MIN_VALIDATOR_COUNT == 147
...
def test_live_v12_validators() -> None:
    ...
    assert inventory["min_validator_count"] == 147
...
def test_live_v43_changelog_prompts_leftover_validators() -> None:
    ...
    assert vm.validate_prompt_decision(REPO_ROOT) == []
    ...
    assert inventory["prompt_decision_required_phrases"] == list(
        vm.PROMPT_DECISION_REQUIRED_PHRASES
    )
...
def test_v43_changelog_prompts_leftover_edge_cases(tmp_path: Path) -> None:
    assert any("missing" in f.message for f in vm.validate_changelog_preamble(tmp_path))
    assert any("missing" in f.message for f in vm.validate_changelog_changed(tmp_path))
    assert any("missing" in f.message for f in vm.validate_changelog_initial(tmp_path))
    assert any("missing" in f.message for f in vm.validate_prompt_expertise(tmp_path))
    assert any("missing" in f.message for f in vm.validate_prompt_decision(tmp_path))
    assert any(
        "missing" in f.message for f in vm.validate_prompt_escalation_identity(tmp_path)
    )

    _write(tmp_path / "CHANGELOG.md", "# Changelog\n\nNo locked content.\n")
    findings = vm.validate_changelog_preamble(tmp_path)
    assert any("missing Keep a Changelog preamble" in f.message for f in findings)
    for phrase in vm.CHANGELOG_PREAMBLE_REQUIRED_PHRASES:
        assert any(phrase in f.message for f in findings)

    findings = vm.validate_changelog_changed(tmp_path)
    assert any("missing Changed section" in f.message for f in findings)
    for phrase in vm.CHANGELOG_CHANGED_REQUIRED_PHRASES:
        assert any(phrase in f.message for f in findings)

    findings = vm.validate_changelog_initial(tmp_path)
    assert any("missing 0.1.0 initial release section" in f.message for f in findings)
    for phrase in vm.CHANGELOG_INITIAL_REQUIRED_PHRASES:
        assert any(phrase in f.message for f in findings)

    _write(tmp_path / "AGENT-PROMPTS.md", "# Prompts\n\nNo locked content.\n")
    findings = vm.validate_prompt_expertise(tmp_path)
    assert any("missing Your Expertise section" in f.message for f in findings)
    for phrase in vm.PROMPT_EXPERTISE_REQUIRED_PHRASES:
        assert any(phrase in f.message for f in findings)

    findings = vm.validate_prompt_decision(tmp_path)
    assert any(
        "missing Decision Making Principles section" in f.message for f in findings
    )
    for phrase in vm.PROMPT_DECISION_REQUIRED_PHRASES:
        assert any(phrase in f.message for f in findings)

    findings = vm.validate_prompt_escalation_identity(tmp_path)
    assert any(
        "missing ESCALATION REQUIRED identity banner" in f.message for f in findings
    )
    for phrase in vm.PROMPT_ESCALATION_IDENTITY_REQUIRED_PHRASES:
        assert any(phrase in f.message for f in findings)
    ...
    assert vm.validate_prompt_decision(tmp_path) == []
    assert vm.validate_prompt_escalation_identity(tmp_path) == []

    def _empty(key: str, msg: str) -> None:
        payload = _inventory_payload()
        payload[key] = []
        findings = vm._inventory_lock_consistency(
            payload, schema_path="schemas/packaging-inventory.json"
    ...
    def _dup(key: str, phrases: tuple[str, ...], msg: str) -> None:
        payload = _inventory_payload()
        payload[key] = [phrases[0], phrases[0], *phrases[1:]]
        findings = vm._inventory_lock_consistency(
            payload, schema_path="schemas/packaging-inventory.json"
    ...
    def _blank(key: str, msg: str) -> None:
        payload = _inventory_payload()
        payload[key] = ["ok", "  "]
        findings = vm._inventory_lock_consistency(
            payload, schema_path="schemas/packaging-inventory.json"
    ...
    def _missing(key: str, only: list[str], msg: str) -> None:
        payload = _inventory_payload()
        payload[key] = only
        findings = vm._inventory_lock_consistency(
            payload, schema_path="schemas/packaging-inventory.json"
    ...
    _empty(
        ...
        "changelog_preamble_required_phrases must not be empty",
    )
    _dup(
        "changelog_preamble_required_phrases",
        vm.CHANGELOG_PREAMBLE_REQUIRED_PHRASES,
        "changelog_preamble_required_phrases must be unique",
    )
    _blank(
        "changelog_preamble_required_phrases",
        "changelog_preamble_required_phrases entries must be non-empty strings",
    )
    _missing(
        "changelog_preamble_required_phrases",
        ["---"],
        "changelog_preamble_required_phrases must include",
    )

    _empty(
        "changelog_changed_required_phrases",
        "changelog_changed_required_phrases must not be empty",
    )
    _dup(
        "changelog_changed_required_phrases",
        vm.CHANGELOG_CHANGED_REQUIRED_PHRASES,
        "changelog_changed_required_phrases must be unique",
    )
    _blank(
        "changelog_changed_required_phrases",
        "changelog_changed_required_phrases entries must be non-empty strings",
    )
    _missing(
        "changelog_changed_required_phrases",
        ["Inventory schema minimum version is **7**"],
        "changelog_changed_required_phrases must include",
    )

    _empty(
        "changelog_initial_required_phrases",
        "changelog_initial_required_phrases must not be empty",
    )
    _dup(
        "changelog_initial_required_phrases",
        vm.CHANGELOG_INITIAL_REQUIRED_PHRASES,
        "changelog_initial_required_phrases must be unique",
    )
    _blank(
        "changelog_initial_required_phrases",
        "changelog_initial_required_phrases entries must be non-empty strings",
    )
    _missing(
        "changelog_initial_required_phrases",
    ...
    _dup(
        "prompt_expertise_required_phrases",
        vm.PROMPT_EXPERTISE_REQUIRED_PHRASES,
        "prompt_expertise_required_phrases must be unique",
    )
    _blank(
        "prompt_expertise_required_phrases",
        "prompt_expertise_required_phrases entries must be non-empty strings",
    )
    _missing(
        "prompt_expertise_required_phrases",
        ["## Your Expertise"],
    ...
    _empty(
        ...
        "prompt_decision_required_phrases must not be empty",
    )
    _dup(
        "prompt_decision_required_phrases",
        vm.PROMPT_DECISION_REQUIRED_PHRASES,
        "prompt_decision_required_phrases must be unique",
    )
    _blank(
        "prompt_decision_required_phrases",
        "prompt_decision_required_phrases entries must be non-empty strings",
    )
    _missing(
        "prompt_decision_required_phrases",
    ...
    _blank(
        "prompt_escalation_identity_required_phrases",
        "prompt_escalation_identity_required_phrases entries must be non-empty strings",
    )
    _missing(
        "prompt_escalation_identity_required_phrases",
        ["🚨 ESCALATION REQUIRED"],
    ...
    findings = vm.validate_packaging_inventory(tmp_path)
    assert any("changelog_preamble_required_phrases" in f.message for f in findings)
    assert any("changelog_changed_required_phrases" in f.message for f in findings)
    assert any("changelog_initial_required_phrases" in f.message for f in findings)
    assert any("prompt_expertise_required_phrases" in f.message for f in findings)
    assert any("prompt_decision_required_phrases" in f.message for f in findings)
    assert any(
