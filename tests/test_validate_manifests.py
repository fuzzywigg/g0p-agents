...
def _inventory_payload(**overrides: object) -> dict:
    payload = {
        ...
        "prompt_specialist_intros_required_phrases": list(
            vm.PROMPT_SPECIALIST_INTROS_REQUIRED_PHRASES
        ),
        "prompt_template_headers_required_phrases": list(
            vm.PROMPT_TEMPLATE_HEADERS_REQUIRED_PHRASES
        ),
        "prompt_constraints_residual_required_phrases": list(
            vm.PROMPT_CONSTRAINTS_RESIDUAL_REQUIRED_PHRASES
        ),
        "prompt_triggers_residual_required_phrases": list(
            vm.PROMPT_TRIGGERS_RESIDUAL_REQUIRED_PHRASES
        ),
        ...
        ("markdownlint_default", False),
            ...
            (
                ...
                (
                    ...
                    list(vm.PROMPT_SPECIALIST_INTROS_REQUIRED_PHRASES)[:-1]
                    + ["invented"],
                ),
                (
                    "prompt_template_headers_required_phrases",
                    list(vm.PROMPT_TEMPLATE_HEADERS_REQUIRED_PHRASES)[:-1]
                    + ["invented"],
                ),
                (
                    "prompt_constraints_residual_required_phrases",
                    list(vm.PROMPT_CONSTRAINTS_RESIDUAL_REQUIRED_PHRASES)[:-1]
                    + ["invented"],
                ),
                (
                    "prompt_triggers_residual_required_phrases",
                    list(vm.PROMPT_TRIGGERS_RESIDUAL_REQUIRED_PHRASES)[:-1]
                    + ["invented"],
                ),
                (
                    "prompt_related_docs_residual_required_phrases",
                    list(vm.PROMPT_RELATED_DOCS_RESIDUAL_REQUIRED_PHRASES)[:-1]
                    + ["invented"],
                ),
                (
                    "prompt_instantiation_required_phrases",
                    list(vm.PROMPT_INSTANTIATION_REQUIRED_PHRASES)[:-1]
                    + ["invented"],
                ),
                (
...
def test_live_v8_validators() -> None:
    ...
    assert vm.INVENTORY_VERSION == 51
    assert len(vm.VALIDATORS) == 178
    assert vm.MIN_VALIDATOR_COUNT == 178
...
def test_live_v12_validators() -> None:
    ...
    assert inventory["min_validator_count"] == 178
...
def test_live_v51_prompt_residual_validators() -> None:
    assert vm.validate_prompt_specialist_intros(REPO_ROOT) == []
    assert vm.validate_prompt_template_headers(REPO_ROOT) == []
    assert vm.validate_prompt_constraints_residual(REPO_ROOT) == []
    assert vm.validate_prompt_triggers_residual(REPO_ROOT) == []
    assert vm.validate_prompt_related_docs_residual(REPO_ROOT) == []
    ...
    assert inventory["version"] == 51
    assert inventory["min_validator_count"] == 178
    assert inventory["prompt_specialist_intros_required_phrases"] == list(
        vm.PROMPT_SPECIALIST_INTROS_REQUIRED_PHRASES
    )
    assert inventory["prompt_template_headers_required_phrases"] == list(
        vm.PROMPT_TEMPLATE_HEADERS_REQUIRED_PHRASES
    )
    assert inventory["prompt_constraints_residual_required_phrases"] == list(
        vm.PROMPT_CONSTRAINTS_RESIDUAL_REQUIRED_PHRASES
    )
    assert inventory["prompt_triggers_residual_required_phrases"] == list(
        vm.PROMPT_TRIGGERS_RESIDUAL_REQUIRED_PHRASES
    )
...
def test_v51_prompt_residual_edge_cases(tmp_path: Path) -> None:
    ...
    cases = [
        ...
        (
            vm.validate_prompt_template_headers,
            "missing QuantumArchitectAgent Prompt Template header",
            vm.PROMPT_TEMPLATE_HEADERS_REQUIRED_PHRASES,
        ),
        (
            vm.validate_prompt_constraints_residual,
            "missing Key Constraints (NEVER VIOLATE) section",
            vm.PROMPT_CONSTRAINTS_RESIDUAL_REQUIRED_PHRASES,
        ),
        (
            vm.validate_prompt_triggers_residual,
    ...
    assert vm.validate_prompt_specialist_intros(tmp_path) == []
    assert vm.validate_prompt_template_headers(tmp_path) == []
    assert vm.validate_prompt_constraints_residual(tmp_path) == []
    assert vm.validate_prompt_triggers_residual(tmp_path) == []
    ...
    (tmp_path / "schemas" / "packaging-inventory.json").write_text(
        json.dumps(
            _inventory_payload(
                ...
                + ["invented specialist intro"],
                prompt_template_headers_required_phrases=list(
                    vm.PROMPT_TEMPLATE_HEADERS_REQUIRED_PHRASES
                )[:-1]
                + ["invented template header"],
                prompt_constraints_residual_required_phrases=list(
                    vm.PROMPT_CONSTRAINTS_RESIDUAL_REQUIRED_PHRASES
                )[:-1]
                + ["invented constraints residual"],
                prompt_triggers_residual_required_phrases=list(
                    vm.PROMPT_TRIGGERS_RESIDUAL_REQUIRED_PHRASES
                )[:-1]
                + ["invented triggers residual"],
                prompt_related_docs_residual_required_phrases=list(
                    vm.PROMPT_RELATED_DOCS_RESIDUAL_REQUIRED_PHRASES
                )[:-1]
                + ["invented related docs residual"],
                prompt_instantiation_required_phrases=list(
                    vm.PROMPT_INSTANTIATION_REQUIRED_PHRASES
                )[:-1]
    ...
    findings = vm.validate_packaging_inventory(tmp_path)
    assert any(
        "prompt_specialist_intros_required_phrases" in f.message for f in findings
    )
    assert any(
        "prompt_template_headers_required_phrases" in f.message for f in findings
    )
    assert any(
        "prompt_constraints_residual_required_phrases" in f.message for f in findings
    )
    assert any(
        "prompt_triggers_residual_required_phrases" in f.message for f in findings
    )
    assert any(
        "prompt_related_docs_residual_required_phrases" in f.message for f in findings
    )
    assert any(
        "prompt_instantiation_required_phrases" in f.message for f in findings
    )


def test_v51_inventory_lock_mismatch_and_consistency_matrix(tmp_path: Path) -> None:
    """Large self-test: every v51 residual phrase key empty/dup/blank/seed + live registry."""
    assert vm.INVENTORY_VERSION == 51
    assert vm.MIN_VALIDATOR_COUNT == 178
    assert len(vm.VALIDATORS) == 178
    assert sorted(vm.VALIDATORS) == json.loads(
    ...
    keys = [
        ...
        (
            "prompt_triggers_residual_required_phrases",
            vm.PROMPT_TRIGGERS_RESIDUAL_REQUIRED_PHRASES,
            "Escalation Triggers/quantum advantage/multi-chain sync/battery drain",
        ),
        (
            "prompt_related_docs_residual_required_phrases",
            vm.PROMPT_RELATED_DOCS_RESIDUAL_REQUIRED_PHRASES,
    ...
    for fn, phrases in [
        ...
        (vm.validate_prompt_instantiation, vm.PROMPT_INSTANTIATION_REQUIRED_PHRASES),
    ]:
        stub = tmp_path / f"stub-{fn.__name__}"
        stub.mkdir(exist_ok=True)
        _write(stub / "AGENT-PROMPTS.md", phrases[0] + "\n")
        findings = fn(stub)
        assert findings
        assert any(phrases[1] in f.message for f in findings)


def test_v51_prompt_residual_per_phrase_missing_matrix(tmp_path: Path) -> None:
    """TOKENMAXX matrix: drop each locked residual phrase independently."""
    bundles = [
        (
            vm.validate_prompt_specialist_intros,
            vm.PROMPT_SPECIALIST_INTROS_REQUIRED_PHRASES,
    ...
    for fn, phrases, section in bundles:
...
def test_v51_prompt_residual_cross_validator_isolation(tmp_path: Path) -> None:
    """Each residual validator only cares about its own phrase set."""
    full = "\n".join(
        [
            *vm.PROMPT_SPECIALIST_INTROS_REQUIRED_PHRASES,
            *vm.PROMPT_TEMPLATE_HEADERS_REQUIRED_PHRASES,
    ...
    _write(tmp_path / "AGENT-PROMPTS.md", full)
    assert vm.validate_prompt_specialist_intros(tmp_path) == []
    assert vm.validate_prompt_template_headers(tmp_path) == []
    assert vm.validate_prompt_constraints_residual(tmp_path) == []
    assert vm.validate_prompt_triggers_residual(tmp_path) == []
    ...
    # strip only specialist intros
    stripped = full
    for phrase in vm.PROMPT_SPECIALIST_INTROS_REQUIRED_PHRASES:
        stripped = stripped.replace(phrase, "X")
    _write(tmp_path / "AGENT-PROMPTS.md", stripped)
    assert vm.validate_prompt_specialist_intros(tmp_path)
    assert vm.validate_prompt_template_headers(tmp_path) == []
    assert vm.validate_prompt_constraints_residual(tmp_path) == []
