...
def test_live_v5_validators() -> None:
    assert vm.validate_bug_report_template(REPO_ROOT) == []
    assert vm.validate_feature_request_template(REPO_ROOT) == []
    assert vm.INVENTORY_VERSION == 35
    assert len(vm.VALIDATORS) == 118
...
def test_live_v8_validators() -> None:
    ...
    assert vm.INVENTORY_VERSION == 35
    assert len(vm.VALIDATORS) == 118
    assert vm.MIN_VALIDATOR_COUNT == 118
...
def test_live_v12_validators() -> None:
    ...
    assert inventory["min_validator_count"] == 118
...
def test_live_v35_hydration_deepener_validators() -> None:
...
def test_v35_hydration_deepener_edge_cases(tmp_path: Path) -> None:
