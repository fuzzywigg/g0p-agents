...
def _inventory_payload(**overrides: object) -> dict:
    payload = {
        ...
        "prompt_metrics_quantum_required_phrases": list(
            vm.PROMPT_METRICS_QUANTUM_REQUIRED_PHRASES
        ),
        "prompt_metrics_chain_required_phrases": list(
            vm.PROMPT_METRICS_CHAIN_REQUIRED_PHRASES
        ),
        "prompt_metrics_edge_required_phrases": list(
            vm.PROMPT_METRICS_EDGE_REQUIRED_PHRASES
        ),
        "prompt_metrics_orch_required_phrases": list(
            vm.PROMPT_METRICS_ORCH_REQUIRED_PHRASES
        ),
        ...
        "prompt_context_placeholders_required_phrases": list(
            vm.PROMPT_CONTEXT_PLACEHOLDERS_REQUIRED_PHRASES
        ),
        "prompt_expertise_residual_required_phrases": list(
            vm.PROMPT_EXPERTISE_RESIDUAL_REQUIRED_PHRASES
        ),
        "prompt_escalate_specialists_required_phrases": list(
            vm.PROMPT_ESCALATE_SPECIALISTS_REQUIRED_PHRASES
        ),
        "prompt_escalate_orch_required_phrases": list(
            vm.PROMPT_ESCALATE_ORCH_REQUIRED_PHRASES
        ),
        "prompt_orchestration_ops_required_phrases": list(
            vm.PROMPT_ORCHESTRATION_OPS_REQUIRED_PHRASES
        ),
        "prompt_usage_agent_action_required_phrases": list(
            vm.PROMPT_USAGE_AGENT_ACTION_REQUIRED_PHRASES
        ),
...
def test_validators_registry_covers_all_checks() -> None:
    expected = {
        ...
        "prompt-metrics-quantum",
        "prompt-metrics-chain",
        "prompt-metrics-edge",
        "prompt-metrics-orch",
        "prompt-principles-detail",
        "prompt-communication-detail",
        "prompt-context-placeholders",
        "prompt-expertise-residual",
        "prompt-escalate-specialists",
        "prompt-escalate-orch",
        "prompt-orchestration-ops",
        "prompt-usage-agent-action",
        "hydration-phase2",
        "hydration-phase5",
        "license-mit",
        "link-check",
...
def test_live_v52_prompt_heavy_residual_validators() -> None:
    assert vm.validate_prompt_metrics_quantum(REPO_ROOT) == []
    assert vm.validate_prompt_metrics_chain(REPO_ROOT) == []
    assert vm.validate_prompt_metrics_edge(REPO_ROOT) == []
    assert vm.validate_prompt_metrics_orch(REPO_ROOT) == []
    assert vm.validate_prompt_principles_detail(REPO_ROOT) == []
    assert vm.validate_prompt_communication_detail(REPO_ROOT) == []
    assert vm.validate_prompt_context_placeholders(REPO_ROOT) == []
    assert vm.validate_prompt_expertise_residual(REPO_ROOT) == []
    assert vm.validate_prompt_escalate_specialists(REPO_ROOT) == []
    assert vm.validate_prompt_escalate_orch(REPO_ROOT) == []
    assert vm.validate_prompt_orchestration_ops(REPO_ROOT) == []
    assert vm.validate_prompt_usage_agent_action(REPO_ROOT) == []
    assert vm.INVENTORY_VERSION == 52
    assert len(vm.VALIDATORS) == 196
    assert vm.MIN_VALIDATOR_COUNT == 196
    for name in [
    ...
    assert inventory["version"] == 52
    assert inventory["min_validator_count"] == 196
    assert inventory["prompt_metrics_quantum_required_phrases"] == list(
        vm.PROMPT_METRICS_QUANTUM_REQUIRED_PHRASES
    )
    assert inventory["prompt_metrics_chain_required_phrases"] == list(
        vm.PROMPT_METRICS_CHAIN_REQUIRED_PHRASES
    )
    assert inventory["prompt_metrics_edge_required_phrases"] == list(
        vm.PROMPT_METRICS_EDGE_REQUIRED_PHRASES
    )
    assert inventory["prompt_metrics_orch_required_phrases"] == list(
        vm.PROMPT_METRICS_ORCH_REQUIRED_PHRASES
    )
    ...
    assert inventory["prompt_context_placeholders_required_phrases"] == list(
        vm.PROMPT_CONTEXT_PLACEHOLDERS_REQUIRED_PHRASES
    )
    assert inventory["prompt_expertise_residual_required_phrases"] == list(
        vm.PROMPT_EXPERTISE_RESIDUAL_REQUIRED_PHRASES
    )
    assert inventory["prompt_escalate_specialists_required_phrases"] == list(
        vm.PROMPT_ESCALATE_SPECIALISTS_REQUIRED_PHRASES
    )
    assert inventory["prompt_escalate_orch_required_phrases"] == list(
        vm.PROMPT_ESCALATE_ORCH_REQUIRED_PHRASES
    )
    assert inventory["prompt_orchestration_ops_required_phrases"] == list(
        vm.PROMPT_ORCHESTRATION_OPS_REQUIRED_PHRASES
    )
    assert inventory["prompt_usage_agent_action_required_phrases"] == list(
        vm.PROMPT_USAGE_AGENT_ACTION_REQUIRED_PHRASES
    )
    assert sorted(vm.VALIDATORS) == inventory["validator_names"]
    # prior prompt leftovers remain green
...
def test_v52_prompt_heavy_residual_edge_cases(tmp_path: Path) -> None:
    for fn in [
        vm.validate_prompt_metrics_quantum,
        vm.validate_prompt_metrics_chain,
        vm.validate_prompt_metrics_edge,
        vm.validate_prompt_metrics_orch,
        vm.validate_prompt_principles_detail,
        vm.validate_prompt_communication_detail,
        vm.validate_prompt_context_placeholders,
        vm.validate_prompt_expertise_residual,
        vm.validate_prompt_escalate_specialists,
        vm.validate_prompt_escalate_orch,
        vm.validate_prompt_orchestration_ops,
        vm.validate_prompt_usage_agent_action,
    ]:
        assert any("missing" in f.message for f in fn(tmp_path))
    ...
    cases = [
        (
            vm.validate_prompt_metrics_quantum,
            'missing metrics-quantum Gate count lock',
            vm.PROMPT_METRICS_QUANTUM_REQUIRED_PHRASES,
        ),
        (
            vm.validate_prompt_metrics_chain,
            'missing metrics-chain Multi-chain sync lock',
            vm.PROMPT_METRICS_CHAIN_REQUIRED_PHRASES,
        ),
        (
            vm.validate_prompt_metrics_edge,
            'missing metrics-edge Apple security review lock',
            vm.PROMPT_METRICS_EDGE_REQUIRED_PHRASES,
        ),
        (
            vm.validate_prompt_metrics_orch,
            'missing metrics-orch Team consensus lock',
            vm.PROMPT_METRICS_ORCH_REQUIRED_PHRASES,
        ),
        (
            vm.validate_prompt_principles_detail,
        ...
        (
            vm.validate_prompt_communication_detail,
            'missing communication-detail architecture diagrams lock',
            vm.PROMPT_COMMUNICATION_DETAIL_REQUIRED_PHRASES,
        ),
        (
            vm.validate_prompt_context_placeholders,
            'missing context-placeholders Current Project Vision lock',
            vm.PROMPT_CONTEXT_PLACEHOLDERS_REQUIRED_PHRASES,
        ),
        (
            vm.validate_prompt_expertise_residual,
        ...
        (
            vm.validate_prompt_escalate_specialists,
            'missing escalate-specialists Escalate when constraints lock',
            vm.PROMPT_ESCALATE_SPECIALISTS_REQUIRED_PHRASES,
        ),
        (
            vm.validate_prompt_escalate_orch,
            'missing escalate-orch Identify conflicts lock',
            vm.PROMPT_ESCALATE_ORCH_REQUIRED_PHRASES,
        ),
        (
            vm.validate_prompt_orchestration_ops,
            'missing orchestration-ops Update risk register lock',
            vm.PROMPT_ORCHESTRATION_OPS_REQUIRED_PHRASES,
        ),
        (
            vm.validate_prompt_usage_agent_action,
            'missing usage-agent-action Agent Action lock',
            vm.PROMPT_USAGE_AGENT_ACTION_REQUIRED_PHRASES,
        ),
    ]
    for fn, section_msg, phrases in cases:
    ...
    assert vm.validate_prompt_metrics_quantum(tmp_path) == []
    assert vm.validate_prompt_metrics_chain(tmp_path) == []
    assert vm.validate_prompt_metrics_edge(tmp_path) == []
    assert vm.validate_prompt_metrics_orch(tmp_path) == []
    assert vm.validate_prompt_principles_detail(tmp_path) == []
    assert vm.validate_prompt_communication_detail(tmp_path) == []
    assert vm.validate_prompt_context_placeholders(tmp_path) == []
    assert vm.validate_prompt_expertise_residual(tmp_path) == []
    assert vm.validate_prompt_escalate_specialists(tmp_path) == []
    assert vm.validate_prompt_escalate_orch(tmp_path) == []
    assert vm.validate_prompt_orchestration_ops(tmp_path) == []
    assert vm.validate_prompt_usage_agent_action(tmp_path) == []
    ...
    (tmp_path / "schemas" / "packaging-inventory.json").write_text(
        json.dumps(
            _inventory_payload(
                prompt_metrics_quantum_required_phrases=list(
                    vm.PROMPT_METRICS_QUANTUM_REQUIRED_PHRASES
                )[:-1] + ["invented prompt-metrics-quantum"],
                prompt_metrics_chain_required_phrases=list(
                    vm.PROMPT_METRICS_CHAIN_REQUIRED_PHRASES
                )[:-1] + ["invented prompt-metrics-chain"],
                prompt_metrics_edge_required_phrases=list(
                    vm.PROMPT_METRICS_EDGE_REQUIRED_PHRASES
                )[:-1] + ["invented prompt-metrics-edge"],
                prompt_metrics_orch_required_phrases=list(
                    vm.PROMPT_METRICS_ORCH_REQUIRED_PHRASES
                )[:-1] + ["invented prompt-metrics-orch"],
                prompt_principles_detail_required_phrases=list(
                    vm.PROMPT_PRINCIPLES_DETAIL_REQUIRED_PHRASES
                )[:-1] + ["invented prompt-principles-detail"],
                prompt_communication_detail_required_phrases=list(
                    vm.PROMPT_COMMUNICATION_DETAIL_REQUIRED_PHRASES
                )[:-1] + ["invented prompt-communication-detail"],
                prompt_context_placeholders_required_phrases=list(
                    vm.PROMPT_CONTEXT_PLACEHOLDERS_REQUIRED_PHRASES
                )[:-1] + ["invented prompt-context-placeholders"],
                prompt_expertise_residual_required_phrases=list(
                    vm.PROMPT_EXPERTISE_RESIDUAL_REQUIRED_PHRASES
                )[:-1] + ["invented prompt-expertise-residual"],
                prompt_escalate_specialists_required_phrases=list(
                    vm.PROMPT_ESCALATE_SPECIALISTS_REQUIRED_PHRASES
                )[:-1] + ["invented prompt-escalate-specialists"],
                prompt_escalate_orch_required_phrases=list(
                    vm.PROMPT_ESCALATE_ORCH_REQUIRED_PHRASES
                )[:-1] + ["invented prompt-escalate-orch"],
                prompt_orchestration_ops_required_phrases=list(
                    vm.PROMPT_ORCHESTRATION_OPS_REQUIRED_PHRASES
                )[:-1] + ["invented prompt-orchestration-ops"],
                prompt_usage_agent_action_required_phrases=list(
                    vm.PROMPT_USAGE_AGENT_ACTION_REQUIRED_PHRASES
                )[:-1] + ["invented prompt-usage-agent-action"],
            )
        ),
        encoding="utf-8",
    )
    findings = vm.validate_packaging_inventory(tmp_path)
    assert any("prompt_metrics_quantum_required_phrases" in f.message for f in findings)
    assert any("prompt_metrics_chain_required_phrases" in f.message for f in findings)
    assert any("prompt_metrics_edge_required_phrases" in f.message for f in findings)
    assert any("prompt_metrics_orch_required_phrases" in f.message for f in findings)
    assert any("prompt_principles_detail_required_phrases" in f.message for f in findings)
    assert any("prompt_communication_detail_required_phrases" in f.message for f in findings)
    assert any("prompt_context_placeholders_required_phrases" in f.message for f in findings)
    assert any("prompt_expertise_residual_required_phrases" in f.message for f in findings)
    assert any("prompt_escalate_specialists_required_phrases" in f.message for f in findings)
    assert any("prompt_escalate_orch_required_phrases" in f.message for f in findings)
    assert any("prompt_orchestration_ops_required_phrases" in f.message for f in findings)
    assert any("prompt_usage_agent_action_required_phrases" in f.message for f in findings)
