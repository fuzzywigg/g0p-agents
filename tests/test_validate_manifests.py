...
def _inventory_payload(**overrides: object) -> dict:
    payload = {
        ...
        "constitution_orchestration_required_phrases": list(
            vm.CONSTITUTION_ORCHESTRATION_REQUIRED_PHRASES
        ),
        "constitution_scratchpad_required_phrases": list(
            vm.CONSTITUTION_SCRATCHPAD_REQUIRED_PHRASES
        ),
        "constitution_conflicts_required_phrases": list(
            vm.CONSTITUTION_CONFLICTS_REQUIRED_PHRASES
        ),
...
def test_validators_registry_covers_all_checks() -> None:
    expected = {
        ...
        "constitution-orchestration",
        "constitution-scratchpad",
    ...
    cases = [
        ...
        ("markdownlint_default", False),
            ...
            (
                ...
                (
                    "constitution_orchestration_required_phrases",
                    list(vm.CONSTITUTION_ORCHESTRATION_REQUIRED_PHRASES)[:-1]
                    + ["invented"],
                ),
                (
                    "constitution_scratchpad_required_phrases",
                    list(vm.CONSTITUTION_SCRATCHPAD_REQUIRED_PHRASES)[:-1]
                    + ["invented"],
                ),
                (
                    "constitution_conflicts_required_phrases",
                    list(vm.CONSTITUTION_CONFLICTS_REQUIRED_PHRASES)[:-1]
                    + ["invented"],
                ),
                (
...
def test_live_v5_validators() -> None:
    assert vm.validate_bug_report_template(REPO_ROOT) == []
    assert vm.validate_feature_request_template(REPO_ROOT) == []
    assert vm.INVENTORY_VERSION == 39
    assert len(vm.VALIDATORS) == 130
...
def test_live_v8_validators() -> None:
    ...
    assert vm.INVENTORY_VERSION == 39
    assert len(vm.VALIDATORS) == 130
    assert vm.MIN_VALIDATOR_COUNT == 130
...
def test_live_v12_validators() -> None:
    ...
    assert inventory["min_validator_count"] == 130
...
def test_v38_contributing_deepener_edge_cases(tmp_path: Path) -> None:
    ...
    assert any(
        "missing Packaging inventory v38 honesty lock" in f.message for f in findings
    )
...
def test_live_v39_constitution_leftover_validators() -> None:
    assert vm.validate_constitution_orchestration(REPO_ROOT) == []
    assert vm.validate_constitution_scratchpad(REPO_ROOT) == []
    assert vm.validate_constitution_conflicts(REPO_ROOT) == []
    assert vm.INVENTORY_VERSION == 39
    assert len(vm.VALIDATORS) == 130
    assert vm.MIN_VALIDATOR_COUNT == 130
    assert "constitution-orchestration" in vm.VALIDATORS
    assert "constitution-scratchpad" in vm.VALIDATORS
    assert "constitution-conflicts" in vm.VALIDATORS
    inventory = json.loads(
        (REPO_ROOT / "schemas" / "packaging-inventory.json").read_text(encoding="utf-8")
    )
    assert inventory["version"] == 39
    assert inventory["constitution_orchestration_required_phrases"] == list(
        vm.CONSTITUTION_ORCHESTRATION_REQUIRED_PHRASES
    )
    assert inventory["constitution_scratchpad_required_phrases"] == list(
        vm.CONSTITUTION_SCRATCHPAD_REQUIRED_PHRASES
    )
    assert inventory["constitution_conflicts_required_phrases"] == list(
        vm.CONSTITUTION_CONFLICTS_REQUIRED_PHRASES
    )
    assert inventory["min_validator_count"] == 130
    assert sorted(vm.VALIDATORS) == inventory["validator_names"]
...
def test_v39_constitution_leftover_edge_cases(tmp_path: Path) -> None:
    assert any(
        "missing" in f.message
        for f in vm.validate_constitution_orchestration(tmp_path)
    )
    assert any(
        "missing" in f.message for f in vm.validate_constitution_scratchpad(tmp_path)
    )
    assert any(
        "missing" in f.message for f in vm.validate_constitution_conflicts(tmp_path)
    )

    _write(tmp_path / "AGENTS-v2.2.md", "# Constitution\n\nNo locked content.\n")
    findings = vm.validate_constitution_orchestration(tmp_path)
    assert any(
        "missing Recipe-Based Orchestration Structure section" in f.message
        for f in findings
    )
    for phrase in vm.CONSTITUTION_ORCHESTRATION_REQUIRED_PHRASES:
        assert any(phrase in f.message for f in findings)

    findings = vm.validate_constitution_scratchpad(tmp_path)
    assert any(
        "missing Scratchpad State Machine section" in f.message for f in findings
    )
    for phrase in vm.CONSTITUTION_SCRATCHPAD_REQUIRED_PHRASES:
        assert any(phrase in f.message for f in findings)

    findings = vm.validate_constitution_conflicts(tmp_path)
    assert any(
        "missing Conflict Resolution Matrix section" in f.message for f in findings
    )
    for phrase in vm.CONSTITUTION_CONFLICTS_REQUIRED_PHRASES:
        assert any(phrase in f.message for f in findings)
    ...
    assert vm.validate_constitution_orchestration(tmp_path) == []
    assert vm.validate_constitution_scratchpad(tmp_path) == []
    assert vm.validate_constitution_conflicts(tmp_path) == []

    empty = _inventory_payload()
    empty["constitution_orchestration_required_phrases"] = []
    findings = vm._inventory_lock_consistency(
        empty, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "constitution_orchestration_required_phrases must not be empty" in f.message
        for f in findings
    )

    dup = _inventory_payload()
    dup["constitution_orchestration_required_phrases"] = [
        vm.CONSTITUTION_ORCHESTRATION_REQUIRED_PHRASES[0],
        vm.CONSTITUTION_ORCHESTRATION_REQUIRED_PHRASES[0],
        *vm.CONSTITUTION_ORCHESTRATION_REQUIRED_PHRASES[1:],
    ]
    findings = vm._inventory_lock_consistency(
        dup, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "constitution_orchestration_required_phrases must be unique" in f.message
        for f in findings
    )

    blank = _inventory_payload()
    blank["constitution_orchestration_required_phrases"] = ["ok", "  "]
    findings = vm._inventory_lock_consistency(
        blank, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "constitution_orchestration_required_phrases entries must be non-empty strings"
        in f.message
        for f in findings
    )

    missing = _inventory_payload()
    missing["constitution_orchestration_required_phrases"] = [
    ...
    assert any(
        "constitution_orchestration_required_phrases must include" in f.message
        for f in findings
    )

    empty_s = _inventory_payload()
    empty_s["constitution_scratchpad_required_phrases"] = []
    findings = vm._inventory_lock_consistency(
        empty_s, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "constitution_scratchpad_required_phrases must not be empty" in f.message
        for f in findings
    )

    dup_s = _inventory_payload()
    dup_s["constitution_scratchpad_required_phrases"] = [
        vm.CONSTITUTION_SCRATCHPAD_REQUIRED_PHRASES[0],
        vm.CONSTITUTION_SCRATCHPAD_REQUIRED_PHRASES[0],
        *vm.CONSTITUTION_SCRATCHPAD_REQUIRED_PHRASES[1:],
    ]
    findings = vm._inventory_lock_consistency(
        dup_s, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "constitution_scratchpad_required_phrases must be unique" in f.message
        for f in findings
    )

    blank_s = _inventory_payload()
    blank_s["constitution_scratchpad_required_phrases"] = ["ok", "  "]
    findings = vm._inventory_lock_consistency(
        blank_s, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "constitution_scratchpad_required_phrases entries must be non-empty strings"
        in f.message
        for f in findings
    )

    missing_s = _inventory_payload()
    missing_s["constitution_scratchpad_required_phrases"] = [
    ...
    assert any(
        "constitution_scratchpad_required_phrases must include" in f.message
        for f in findings
    )

    empty_c = _inventory_payload()
    empty_c["constitution_conflicts_required_phrases"] = []
    findings = vm._inventory_lock_consistency(
        empty_c, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "constitution_conflicts_required_phrases must not be empty" in f.message
        for f in findings
    )

    dup_c = _inventory_payload()
    dup_c["constitution_conflicts_required_phrases"] = [
        vm.CONSTITUTION_CONFLICTS_REQUIRED_PHRASES[0],
        vm.CONSTITUTION_CONFLICTS_REQUIRED_PHRASES[0],
        *vm.CONSTITUTION_CONFLICTS_REQUIRED_PHRASES[1:],
    ]
    findings = vm._inventory_lock_consistency(
        dup_c, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "constitution_conflicts_required_phrases must be unique" in f.message
        for f in findings
    )

    blank_c = _inventory_payload()
    blank_c["constitution_conflicts_required_phrases"] = ["ok", "  "]
    findings = vm._inventory_lock_consistency(
        blank_c, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "constitution_conflicts_required_phrases entries must be non-empty strings"
        in f.message
        for f in findings
    )

    missing_c = _inventory_payload()
    missing_c["constitution_conflicts_required_phrases"] = [
    ...
    assert any(
        "constitution_conflicts_required_phrases must include" in f.message
        for f in findings
    )
    ...
    findings = vm.validate_packaging_inventory(tmp_path)
    assert any(
        "constitution_orchestration_required_phrases" in f.message for f in findings
    )
    assert any(
        "constitution_scratchpad_required_phrases" in f.message for f in findings
    )
    assert any(
        "constitution_conflicts_required_phrases" in f.message for f in findings
    )
