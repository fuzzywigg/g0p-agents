...
# ---------------------------------------------------------------------------
# Execution residual HEAVY edges after #147.
# EXISTING eight execution-* phrase-lock validators — no invented product /
# inventory bump. Distinct from goose-recipe #147, orch-timeout drafts
# (#145/#146), and prompts v51/v52 / prompt-validator #144.
# ---------------------------------------------------------------------------
...
def test_execution_residual_empty_maps_and_empty_doc(tmp_path: Path) -> None:
    ...
    for name, _fn, _phrases, key in modules:
        if key == "execution_summary_required_phrases":
            # Packaging-style key: schema rejects empty via validate_packaging_inventory.
            _copy_schemas(tmp_path)
            for rel in _inventory_payload()["required_paths"]:
                target = tmp_path / rel
                target.parent.mkdir(parents=True, exist_ok=True)
...
def test_execution_residual_invalid_keys(tmp_path: Path) -> None:
    ...
    # Live execution names remain selectable
    findings = vm.run_all_validations(
        REPO_ROOT,
...
def test_execution_residual_inventory_mismatch_matrix(tmp_path: Path) -> None:
    ...
    for name, _fn, phrases, key in modules:
        if key == "execution_summary_required_phrases":
            # Packaging-style: schema uniqueness + lock-mismatch (not consistency empties).
            _copy_schemas(tmp_path)
            for rel in _inventory_payload()["required_paths"]:
                target = tmp_path / rel
                target.parent.mkdir(parents=True, exist_ok=True)
...
def test_execution_residual_concurrent_validate_races(tmp_path: Path) -> None:
    ...
    def _empty_map_check(key: str) -> bool:
        if key == "execution_summary_required_phrases":
            ...
            # Lock-mismatch path is race-safe without writing schemas.
            findings = []
            if (
                tuple(payload.get(key, ()))
                != vm.EXECUTION_SUMMARY_REQUIRED_PHRASES
            ):
                findings.append(
                    vm._lock_mismatch(
            ...
            return any(
                "execution_summary_required_phrases" in f.message for f in findings
            )
        payload = _inventory_payload()
        payload[key] = []
        findings = vm._inventory_lock_consistency(
...
def test_execution_residual_historic_four_fixture_edges(tmp_path: Path) -> None:
    ...
    # Invented timeout-orchestration sibling agent is refused (historic four only).
    _write(
        tmp_path / "EXECUTION-SUMMARY.md",
    ...
    # Live root stays green alongside this residual fixture edge.
    assert vm.validate_execution_summary(REPO_ROOT) == []
    assert vm.validate_goose_orchestration(REPO_ROOT) == []
