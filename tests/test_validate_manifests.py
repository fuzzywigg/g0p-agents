...
def test_live_v49_prompt_responsibilities_context_decision_validators() -> None:
...
def test_v49_prompt_responsibilities_context_decision_edge_cases(
    tmp_path: Path,
) -> None:
    for fn in [
    ...
    # Current Project Vision section gate
    phrases_wo_vision = [
        p
        for p in vm.PROMPT_PROJECT_CONTEXT_REQUIRED_PHRASES
        if p != "## Current Project Vision"
    ]
    _write(
        tmp_path / "AGENT-PROMPTS.md",
        "\n".join(["## Current Project Context", *phrases_wo_vision]),
    )
    findings = vm.validate_prompt_project_context(tmp_path)
    assert any("missing Current Project Vision section" in f.message for f in findings)
...
def test_v49_inventory_lock_mismatch_and_consistency_matrix(tmp_path: Path) -> None:
    ...
    keys = [
        (
            "prompt_responsibilities_required_phrases",
            vm.PROMPT_RESPONSIBILITIES_REQUIRED_PHRASES,
            "Core Responsibilities/quantum/Android-iOS/collect outputs",
        ),
        (
            "prompt_project_context_required_phrases",
            vm.PROMPT_PROJECT_CONTEXT_REQUIRED_PHRASES,
            "Current Project Context/Ethereum/Vision/HIPAA constraint",
        ),
        (
            "prompt_decision_authority_required_phrases",
            vm.PROMPT_DECISION_AUTHORITY_REQUIRED_PHRASES,
            "Decision Authority/final calls/trade-offs/irresolvable conflicts",
        ),
    ]
    for key, phrases, include_token in keys:
        payload = _inventory_payload()
