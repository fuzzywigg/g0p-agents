...
def _inventory_payload(**overrides: object) -> dict:
    payload = {
        ...
        "prompt_matrix_rows_required_phrases": list(
            vm.PROMPT_MATRIX_ROWS_REQUIRED_PHRASES
        ),
        "prompt_monthly_checkboxes_required_phrases": list(
            vm.PROMPT_MONTHLY_CHECKBOXES_REQUIRED_PHRASES
        ),
        "prompt_usage_numbered_required_phrases": list(
            vm.PROMPT_USAGE_NUMBERED_REQUIRED_PHRASES
        ),
        "prompt_example_steps_required_phrases": list(
            vm.PROMPT_EXAMPLE_STEPS_REQUIRED_PHRASES
        ),
        "prompt_living_bullets_required_phrases": list(
            vm.PROMPT_LIVING_BULLETS_REQUIRED_PHRASES
        ),
        "prompt_quantum_resp_numbered_required_phrases": list(
            vm.PROMPT_QUANTUM_RESP_NUMBERED_REQUIRED_PHRASES
        ),
        "prompt_bc_resp_numbered_required_phrases": list(
            vm.PROMPT_BC_RESP_NUMBERED_REQUIRED_PHRASES
        ),
        "prompt_edge_resp_numbered_required_phrases": list(
            vm.PROMPT_EDGE_RESP_NUMBERED_REQUIRED_PHRASES
        ),
        "prompt_orch_resp_numbered_required_phrases": list(
            vm.PROMPT_ORCH_RESP_NUMBERED_REQUIRED_PHRASES
        ),
        "prompt_user_input_required_phrases": list(
            vm.PROMPT_USER_INPUT_REQUIRED_PHRASES
        ),
        "prompt_cannot_delegate_numbered_required_phrases": list(
            vm.PROMPT_CANNOT_DELEGATE_NUMBERED_REQUIRED_PHRASES
        ),
        "prompt_decision_bullets_required_phrases": list(
            vm.PROMPT_DECISION_BULLETS_REQUIRED_PHRASES
        ),
...
def test_live_v8_validators() -> None:
    ...
    assert vm.INVENTORY_VERSION == 53
    assert len(vm.VALIDATORS) == 208
    assert vm.MIN_VALIDATOR_COUNT == 208
...
def test_live_v12_validators() -> None:
    ...
    assert inventory["min_validator_count"] == 208
...
def test_v38_contributing_deepener_edge_cases(tmp_path: Path) -> None:
    ...
    assert any(
        "missing Packaging inventory v53 honesty lock" in f.message for f in findings
    )
...
def test_v47_inventory_lock_mismatch_and_consistency_matrix(tmp_path: Path) -> None:
    ...
    assert any(
        "Packaging inventory v53" in phrase
        for phrase in vm.CONTRIBUTING_CI_HONESTY_REQUIRED_PHRASES
    )
...
def test_live_v53_prompt_listform_residual_validators() -> None:
    assert vm.validate_prompt_matrix_rows(REPO_ROOT) == []
    assert vm.validate_prompt_monthly_checkboxes(REPO_ROOT) == []
    assert vm.validate_prompt_usage_numbered(REPO_ROOT) == []
    assert vm.validate_prompt_example_steps(REPO_ROOT) == []
    assert vm.validate_prompt_living_bullets(REPO_ROOT) == []
    assert vm.validate_prompt_quantum_resp_numbered(REPO_ROOT) == []
    assert vm.validate_prompt_bc_resp_numbered(REPO_ROOT) == []
    assert vm.validate_prompt_edge_resp_numbered(REPO_ROOT) == []
    assert vm.validate_prompt_orch_resp_numbered(REPO_ROOT) == []
    assert vm.validate_prompt_user_input(REPO_ROOT) == []
    assert vm.validate_prompt_cannot_delegate_numbered(REPO_ROOT) == []
    assert vm.validate_prompt_decision_bullets(REPO_ROOT) == []
    assert vm.INVENTORY_VERSION == 53
    assert len(vm.VALIDATORS) == 208
    assert vm.MIN_VALIDATOR_COUNT == 208
    inventory = json.loads(
    ...
    assert inventory["version"] == 53
    assert inventory["min_validator_count"] == 208
    assert inventory["prompt_matrix_rows_required_phrases"] == list(
        vm.PROMPT_MATRIX_ROWS_REQUIRED_PHRASES
    )
    assert inventory["prompt_monthly_checkboxes_required_phrases"] == list(
        vm.PROMPT_MONTHLY_CHECKBOXES_REQUIRED_PHRASES
    )
    assert inventory["prompt_usage_numbered_required_phrases"] == list(
        vm.PROMPT_USAGE_NUMBERED_REQUIRED_PHRASES
    )
    assert inventory["prompt_example_steps_required_phrases"] == list(
        vm.PROMPT_EXAMPLE_STEPS_REQUIRED_PHRASES
    )
    assert inventory["prompt_living_bullets_required_phrases"] == list(
        vm.PROMPT_LIVING_BULLETS_REQUIRED_PHRASES
    )
    assert inventory["prompt_quantum_resp_numbered_required_phrases"] == list(
        vm.PROMPT_QUANTUM_RESP_NUMBERED_REQUIRED_PHRASES
    )
    assert inventory["prompt_bc_resp_numbered_required_phrases"] == list(
        vm.PROMPT_BC_RESP_NUMBERED_REQUIRED_PHRASES
    )
    assert inventory["prompt_edge_resp_numbered_required_phrases"] == list(
        vm.PROMPT_EDGE_RESP_NUMBERED_REQUIRED_PHRASES
    )
    assert inventory["prompt_orch_resp_numbered_required_phrases"] == list(
        vm.PROMPT_ORCH_RESP_NUMBERED_REQUIRED_PHRASES
    )
    assert inventory["prompt_user_input_required_phrases"] == list(
        vm.PROMPT_USER_INPUT_REQUIRED_PHRASES
    )
    assert inventory["prompt_cannot_delegate_numbered_required_phrases"] == list(
        vm.PROMPT_CANNOT_DELEGATE_NUMBERED_REQUIRED_PHRASES
    )
    assert inventory["prompt_decision_bullets_required_phrases"] == list(
        vm.PROMPT_DECISION_BULLETS_REQUIRED_PHRASES
    )
    assert sorted(vm.VALIDATORS) == inventory["validator_names"]
    assert vm.validate_prompt_metrics_detail(REPO_ROOT) == []
...
def test_v53_prompt_listform_residual_edge_cases(tmp_path: Path) -> None:
    for fn in [
        vm.validate_prompt_matrix_rows,
        vm.validate_prompt_monthly_checkboxes,
        vm.validate_prompt_usage_numbered,
        vm.validate_prompt_example_steps,
        vm.validate_prompt_living_bullets,
        vm.validate_prompt_quantum_resp_numbered,
        vm.validate_prompt_bc_resp_numbered,
        vm.validate_prompt_edge_resp_numbered,
        vm.validate_prompt_orch_resp_numbered,
        vm.validate_prompt_user_input,
        vm.validate_prompt_cannot_delegate_numbered,
        vm.validate_prompt_decision_bullets,
    ]:
        assert any("missing" in f.message for f in fn(tmp_path))
    ...
    cases = [
        (
            vm.validate_prompt_matrix_rows,
            "missing matrix-rows Algorithm Complexity lock",
            vm.PROMPT_MATRIX_ROWS_REQUIRED_PHRASES,
        ),
        (
            vm.validate_prompt_monthly_checkboxes,
            "missing monthly-checkboxes Review postmortem lock",
            vm.PROMPT_MONTHLY_CHECKBOXES_REQUIRED_PHRASES,
        ),
        (
            vm.validate_prompt_usage_numbered,
            "missing usage-numbered Copy template lock",
            vm.PROMPT_USAGE_NUMBERED_REQUIRED_PHRASES,
        ),
        (
            vm.validate_prompt_example_steps,
            "missing example-steps Reads scratchpad lock",
            vm.PROMPT_EXAMPLE_STEPS_REQUIRED_PHRASES,
        ),
        (
            vm.validate_prompt_living_bullets,
            "missing living-bullets Update them whenever lock",
            vm.PROMPT_LIVING_BULLETS_REQUIRED_PHRASES,
        ),
        (
            vm.validate_prompt_quantum_resp_numbered,
            "missing quantum-resp-numbered Design quantum lock",
            vm.PROMPT_QUANTUM_RESP_NUMBERED_REQUIRED_PHRASES,
        ),
        (
            vm.validate_prompt_bc_resp_numbered,
            "missing bc-resp-numbered Design multi-chain lock",
            vm.PROMPT_BC_RESP_NUMBERED_REQUIRED_PHRASES,
        ),
        (
            vm.validate_prompt_edge_resp_numbered,
            "missing edge-resp-numbered Implement quantum-safe lock",
            vm.PROMPT_EDGE_RESP_NUMBERED_REQUIRED_PHRASES,
        ),
        (
            vm.validate_prompt_orch_resp_numbered,
            "missing orch-resp-numbered Collect outputs lock",
            vm.PROMPT_ORCH_RESP_NUMBERED_REQUIRED_PHRASES,
        ),
        (
            vm.validate_prompt_user_input,
            "missing user-input Review factorization lock",
            vm.PROMPT_USER_INPUT_REQUIRED_PHRASES,
        ),
        (
            vm.validate_prompt_cannot_delegate_numbered,
            "missing cannot-delegate-numbered Final go/no-go lock",
            vm.PROMPT_CANNOT_DELEGATE_NUMBERED_REQUIRED_PHRASES,
        ),
        (
            vm.validate_prompt_decision_bullets,
            "missing decision-bullets Trade-offs lock",
            vm.PROMPT_DECISION_BULLETS_REQUIRED_PHRASES,
        ),
    ]
    for fn, section_msg, phrases in cases:
    ...
    assert vm.validate_prompt_matrix_rows(tmp_path) == []
    assert vm.validate_prompt_monthly_checkboxes(tmp_path) == []
    assert vm.validate_prompt_usage_numbered(tmp_path) == []
    assert vm.validate_prompt_example_steps(tmp_path) == []
    assert vm.validate_prompt_living_bullets(tmp_path) == []
    assert vm.validate_prompt_quantum_resp_numbered(tmp_path) == []
    assert vm.validate_prompt_bc_resp_numbered(tmp_path) == []
    assert vm.validate_prompt_edge_resp_numbered(tmp_path) == []
    assert vm.validate_prompt_orch_resp_numbered(tmp_path) == []
    assert vm.validate_prompt_user_input(tmp_path) == []
    assert vm.validate_prompt_cannot_delegate_numbered(tmp_path) == []
    assert vm.validate_prompt_decision_bullets(tmp_path) == []
    ...
    def _missing(key: str, seed: list[str]) -> None:
        payload = _inventory_payload()
        payload[key] = ["__not_a_required_anchor__"]
        findings = vm._inventory_lock_consistency(
            payload, schema_path="schemas/packaging-inventory.json"
        )
        assert any(f"{key} must include" in f.message for f in findings)
    ...
    for key, phrases in [
        ("prompt_matrix_rows_required_phrases", vm.PROMPT_MATRIX_ROWS_REQUIRED_PHRASES),
        (
            "prompt_monthly_checkboxes_required_phrases",
            vm.PROMPT_MONTHLY_CHECKBOXES_REQUIRED_PHRASES,
        ),
        ("prompt_usage_numbered_required_phrases", vm.PROMPT_USAGE_NUMBERED_REQUIRED_PHRASES),
        ("prompt_example_steps_required_phrases", vm.PROMPT_EXAMPLE_STEPS_REQUIRED_PHRASES),
        ("prompt_living_bullets_required_phrases", vm.PROMPT_LIVING_BULLETS_REQUIRED_PHRASES),
        (
            "prompt_quantum_resp_numbered_required_phrases",
            vm.PROMPT_QUANTUM_RESP_NUMBERED_REQUIRED_PHRASES,
        ),
        ("prompt_bc_resp_numbered_required_phrases", vm.PROMPT_BC_RESP_NUMBERED_REQUIRED_PHRASES),
        (
        ...
        ("prompt_user_input_required_phrases", vm.PROMPT_USER_INPUT_REQUIRED_PHRASES),
        (
            "prompt_cannot_delegate_numbered_required_phrases",
            vm.PROMPT_CANNOT_DELEGATE_NUMBERED_REQUIRED_PHRASES,
        ),
        ("prompt_decision_bullets_required_phrases", vm.PROMPT_DECISION_BULLETS_REQUIRED_PHRASES),
    ]:
        _empty(key)
        _dup(key, phrases)
        _blank(key)
    ...
    for rel in _inventory_payload()["required_paths"]:
        ...
        if not target.exists():
            target.write_text("ok\n", encoding="utf-8")
    invented = {
        "prompt_matrix_rows_required_phrases": list(vm.PROMPT_MATRIX_ROWS_REQUIRED_PHRASES)[:-1]
        + ["invented matrix rows"],
        "prompt_monthly_checkboxes_required_phrases": list(
            vm.PROMPT_MONTHLY_CHECKBOXES_REQUIRED_PHRASES
        )[:-1]
        + ["invented monthly checkboxes"],
        "prompt_usage_numbered_required_phrases": list(
            vm.PROMPT_USAGE_NUMBERED_REQUIRED_PHRASES
        )[:-1]
        + ["invented usage numbered"],
        "prompt_example_steps_required_phrases": list(
            vm.PROMPT_EXAMPLE_STEPS_REQUIRED_PHRASES
        )[:-1]
        + ["invented example steps"],
        "prompt_living_bullets_required_phrases": list(
            vm.PROMPT_LIVING_BULLETS_REQUIRED_PHRASES
        )[:-1]
        + ["invented living bullets"],
        "prompt_quantum_resp_numbered_required_phrases": list(
            vm.PROMPT_QUANTUM_RESP_NUMBERED_REQUIRED_PHRASES
        )[:-1]
        + ["invented quantum resp numbered"],
        "prompt_bc_resp_numbered_required_phrases": list(
            vm.PROMPT_BC_RESP_NUMBERED_REQUIRED_PHRASES
        )[:-1]
        + ["invented bc resp numbered"],
        "prompt_edge_resp_numbered_required_phrases": list(
            vm.PROMPT_EDGE_RESP_NUMBERED_REQUIRED_PHRASES
        )[:-1]
        + ["invented edge resp numbered"],
        "prompt_orch_resp_numbered_required_phrases": list(
            vm.PROMPT_ORCH_RESP_NUMBERED_REQUIRED_PHRASES
        )[:-1]
        + ["invented orch resp numbered"],
        "prompt_user_input_required_phrases": list(vm.PROMPT_USER_INPUT_REQUIRED_PHRASES)[:-1]
        + ["invented user input"],
        "prompt_cannot_delegate_numbered_required_phrases": list(
            vm.PROMPT_CANNOT_DELEGATE_NUMBERED_REQUIRED_PHRASES
        )[:-1]
        + ["invented cannot delegate numbered"],
        "prompt_decision_bullets_required_phrases": list(
            vm.PROMPT_DECISION_BULLETS_REQUIRED_PHRASES
        )[:-1]
        + ["invented decision bullets"],
    }
    (tmp_path / "schemas" / "packaging-inventory.json").write_text(
        json.dumps(_inventory_payload(**invented)),
        encoding="utf-8",
    )
    findings = vm.validate_packaging_inventory(tmp_path)
    for key in invented:
...
def test_v53_inventory_lock_mismatch_and_consistency_matrix(tmp_path: Path) -> None:
    """Large self-test: every v53 list-form phrase key empty/dup/blank/seed + live registry."""
    assert vm.INVENTORY_VERSION == 53
    assert vm.MIN_VALIDATOR_COUNT == 208
    assert len(vm.VALIDATORS) == 208
    assert sorted(vm.VALIDATORS) == json.loads(
...
# Prompt-pre-v52 residual HEAVY edges after #144/#147 tip (retained post-#151).
# EXISTING thirty-three pre-v52 prompt-* fixtures only — excludes twelve v52
# (#144) and twelve v53 list-form modules. Rejects invented timeout sibling
# keys. Distinct from merged #144/#147/#151 and closed CONFLICTING timeout PRs.
# ---------------------------------------------------------------------------

_V52_PROMPT_RESIDUAL_EXCLUDE: frozenset[str] = frozenset(
    {
        # twelve post-#141 v52 residual modules (#144)
        "prompt-metrics-detail",
        "prompt-orch-metrics-detail",
        "prompt-communication-detail",
        "prompt-principles-detail",
        ...
        "prompt-matrix-resolutions",
        "prompt-usage-detail",
        # twelve v53 list-form residual modules (this PR)
        "prompt-matrix-rows",
        "prompt-monthly-checkboxes",
        "prompt-usage-numbered",
        "prompt-example-steps",
...
def test_prompt_pre_v52_residual_modules_existing_only() -> None:
    ...
    # #144 twelve + v53 list-form twelve remain live but are excluded from this slice
...
def test_prompt_pre_v52_residual_live_green() -> None:
    """All thirty-three live pre-v52 prompt validators remain clean; inventory v53/208."""
    modules = _prompt_pre_v52_residual_modules()
    assert len(modules) == 33
    for name, fn, _phrases, _key in modules:
        assert fn(REPO_ROOT) == [], name
...
def test_constitution_residual_live_green() -> None:
    """All fourteen live constitution residual validators remain clean; inventory v53/208."""
    modules = _constitution_residual_modules()
    assert len(modules) == 14
    for name, fn, _phrases, _key in modules:
        assert fn(REPO_ROOT) == [], name
    ...
    # Adjacent #147 / #151 / list-form slices remain green alongside this residual
    assert vm.validate_goose_howto(REPO_ROOT) == []
    assert vm.validate_prompt_orchestration_matrix(REPO_ROOT) == []
    assert vm.validate_prompt_metrics_detail(REPO_ROOT) == []
    assert vm.validate_prompt_matrix_rows(REPO_ROOT) == []
