...
def _inventory_payload(**overrides: object) -> dict:
    payload = {
        ...
        "security_metadata_required_phrases": list(
            vm.SECURITY_METADATA_REQUIRED_PHRASES
        ),
        "security_non_issues_required_phrases": list(
            vm.SECURITY_NON_ISSUES_REQUIRED_PHRASES
        ),
        "security_surfaces_required_phrases": list(
            vm.SECURITY_SURFACES_REQUIRED_PHRASES
        ),
        ...
        ("markdownlint_default", False),
            ...
            (
                ...
                (
                    ...
                    list(vm.SECURITY_METADATA_REQUIRED_PHRASES)[:-1] + ["invented"],
                ),
                (
                    "security_non_issues_required_phrases",
                    list(vm.SECURITY_NON_ISSUES_REQUIRED_PHRASES)[:-1] + ["invented"],
                ),
                (
                    "security_surfaces_required_phrases",
                    list(vm.SECURITY_SURFACES_REQUIRED_PHRASES)[:-1] + ["invented"],
                ),
...
def test_live_v5_validators() -> None:
    assert vm.validate_bug_report_template(REPO_ROOT) == []
    assert vm.validate_feature_request_template(REPO_ROOT) == []
    assert vm.INVENTORY_VERSION == 30
    assert len(vm.VALIDATORS) == 103
...
def test_live_v8_validators() -> None:
    ...
    assert vm.INVENTORY_VERSION == 30
    assert len(vm.VALIDATORS) == 103
    assert vm.MIN_VALIDATOR_COUNT == 103
...
def test_live_v12_validators() -> None:
    ...
    assert inventory["min_validator_count"] == 103
...
def test_live_v30_security_policy_validators() -> None:
    assert vm.validate_security_metadata(REPO_ROOT) == []
    assert vm.validate_security_non_issues(REPO_ROOT) == []
    assert vm.validate_security_surfaces(REPO_ROOT) == []
    assert vm.INVENTORY_VERSION == 30
    assert len(vm.VALIDATORS) == 103
    assert vm.MIN_VALIDATOR_COUNT == 103
    assert "security-metadata" in vm.VALIDATORS
    assert "security-non-issues" in vm.VALIDATORS
    assert "security-surfaces" in vm.VALIDATORS
    inventory = json.loads(
        (REPO_ROOT / "schemas" / "packaging-inventory.json").read_text(encoding="utf-8")
    )
    assert inventory["version"] == 30
    assert inventory["security_metadata_required_phrases"] == list(
        vm.SECURITY_METADATA_REQUIRED_PHRASES
    )
    assert inventory["security_non_issues_required_phrases"] == list(
        vm.SECURITY_NON_ISSUES_REQUIRED_PHRASES
    )
    assert inventory["security_surfaces_required_phrases"] == list(
        vm.SECURITY_SURFACES_REQUIRED_PHRASES
    )
    assert inventory["min_validator_count"] == 103
    assert sorted(vm.VALIDATORS) == inventory["validator_names"]
...
def test_v30_security_policy_edge_cases(tmp_path: Path) -> None:
    assert any("missing" in f.message for f in vm.validate_security_metadata(tmp_path))
    assert any(
        "missing" in f.message for f in vm.validate_security_non_issues(tmp_path)
    )
    assert any("missing" in f.message for f in vm.validate_security_surfaces(tmp_path))

    _write(tmp_path / "SECURITY.md", "# Security\n\nNo locked content.\n")
    findings = vm.validate_security_metadata(tmp_path)
    assert any("missing Security Policy heading" in f.message for f in findings)
    for phrase in vm.SECURITY_METADATA_REQUIRED_PHRASES:
        assert any(phrase in f.message for f in findings)

    findings = vm.validate_security_non_issues(tmp_path)
    assert any("missing Known Non-Issues section" in f.message for f in findings)
    for phrase in vm.SECURITY_NON_ISSUES_REQUIRED_PHRASES:
        assert any(phrase in f.message for f in findings)

    findings = vm.validate_security_surfaces(tmp_path)
    assert any("missing Supported Versions section" in f.message for f in findings)
    for phrase in vm.SECURITY_SURFACES_REQUIRED_PHRASES:
        assert any(phrase in f.message for f in findings)
    ...
    assert vm.validate_security_metadata(tmp_path) == []
    assert vm.validate_security_non_issues(tmp_path) == []
    assert vm.validate_security_surfaces(tmp_path) == []

    empty = _inventory_payload()
    empty["security_metadata_required_phrases"] = []
    findings = vm._inventory_lock_consistency(
        empty, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "security_metadata_required_phrases must not be empty" in f.message
        for f in findings
    )

    dup = _inventory_payload()
    dup["security_metadata_required_phrases"] = [
        vm.SECURITY_METADATA_REQUIRED_PHRASES[0],
        vm.SECURITY_METADATA_REQUIRED_PHRASES[0],
        *vm.SECURITY_METADATA_REQUIRED_PHRASES[1:],
    ]
    findings = vm._inventory_lock_consistency(
        dup, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "security_metadata_required_phrases must be unique" in f.message
        for f in findings
    )

    blank = _inventory_payload()
    blank["security_metadata_required_phrases"] = ["ok", "  "]
    findings = vm._inventory_lock_consistency(
        blank, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "security_metadata_required_phrases entries must be non-empty strings"
        in f.message
        for f in findings
    )

    missing = _inventory_payload()
    missing["security_metadata_required_phrases"] = ["## Supported Versions"]
    findings = vm._inventory_lock_consistency(
        missing, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "security_metadata_required_phrases must include Status" in f.message
        for f in findings
    )

    empty_n = _inventory_payload()
    empty_n["security_non_issues_required_phrases"] = []
    findings = vm._inventory_lock_consistency(
        empty_n, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "security_non_issues_required_phrases must not be empty" in f.message
        for f in findings
    )

    dup_n = _inventory_payload()
    dup_n["security_non_issues_required_phrases"] = [
        vm.SECURITY_NON_ISSUES_REQUIRED_PHRASES[0],
        vm.SECURITY_NON_ISSUES_REQUIRED_PHRASES[0],
        *vm.SECURITY_NON_ISSUES_REQUIRED_PHRASES[1:],
    ]
    findings = vm._inventory_lock_consistency(
        dup_n, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "security_non_issues_required_phrases must be unique" in f.message
        for f in findings
    )

    blank_n = _inventory_payload()
    blank_n["security_non_issues_required_phrases"] = ["ok", "  "]
    findings = vm._inventory_lock_consistency(
        blank_n, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "security_non_issues_required_phrases entries must be non-empty strings"
        in f.message
        for f in findings
    )

    missing_n = _inventory_payload()
    missing_n["security_non_issues_required_phrases"] = ["## Known Non-Issues"]
    findings = vm._inventory_lock_consistency(
        missing_n, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "security_non_issues_required_phrases must include Known" in f.message
        for f in findings
    )

    empty_s = _inventory_payload()
    empty_s["security_surfaces_required_phrases"] = []
    findings = vm._inventory_lock_consistency(
        empty_s, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "security_surfaces_required_phrases must not be empty" in f.message
        for f in findings
    )

    dup_s = _inventory_payload()
    dup_s["security_surfaces_required_phrases"] = [
        vm.SECURITY_SURFACES_REQUIRED_PHRASES[0],
        vm.SECURITY_SURFACES_REQUIRED_PHRASES[0],
        *vm.SECURITY_SURFACES_REQUIRED_PHRASES[1:],
    ]
    findings = vm._inventory_lock_consistency(
        dup_s, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "security_surfaces_required_phrases must be unique" in f.message
        for f in findings
    )

    blank_s = _inventory_payload()
    blank_s["security_surfaces_required_phrases"] = ["ok", "  "]
    findings = vm._inventory_lock_consistency(
        blank_s, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "security_surfaces_required_phrases entries must be non-empty strings"
        in f.message
        for f in findings
    )

    missing_s = _inventory_payload()
    missing_s["security_surfaces_required_phrases"] = ["potential injection risks"]
    findings = vm._inventory_lock_consistency(
        missing_s, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "security_surfaces_required_phrases must include injection" in f.message
        for f in findings
    )
    ...
    findings = vm.validate_packaging_inventory(tmp_path)
    assert any("security_metadata_required_phrases" in f.message for f in findings)
    assert any("security_non_issues_required_phrases" in f.message for f in findings)
    assert any("security_surfaces_required_phrases" in f.message for f in findings)
