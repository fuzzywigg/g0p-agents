...
# ---------------------------------------------------------------------------
# Changelog residual HEAVY edges after tip through #172 (goose-schema+security).
...
# inventory bump. Distinct from merged #172 (goose-schema+security), #166
# (execution), #161 (hydration), #156 (constitution), #151 (prompt-pre-v52),
# #147 (goose-recipe), open MERGEABLE #174 (contributing), and closed CONFLICTING
# #171 changelog tip refill / #150/#158 invent/security siblings.
# ---------------------------------------------------------------------------
...
def test_changelog_residual_modules_existing_only() -> None:
    """Slice targets seven existing changelog modules — not #172/#166/#161/#156."""
    ...
    assert "prompt-orchestration-matrix" in vm.VALIDATORS
    assert "goose-howto" in vm.VALIDATORS
    assert "contributing" in vm.VALIDATORS  # open #174 niche
    assert "constitution-crypto" not in {m[0] for m in modules}
...
def test_changelog_residual_live_green() -> None:
    ...
    # Adjacent tip siblings through #172 remain green alongside this residual
    assert vm.validate_constitution_crypto(REPO_ROOT) == []
    assert vm.validate_hydration_list_b(REPO_ROOT) == []
    assert vm.validate_execution_summary(REPO_ROOT) == []
    assert vm.validate_security_packaging(REPO_ROOT) == []
