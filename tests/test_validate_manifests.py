        ...
        ("markdownlint_default", False),
            ...
            (
                ...
                (
                    ...
                    list(vm.IMPLEMENTATION_ISSUES_REQUIRED_PHRASES)[:-1] + ["invented"],
                ),
                (
                    "implementation_faq_required_phrases",
                    list(vm.IMPLEMENTATION_FAQ_REQUIRED_PHRASES)[:-1] + ["invented"],
                ),
                (
                    "implementation_support_required_phrases",
                    list(vm.IMPLEMENTATION_SUPPORT_REQUIRED_PHRASES)[:-1]
                    + ["invented"],
                ),
                (
...
def test_live_v35_implementation_deepener_validators() -> None:
...
def test_v35_implementation_deepener_edge_cases(tmp_path: Path) -> None:
    assert any(
    ...
    assert any("missing Cirq-too-slow common issue" in f.message for f in findings)
    for phrase in vm.IMPLEMENTATION_ISSUES_REQUIRED_PHRASES:
        assert any(phrase in f.message for f in findings)

    findings = vm.validate_implementation_faq(tmp_path)
    assert any("missing real-quantum-computer FAQ lock" in f.message for f in findings)
    ...
    assert any(
        "implementation_issues_required_phrases must include Issue 1" in f.message
        for f in findings
    )
    ...
    assert any(
        "implementation_faq_required_phrases must include FAQ" in f.message
        for f in findings
    )
    ...
    assert any(
        "implementation_support_required_phrases must include Support" in f.message
        for f in findings
    )
    ...
    findings = vm.validate_packaging_inventory(tmp_path)
    assert any("implementation_issues_required_phrases" in f.message for f in findings)
