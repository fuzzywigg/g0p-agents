...
def _inventory_payload(**overrides: object) -> dict:
    payload = {
        ...
        "issue_sections_required_phrases": list(vm.ISSUE_SECTIONS_REQUIRED_PHRASES),
...
def test_live_v5_validators() -> None:
    assert vm.validate_bug_report_template(REPO_ROOT) == []
    assert vm.validate_feature_request_template(REPO_ROOT) == []
    assert vm.INVENTORY_VERSION == 22
    assert len(vm.VALIDATORS) == 79
...
def test_live_v8_validators() -> None:
    ...
    assert vm.INVENTORY_VERSION == 22
    assert len(vm.VALIDATORS) == 79
    assert vm.MIN_VALIDATOR_COUNT == 79
...
def test_live_v12_validators() -> None:
    ...
    assert inventory["min_validator_count"] == 79
...
def test_live_v22_docs_governance_validators() -> None:
    assert vm.validate_readme_honesty(REPO_ROOT) == []
    assert vm.validate_contributing_issues(REPO_ROOT) == []
    assert vm.validate_issue_sections(REPO_ROOT) == []
    assert vm.INVENTORY_VERSION == 22
    assert len(vm.VALIDATORS) == 79
    assert vm.MIN_VALIDATOR_COUNT == 79
    ...
    assert tuple(vm.CONTRIBUTING_ISSUES_REQUIRED_PHRASES) == (
        "## Issue Reporting",
        "**Bug**",
        "**Feature**",
        "**Agent Task**",
    ...
    assert tuple(vm.ISSUE_SECTIONS_REQUIRED_PHRASES) == (
        "## Problem",
        "## Proposed Solution",
        "## Acceptance Criteria",
    )
    ...
    assert inventory["issue_sections_required_phrases"] == list(
        vm.ISSUE_SECTIONS_REQUIRED_PHRASES
    )
    assert inventory["min_validator_count"] == 79
    assert sorted(vm.VALIDATORS) == inventory["validator_names"]
    assert "readme-honesty" in vm.VALIDATORS
    assert "contributing-issues" in vm.VALIDATORS
    assert "issue-sections" in vm.VALIDATORS
    assert vm.validate_readme_packaging(REPO_ROOT) == []
    assert vm.validate_readme_badges(REPO_ROOT) == []
    assert vm.validate_contributing_packaging(REPO_ROOT) == []
    assert vm.validate_issue_metadata(REPO_ROOT) == []
...
def test_v22_docs_governance_edge_cases(tmp_path: Path) -> None:
    ...
    assert any("missing" in f.message for f in vm.validate_issue_sections(tmp_path))
    ...
    for rel in vm.ISSUE_TEMPLATE_FILES:
        _write(tmp_path / rel, "# template\n\nNo sections.\n")
    findings = vm.validate_issue_sections(tmp_path)
    for phrase in vm.ISSUE_SECTIONS_REQUIRED_PHRASES:
        assert any(phrase in f.message for f in findings)
    ...
    _write(
        ...
        "\n".join(
            [
                ...
                "",
                "**This is not a live hive, runtime, or production swarm.**",
                "prompt fiction. There is no running hive mind in this tree.",
                "Docs-only archive of 2025 protocols.",
                "",
            ]
        ),
    )
    ...
    for rel in vm.ISSUE_TEMPLATE_FILES:
        _write(tmp_path / rel, body)
    assert vm.validate_issue_sections(tmp_path) == []

    empty_honesty = _inventory_payload()
    ...
    dup_honesty = _inventory_payload()
    ...
    blank_honesty = _inventory_payload()
    blank_honesty["readme_honesty_required_phrases"] = ["prompt fiction", "  "]
    findings = vm._inventory_lock_consistency(
    ...
    missing_honesty = _inventory_payload()
    missing_honesty["readme_honesty_required_phrases"] = [
        "archived reference",
        "Docs-only archive",
    ]
    findings = vm._inventory_lock_consistency(
    ...
    empty_issues = _inventory_payload()
    empty_issues["contributing_issues_required_phrases"] = []
    findings = vm._inventory_lock_consistency(
        empty_issues, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
    ...
    dup_issues = _inventory_payload()
    dup_issues["contributing_issues_required_phrases"] = [
        vm.CONTRIBUTING_ISSUES_REQUIRED_PHRASES[0],
        vm.CONTRIBUTING_ISSUES_REQUIRED_PHRASES[0],
        *vm.CONTRIBUTING_ISSUES_REQUIRED_PHRASES[1:],
    ]
    findings = vm._inventory_lock_consistency(
        dup_issues, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
    ...
    blank_issues = _inventory_payload()
    blank_issues["contributing_issues_required_phrases"] = ["**Bug**", " "]
    findings = vm._inventory_lock_consistency(
        blank_issues, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
    ...
    missing_issues = _inventory_payload()
    missing_issues["contributing_issues_required_phrases"] = [
        "## Issue Reporting",
        "**Bug**",
    ]
    findings = vm._inventory_lock_consistency(
        missing_issues, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
    ...
    empty_sections = _inventory_payload()
    empty_sections["issue_sections_required_phrases"] = []
    findings = vm._inventory_lock_consistency(
        empty_sections, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "issue_sections_required_phrases must not be empty" in f.message
        for f in findings
    )

    dup_sections = _inventory_payload()
    dup_sections["issue_sections_required_phrases"] = [
        vm.ISSUE_SECTIONS_REQUIRED_PHRASES[0],
        vm.ISSUE_SECTIONS_REQUIRED_PHRASES[0],
        *vm.ISSUE_SECTIONS_REQUIRED_PHRASES[1:],
    ]
    findings = vm._inventory_lock_consistency(
        dup_sections, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "issue_sections_required_phrases must be unique" in f.message for f in findings
    )

    blank_sections = _inventory_payload()
    blank_sections["issue_sections_required_phrases"] = ["## Problem", "  "]
    findings = vm._inventory_lock_consistency(
        blank_sections, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
    ...
    missing_sections = _inventory_payload()
    missing_sections["issue_sections_required_phrases"] = [
        "## Problem",
        "## Proposed Solution",
    ]
    findings = vm._inventory_lock_consistency(
        missing_sections, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
    ...
    (tmp_path / "schemas" / "packaging-inventory.json").write_text(
        json.dumps(
            _inventory_payload(
                readme_honesty_required_phrases=list(
                    vm.README_HONESTY_REQUIRED_PHRASES
                )[:-1]
                + ["invented honesty"],
                contributing_issues_required_phrases=list(
                ...
                + ["invented issues"],
                issue_sections_required_phrases=list(
                    vm.ISSUE_SECTIONS_REQUIRED_PHRASES
                )[:-1]
                + ["invented sections"],
            )
    ...
    assert any("issue_sections_required_phrases" in f.message for f in findings)
