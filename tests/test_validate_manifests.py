...
# ---------------------------------------------------------------------------
# Security/manifest residual HEAVY edges after #151 tip.
# EXISTING ten security / security-* phrase-lock validators — no invented
# product / inventory bump. Distinct leftover from merged #144/#147/#151 and
# closed CONFLICTING #148/#149/#150/#155.
# ---------------------------------------------------------------------------
...
def test_security_residual_live_green() -> None:
    ...
    # Adjacent #144 / #147 / #151 slices remain green alongside this residual
    assert vm.validate_prompt_metrics_detail(REPO_ROOT) == []
    assert vm.validate_goose_howto(REPO_ROOT) == []
    assert vm.validate_prompt_orchestration_matrix(REPO_ROOT) == []
    assert vm.validate_scratchpad(REPO_ROOT) == []
