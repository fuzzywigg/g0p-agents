...
def test_live_v5_validators() -> None:
    assert vm.validate_bug_report_template(REPO_ROOT) == []
    assert vm.validate_feature_request_template(REPO_ROOT) == []
    assert vm.INVENTORY_VERSION == 27
    assert len(vm.VALIDATORS) == 94
...
def test_live_v8_validators() -> None:
    ...
    assert vm.INVENTORY_VERSION == 27
    assert len(vm.VALIDATORS) == 94
    assert vm.MIN_VALIDATOR_COUNT == 94
...
def test_live_v12_validators() -> None:
    ...
    assert inventory["min_validator_count"] == 94
...
def test_v27_implementation_guide_edge_cases(tmp_path: Path) -> None:
    ...
    assert any("missing Quantum tools section" in f.message for f in findings)
    assert any("missing Orchestration tools section" in f.message for f in findings)
    ...
    assert any("missing By End of Week 1 criteria" in f.message for f in findings)
    assert any("missing By End of Month criteria" in f.message for f in findings)
    ...
    assert any("implementation_success_required_phrases" in f.message for f in findings)
