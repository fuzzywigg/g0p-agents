...
def _inventory_payload(**overrides: object) -> dict:
    payload = {
        ...
        "contributing_validation_required_phrases": list(
            vm.CONTRIBUTING_VALIDATION_REQUIRED_PHRASES
        ),
...
def test_live_v29_contributing_validators() -> None:
    ...
    assert tuple(vm.CONTRIBUTING_GOVERNANCE_REQUIRED_PHRASES) == (
        "## Governance",
        "Structural changes",
    ...
    assert inventory["version"] == 29
    assert inventory["contributing_validation_required_phrases"] == list(
        vm.CONTRIBUTING_VALIDATION_REQUIRED_PHRASES
    )
...
def test_v29_contributing_edge_cases(tmp_path: Path) -> None:
    assert any(
        "missing" in f.message for f in vm.validate_contributing_validation(tmp_path)
    )
    assert any(
        "missing" in f.message for f in vm.validate_contributing_issues(tmp_path)
    )
    assert any(
    ...
    _write(tmp_path / "CONTRIBUTING.md", "# Contributing\n\nNo locked content.\n")
    findings = vm.validate_contributing_validation(tmp_path)
    assert any("missing Local validation section" in f.message for f in findings)
    for phrase in vm.CONTRIBUTING_VALIDATION_REQUIRED_PHRASES:
        assert any(phrase in f.message for f in findings)
    ...
    assert vm.validate_contributing_validation(tmp_path) == []
    assert vm.validate_contributing_issues(tmp_path) == []
    assert vm.validate_contributing_governance(tmp_path) == []

    empty = _inventory_payload()
    empty["contributing_validation_required_phrases"] = []
    findings = vm._inventory_lock_consistency(
        empty, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "contributing_validation_required_phrases must not be empty" in f.message
        for f in findings
    )

    dup = _inventory_payload()
    dup["contributing_validation_required_phrases"] = [
        vm.CONTRIBUTING_VALIDATION_REQUIRED_PHRASES[0],
        vm.CONTRIBUTING_VALIDATION_REQUIRED_PHRASES[0],
        *vm.CONTRIBUTING_VALIDATION_REQUIRED_PHRASES[1:],
    ]
    findings = vm._inventory_lock_consistency(
        dup, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "contributing_validation_required_phrases must be unique" in f.message
        for f in findings
    )

    blank = _inventory_payload()
    blank["contributing_validation_required_phrases"] = ["ok", "  "]
    findings = vm._inventory_lock_consistency(
        blank, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "contributing_validation_required_phrases entries must be non-empty strings"
        in f.message
        for f in findings
    )

    missing = _inventory_payload()
    missing["contributing_validation_required_phrases"] = [
        "## Local validation (docs packaging)"
    ]
    findings = vm._inventory_lock_consistency(
        missing, schema_path="schemas/packaging-inventory.json"
    ...
    assert any(
        "contributing_validation_required_phrases must include Local" in f.message
        for f in findings
    )

    empty_i = _inventory_payload()
    empty_i["contributing_issues_required_phrases"] = []
    findings = vm._inventory_lock_consistency(
    ...
    dup_i["contributing_issues_required_phrases"] = [
    ...
    blank_i["contributing_issues_required_phrases"] = ["ok", "  "]
    findings = vm._inventory_lock_consistency(
    ...
    missing_i["contributing_issues_required_phrases"] = ["## Issue Reporting"]
    findings = vm._inventory_lock_consistency(
    ...
    findings = vm.validate_packaging_inventory(tmp_path)
    assert any(
        "contributing_validation_required_phrases" in f.message for f in findings
    )
