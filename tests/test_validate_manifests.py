...
def _inventory_payload(**overrides: object) -> dict:
    payload = {
        ...
        "hydration_issues_required_phrases": list(vm.HYDRATION_ISSUES_REQUIRED_PHRASES),
...
def test_live_v30_hydration_report_validators() -> None:
    ...
    assert len(vm.VALIDATORS) == 103
    assert vm.MIN_VALIDATOR_COUNT == 103
    assert "hydration-findings" in vm.VALIDATORS
    assert "hydration-list-a" in vm.VALIDATORS
    assert "hydration-issues" in vm.VALIDATORS
    inventory = json.loads(
        (REPO_ROOT / "schemas" / "packaging-inventory.json").read_text(encoding="utf-8")
    )
    assert inventory["version"] == 30
...
def test_v30_hydration_report_edge_cases(tmp_path: Path) -> None:
    assert any(
        "missing" in f.message for f in vm.validate_hydration_findings(tmp_path)
    )
    ...
    assert any(
        "missing PHASE 1 Identity findings section" in f.message for f in findings
    )
    assert any("missing PHASE 1 CI/CD findings section" in f.message for f in findings)
    assert any(
        "missing PHASE 1 Git State findings section" in f.message for f in findings
    )
    ...
    assert any("missing LIST B Deferred section" in f.message for f in findings)
    ...
    empty = _inventory_payload()
    empty["hydration_findings_required_phrases"] = []
    findings = vm._inventory_lock_consistency(
        empty, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "hydration_findings_required_phrases must not be empty" in f.message
        for f in findings
    )

    dup = _inventory_payload()
    dup["hydration_findings_required_phrases"] = [
        vm.HYDRATION_FINDINGS_REQUIRED_PHRASES[0],
        vm.HYDRATION_FINDINGS_REQUIRED_PHRASES[0],
        *vm.HYDRATION_FINDINGS_REQUIRED_PHRASES[1:],
    ]
    findings = vm._inventory_lock_consistency(
        dup, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "hydration_findings_required_phrases must be unique" in f.message
        for f in findings
    )

    blank = _inventory_payload()
    blank["hydration_findings_required_phrases"] = ["ok", "  "]
    findings = vm._inventory_lock_consistency(
        blank, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "hydration_findings_required_phrases entries must be non-empty strings"
        in f.message
        for f in findings
    )

    missing = _inventory_payload()
    missing["hydration_findings_required_phrases"] = ["### Identity"]
    findings = vm._inventory_lock_consistency(
        missing, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "hydration_findings_required_phrases must include Identity" in f.message
        for f in findings
    )

    empty_a = _inventory_payload()
    empty_a["hydration_list_a_required_phrases"] = []
    findings = vm._inventory_lock_consistency(
        empty_a, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "hydration_list_a_required_phrases must not be empty" in f.message
        for f in findings
    )

    dup_a = _inventory_payload()
    dup_a["hydration_list_a_required_phrases"] = [
        vm.HYDRATION_LIST_A_REQUIRED_PHRASES[0],
        vm.HYDRATION_LIST_A_REQUIRED_PHRASES[0],
        *vm.HYDRATION_LIST_A_REQUIRED_PHRASES[1:],
    ]
    findings = vm._inventory_lock_consistency(
        dup_a, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "hydration_list_a_required_phrases must be unique" in f.message
        for f in findings
    )

    blank_a = _inventory_payload()
    blank_a["hydration_list_a_required_phrases"] = ["ok", "  "]
    findings = vm._inventory_lock_consistency(
    ...
    missing_a["hydration_list_a_required_phrases"] = ["### LIST A — Researchable"]
    findings = vm._inventory_lock_consistency(
        missing_a, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "hydration_list_a_required_phrases must include LIST A" in f.message
        for f in findings
    )

    empty_i = _inventory_payload()
    empty_i["hydration_issues_required_phrases"] = []
    findings = vm._inventory_lock_consistency(
        empty_i, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "hydration_issues_required_phrases must not be empty" in f.message
        for f in findings
    )

    dup_i = _inventory_payload()
    dup_i["hydration_issues_required_phrases"] = [
        vm.HYDRATION_ISSUES_REQUIRED_PHRASES[0],
        vm.HYDRATION_ISSUES_REQUIRED_PHRASES[0],
        *vm.HYDRATION_ISSUES_REQUIRED_PHRASES[1:],
    ]
    findings = vm._inventory_lock_consistency(
        dup_i, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "hydration_issues_required_phrases must be unique" in f.message
        for f in findings
    )

    blank_i = _inventory_payload()
    blank_i["hydration_issues_required_phrases"] = ["ok", "  "]
    findings = vm._inventory_lock_consistency(
        blank_i, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "hydration_issues_required_phrases entries must be non-empty strings"
        in f.message
        for f in findings
    )

    missing_i = _inventory_payload()
    missing_i["hydration_issues_required_phrases"] = ["## PHASE 4: ISSUES GENERATED"]
    findings = vm._inventory_lock_consistency(
        missing_i, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "hydration_issues_required_phrases must include PHASE 4" in f.message
        for f in findings
    )
    ...
    assert any("hydration_issues_required_phrases" in f.message for f in findings)
