...
def test_live_v5_validators() -> None:
    assert vm.validate_bug_report_template(REPO_ROOT) == []
    assert vm.validate_feature_request_template(REPO_ROOT) == []
    assert vm.INVENTORY_VERSION == 29
    assert len(vm.VALIDATORS) == 100
...
def test_live_v8_validators() -> None:
    ...
    assert vm.INVENTORY_VERSION == 29
    assert len(vm.VALIDATORS) == 100
    assert vm.MIN_VALIDATOR_COUNT == 100
...
def test_live_v12_validators() -> None:
    ...
    assert inventory["min_validator_count"] == 100
...
def test_live_v29_hydration_report_validators() -> None:
...
def test_v29_hydration_report_edge_cases(tmp_path: Path) -> None:
    ...
    assert any(
        "hydration_findings_required_phrases must include PHASE 1" in f.message
        for f in findings
    )
