...
def _inventory_payload(**overrides: object) -> dict:
    payload = {
        ...
        "prompt_project_context_required_phrases": list(
            vm.PROMPT_PROJECT_CONTEXT_REQUIRED_PHRASES
        ),
        "prompt_authority_required_phrases": list(vm.PROMPT_AUTHORITY_REQUIRED_PHRASES),
        ...
        ("markdownlint_default", False),
            ...
            (
                ...
                (
                    ...
                    list(vm.PROMPT_PROJECT_CONTEXT_REQUIRED_PHRASES)[:-1]
                    + ["invented"],
                ),
                (
                    "prompt_authority_required_phrases",
                    list(vm.PROMPT_AUTHORITY_REQUIRED_PHRASES)[:-1] + ["invented"],
                ),
                (
...
def test_live_v5_validators() -> None:
    assert vm.validate_bug_report_template(REPO_ROOT) == []
    assert vm.validate_feature_request_template(REPO_ROOT) == []
    assert vm.INVENTORY_VERSION == 49
    assert len(vm.VALIDATORS) == 169
...
def test_live_v8_validators() -> None:
    ...
    assert vm.INVENTORY_VERSION == 49
    assert len(vm.VALIDATORS) == 169
    assert vm.MIN_VALIDATOR_COUNT == 169
...
def test_live_v12_validators() -> None:
    ...
    assert inventory["min_validator_count"] == 169
...
def test_v38_contributing_deepener_edge_cases(tmp_path: Path) -> None:
    ...
    assert any(
        "missing Packaging inventory v49 honesty lock" in f.message for f in findings
    )
...
def test_live_v44_hydration_deepener_validators() -> None:
    ...
    assert sorted(vm.VALIDATORS) == inventory["validator_names"]
    assert "Packaging inventory v49" in (
...
def test_live_v47_prompt_tools_communication_escalation_validators() -> None:
    ...
    assert "inventory v49 locks" in (REPO_ROOT / "README.md").read_text(
        encoding="utf-8"
    )
...
def test_v47_inventory_lock_mismatch_and_consistency_matrix(tmp_path: Path) -> None:
    ...
    assert any(
        "Packaging inventory v49" in phrase
        for phrase in vm.CONTRIBUTING_CI_HONESTY_REQUIRED_PHRASES
    )
...
def test_live_v49_prompt_responsibilities_project_context_authority_validators() -> None:
    assert vm.validate_prompt_responsibilities(REPO_ROOT) == []
    assert vm.validate_prompt_project_context(REPO_ROOT) == []
    assert vm.validate_prompt_authority(REPO_ROOT) == []
    assert vm.INVENTORY_VERSION == 49
    assert len(vm.VALIDATORS) == 169
    assert vm.MIN_VALIDATOR_COUNT == 169
    for name in [
    ...
    assert inventory["prompt_project_context_required_phrases"] == list(
        vm.PROMPT_PROJECT_CONTEXT_REQUIRED_PHRASES
    )
    assert inventory["prompt_authority_required_phrases"] == list(
        vm.PROMPT_AUTHORITY_REQUIRED_PHRASES
    )
    assert sorted(vm.VALIDATORS) == inventory["validator_names"]
    # prior prompt leftovers remain green
    assert vm.validate_prompt_orchestration_matrix(REPO_ROOT) == []
...
def test_v49_prompt_responsibilities_project_context_authority_edge_cases(
    tmp_path: Path,
) -> None:
    for fn in [
        vm.validate_prompt_responsibilities,
        vm.validate_prompt_project_context,
        vm.validate_prompt_authority,
    ]:
        assert any("missing" in f.message for f in fn(tmp_path))
    ...
    # Escalation Authority section gate
    _write(
        tmp_path / "AGENT-PROMPTS.md",
        "\n".join(
            [
    ...
    findings = vm.validate_prompt_authority(tmp_path)
    # Decision Authority heading satisfies one gate; Escalation may still be in phrases
    # If Escalation Authority heading is in phrases, section gate for Escalation is also satisfied.
    # Force missing Escalation Authority heading while keeping other phrases:
    body = "\n".join(
        p for p in vm.PROMPT_AUTHORITY_REQUIRED_PHRASES if p != "## Your Escalation Authority"
    )
    _write(tmp_path / "AGENT-PROMPTS.md", "## Your Decision Authority\n" + body + "\n")
    findings = vm.validate_prompt_authority(tmp_path)
    assert any("missing Your Escalation Authority section" in f.message for f in findings)
    ...
    assert vm.validate_prompt_project_context(tmp_path) == []
    assert vm.validate_prompt_authority(tmp_path) == []
    ...
    (tmp_path / "schemas" / "packaging-inventory.json").write_text(
        json.dumps(
            _inventory_payload(
                ...
                prompt_authority_required_phrases=list(
                    vm.PROMPT_AUTHORITY_REQUIRED_PHRASES
                )[:-1]
                + ["invented authority"],
            )
    ...
    assert any(
        "prompt_project_context_required_phrases" in f.message for f in findings
    )
    assert any("prompt_authority_required_phrases" in f.message for f in findings)
