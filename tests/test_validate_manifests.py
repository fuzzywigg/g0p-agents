...
def _inventory_payload(**overrides: object) -> dict:
    payload = {
        ...
        "prompt_instantiation_residual_required_phrases": list(
            vm.PROMPT_INSTANTIATION_RESIDUAL_REQUIRED_PHRASES
        ),
        "prompt_related_docs_residual_required_phrases": list(
            vm.PROMPT_RELATED_DOCS_RESIDUAL_REQUIRED_PHRASES
        ),
        "prompt_living_update_required_phrases": list(
            vm.PROMPT_LIVING_UPDATE_REQUIRED_PHRASES
        ),
        "prompt_conflict_resolve_required_phrases": list(
            vm.PROMPT_CONFLICT_RESOLVE_REQUIRED_PHRASES
        ),
        "prompt_vision_monthly_required_phrases": list(
            vm.PROMPT_VISION_MONTHLY_REQUIRED_PHRASES
        ),
        "prompt_usage_input_required_phrases": list(
            vm.PROMPT_USAGE_INPUT_REQUIRED_PHRASES
        ),
        "prompt_go_nogo_required_phrases": list(
            vm.PROMPT_GO_NOGO_REQUIRED_PHRASES
        ),
        "prompt_comm_residual_required_phrases": list(
            vm.PROMPT_COMM_RESIDUAL_REQUIRED_PHRASES
        ),
        "prompt_metrics_residual_required_phrases": list(
            vm.PROMPT_METRICS_RESIDUAL_REQUIRED_PHRASES
        ),
...
def test_validators_registry_covers_all_checks() -> None:
    expected = {
        ...
        "prompt-instantiation-residual",
        "prompt-related-docs-residual",
        "prompt-living-update",
        "prompt-conflict-resolve",
        "prompt-vision-monthly",
        "prompt-usage-input",
        "prompt-go-nogo",
        "prompt-comm-residual",
        "prompt-metrics-residual",
        "hydration-phase2",
        "hydration-phase5",
        "license-mit",
        "link-check",
...
def test_live_v8_validators() -> None:
    ...
    assert vm.INVENTORY_VERSION == 51
    assert len(vm.VALIDATORS) == 193
    assert vm.MIN_VALIDATOR_COUNT == 193
...
def test_live_v12_validators() -> None:
    ...
    assert inventory["min_validator_count"] == 193
...
def test_v38_contributing_deepener_edge_cases(tmp_path: Path) -> None:
    ...
    assert any(
        "missing Packaging inventory v51 honesty lock" in f.message for f in findings
    )
...
def test_live_v44_hydration_deepener_validators() -> None:
    ...
    assert sorted(vm.VALIDATORS) == inventory["validator_names"]
    assert "Packaging inventory v51" in (
...
def test_live_v51_prompt_residuals_r2_validators() -> None:
    for name in [
        "prompt-instantiation-residual",
        "prompt-related-docs-residual",
        "prompt-living-update",
    ...
    assert inventory["version"] == 51
    assert inventory["min_validator_count"] == 193
    assert inventory["prompt_instantiation_residual_required_phrases"] == list(
        vm.PROMPT_INSTANTIATION_RESIDUAL_REQUIRED_PHRASES
    )
    assert inventory["prompt_related_docs_residual_required_phrases"] == list(
        vm.PROMPT_RELATED_DOCS_RESIDUAL_REQUIRED_PHRASES
    )
    assert inventory["prompt_living_update_required_phrases"] == list(
        vm.PROMPT_LIVING_UPDATE_REQUIRED_PHRASES
    )
    assert inventory["prompt_conflict_resolve_required_phrases"] == list(
        vm.PROMPT_CONFLICT_RESOLVE_REQUIRED_PHRASES
    )
    assert inventory["prompt_vision_monthly_required_phrases"] == list(
        vm.PROMPT_VISION_MONTHLY_REQUIRED_PHRASES
    )
    assert inventory["prompt_usage_input_required_phrases"] == list(
        vm.PROMPT_USAGE_INPUT_REQUIRED_PHRASES
    )
    assert inventory["prompt_go_nogo_required_phrases"] == list(
        vm.PROMPT_GO_NOGO_REQUIRED_PHRASES
    )
    assert inventory["prompt_comm_residual_required_phrases"] == list(
        vm.PROMPT_COMM_RESIDUAL_REQUIRED_PHRASES
    )
    assert inventory["prompt_metrics_residual_required_phrases"] == list(
        vm.PROMPT_METRICS_RESIDUAL_REQUIRED_PHRASES
    )
    assert sorted(vm.VALIDATORS) == inventory["validator_names"]
    # prior v51 heavy leftovers remain green
    assert vm.validate_prompt_role_blurbs(REPO_ROOT) == []
    assert vm.validate_prompt_instantiation(REPO_ROOT) == []
    assert vm.validate_prompt_related_health(REPO_ROOT) == []
    assert vm.validate_prompt_living_docs(REPO_ROOT) == []
...
def test_v51_prompt_residuals_r2_edge_cases(tmp_path: Path) -> None:
    validators = [
        vm.validate_prompt_instantiation_residual,
        vm.validate_prompt_related_docs_residual,
        vm.validate_prompt_living_update,
        vm.validate_prompt_conflict_resolve,
        vm.validate_prompt_vision_monthly,
        vm.validate_prompt_usage_input,
        vm.validate_prompt_go_nogo,
        vm.validate_prompt_comm_residual,
        vm.validate_prompt_metrics_residual,
    ]
    ...
    cases = [
        (
            vm.validate_prompt_instantiation_residual,
            'missing instantiation-residual Copy template lock',
            vm.PROMPT_INSTANTIATION_RESIDUAL_REQUIRED_PHRASES,
        ),
        (
            vm.validate_prompt_related_docs_residual,
            'missing related-docs-residual postmortem incident log lock',
            vm.PROMPT_RELATED_DOCS_RESIDUAL_REQUIRED_PHRASES,
        ),
        (
            vm.validate_prompt_living_update,
            'missing living-update living documents lock',
            vm.PROMPT_LIVING_UPDATE_REQUIRED_PHRASES,
        ),
        (
            vm.validate_prompt_conflict_resolve,
            'missing conflict-resolve testnet approach lock',
            vm.PROMPT_CONFLICT_RESOLVE_REQUIRED_PHRASES,
        ),
        (
            vm.validate_prompt_vision_monthly,
            'missing vision-monthly Current Project Vision lock',
            vm.PROMPT_VISION_MONTHLY_REQUIRED_PHRASES,
        ),
        (
            vm.validate_prompt_usage_input,
            'missing usage-input User Input factorization lock',
            vm.PROMPT_USAGE_INPUT_REQUIRED_PHRASES,
        ),
        (
            vm.validate_prompt_go_nogo,
            'missing go-nogo Make final go/no-go decision lock',
            vm.PROMPT_GO_NOGO_REQUIRED_PHRASES,
        ),
        (
            vm.validate_prompt_comm_residual,
            'missing comm-residual Conservative on claims lock',
            vm.PROMPT_COMM_RESIDUAL_REQUIRED_PHRASES,
        ),
        (
            vm.validate_prompt_metrics_residual,
            'missing metrics-residual Team consensus lock',
            vm.PROMPT_METRICS_RESIDUAL_REQUIRED_PHRASES,
        ),
    ]
    for fn, section_msg, phrases in cases:
    ...
    assert vm.validate_prompt_instantiation_residual(tmp_path) == []
    assert vm.validate_prompt_related_docs_residual(tmp_path) == []
    assert vm.validate_prompt_living_update(tmp_path) == []
    assert vm.validate_prompt_conflict_resolve(tmp_path) == []
    assert vm.validate_prompt_vision_monthly(tmp_path) == []
    assert vm.validate_prompt_usage_input(tmp_path) == []
    assert vm.validate_prompt_go_nogo(tmp_path) == []
    assert vm.validate_prompt_comm_residual(tmp_path) == []
    assert vm.validate_prompt_metrics_residual(tmp_path) == []
    ...
    for key, phrases, seed in [
        ...
        (
            ...
            vm.PROMPT_USAGE_INPUT_REQUIRED_PHRASES,
            ['Review the proposed factorization circuit'],
        ),
        (
    ...
    overrides = {
        ...
        + ["invented prompt-instantiation-residual"],
        "prompt_related_docs_residual_required_phrases": list(
            vm.PROMPT_RELATED_DOCS_RESIDUAL_REQUIRED_PHRASES
        )[:-1]
        + ["invented prompt-related-docs-residual"],
        "prompt_living_update_required_phrases": list(
            vm.PROMPT_LIVING_UPDATE_REQUIRED_PHRASES
        )[:-1]
        + ["invented prompt-living-update"],
        "prompt_conflict_resolve_required_phrases": list(
            vm.PROMPT_CONFLICT_RESOLVE_REQUIRED_PHRASES
        )[:-1]
        + ["invented prompt-conflict-resolve"],
        "prompt_vision_monthly_required_phrases": list(
            vm.PROMPT_VISION_MONTHLY_REQUIRED_PHRASES
        )[:-1]
        + ["invented prompt-vision-monthly"],
        "prompt_usage_input_required_phrases": list(
            vm.PROMPT_USAGE_INPUT_REQUIRED_PHRASES
        )[:-1]
        + ["invented prompt-usage-input"],
        "prompt_go_nogo_required_phrases": list(
            vm.PROMPT_GO_NOGO_REQUIRED_PHRASES
        )[:-1]
        + ["invented prompt-go-nogo"],
        "prompt_comm_residual_required_phrases": list(
            vm.PROMPT_COMM_RESIDUAL_REQUIRED_PHRASES
        )[:-1]
        + ["invented prompt-comm-residual"],
        "prompt_metrics_residual_required_phrases": list(
            vm.PROMPT_METRICS_RESIDUAL_REQUIRED_PHRASES
        )[:-1]
        + ["invented prompt-metrics-residual"],
    }
    (tmp_path / "schemas" / "packaging-inventory.json").write_text(
        json.dumps(_inventory_payload(**overrides)),
        encoding="utf-8",
    )
    findings = vm.validate_packaging_inventory(tmp_path)
    for key in [
...
def test_v51_residuals_r2_inventory_lock_mismatch_and_consistency_matrix(
    tmp_path: Path,
) -> None:
    """Large self-test: every v51 residuals-r2 phrase key empty/dup/blank/seed."""
    assert vm.INVENTORY_VERSION == 51
    assert vm.MIN_VALIDATOR_COUNT == 193
    assert len(vm.VALIDATORS) == 193
    assert sorted(vm.VALIDATORS) == json.loads(
    ...
    keys = [
        (
            'prompt_instantiation_residual_required_phrases',
            vm.PROMPT_INSTANTIATION_RESIDUAL_REQUIRED_PHRASES,
            'Copy template/Fill INSERT/Paste LLM/postmortem incident log',
        ),
        (
            'prompt_related_docs_residual_required_phrases',
            vm.PROMPT_RELATED_DOCS_RESIDUAL_REQUIRED_PHRASES,
        ...
        (
            'prompt_living_update_required_phrases',
            vm.PROMPT_LIVING_UPDATE_REQUIRED_PHRASES,
            'living documents/Update them whenever/AGENTS.md constraints change',
        ),
        (
            'prompt_conflict_resolve_required_phrases',
            vm.PROMPT_CONFLICT_RESOLVE_REQUIRED_PHRASES,
            'testnet approach/Redesign contract/hybrid post-quantum/escalate to stakeholder',
        ),
        (
            'prompt_vision_monthly_required_phrases',
            vm.PROMPT_VISION_MONTHLY_REQUIRED_PHRASES,
            'Current Project Vision/risk register/Communicate progress/Timeline INSERT',
        ),
        (
            'prompt_usage_input_required_phrases',
            vm.PROMPT_USAGE_INPUT_REQUIRED_PHRASES,
            'User Input factorization/Cirq optimization/scratchpad to track progress',
        ),
        (
            'prompt_go_nogo_required_phrases',
            vm.PROMPT_GO_NOGO_REQUIRED_PHRASES,
            'Make final go/no-go decision/Identify conflicts/Collect outputs',
        ),
        (
            'prompt_comm_residual_required_phrases',
            vm.PROMPT_COMM_RESIDUAL_REQUIRED_PHRASES,
            'Conservative claims/Flag quantum advantage/Explain trade-off/Acknowledge risks',
        ),
        (
            'prompt_metrics_residual_required_phrases',
            vm.PROMPT_METRICS_RESIDUAL_REQUIRED_PHRASES,
            'Team consensus/Stakeholder satisfaction/Qualtran/Gate count optimized',
        ),
    ]
    for key, phrases, include_token in keys:
        payload = _inventory_payload()
        ...
        payload = _inventory_payload()
        payload[key] = ["decoy residual phrase"]
        findings = vm._inventory_lock_consistency(
            payload, schema_path="schemas/packaging-inventory.json"
        )
        assert any(f"{key} must include" in f.message for f in findings)
