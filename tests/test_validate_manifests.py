        ...
        ("markdownlint_default", False),
            ...
            (
                ...
                (
                    ...
                    list(vm.GOOSE_HOWTO_REQUIRED_PHRASES)[:-1] + ["invented"],
                ),
                (
                    "goose_state_machine_required_phrases",
                    list(vm.GOOSE_STATE_MACHINE_REQUIRED_PHRASES)[:-1] + ["invented"],
                ),
                (
                    "goose_naming_required_phrases",
                    list(vm.GOOSE_NAMING_REQUIRED_PHRASES)[:-1] + ["invented"],
                ),
...
def test_live_v5_validators() -> None:
    assert vm.validate_bug_report_template(REPO_ROOT) == []
    assert vm.validate_feature_request_template(REPO_ROOT) == []
    assert vm.INVENTORY_VERSION == 25
    assert len(vm.VALIDATORS) == 88
...
def test_live_v8_validators() -> None:
    ...
    assert vm.INVENTORY_VERSION == 25
    assert len(vm.VALIDATORS) == 88
    assert vm.MIN_VALIDATOR_COUNT == 88
...
def test_live_v12_validators() -> None:
    ...
    assert inventory["min_validator_count"] == 88
...
def test_live_v25_goose_recipes_validators() -> None:
    ...
    assert len(vm.VALIDATORS) == 88
    assert vm.MIN_VALIDATOR_COUNT == 88
    assert tuple(vm.GOOSE_HOWTO_REQUIRED_PHRASES) == (
    ...
    assert tuple(vm.GOOSE_STATE_MACHINE_REQUIRED_PHRASES) == (
    ...
    assert tuple(vm.GOOSE_NAMING_REQUIRED_PHRASES) == (
...
def test_v25_goose_recipes_edge_cases(tmp_path: Path) -> None:
    assert any("missing" in f.message for f in vm.validate_goose_howto(tmp_path))
    assert any(
        "missing" in f.message for f in vm.validate_goose_state_machine(tmp_path)
    )
    ...
    assert any("missing How to Use These Recipes section" in f.message for f in findings)
    ...
    _write(
        ...
        "\n".join(
            [
                ...
                "",
                (
                    "Every recipe updates `./agentic_flows/scratchpad.txt` "
                ...
                "5. **Update AGENTS.md** if it defines new constraints",
                "",
            ]
        ),
    )
    ...
    inventory = _inventory_payload()
    empty_howto = dict(inventory)
    empty_howto["goose_howto_required_phrases"] = []
    findings = vm._inventory_lock_consistency(
        empty_howto, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "goose_howto_required_phrases must not be empty" in f.message for f in findings
    )

    dup_howto = dict(inventory)
    dup_howto["goose_howto_required_phrases"] = [
        vm.GOOSE_HOWTO_REQUIRED_PHRASES[0],
        vm.GOOSE_HOWTO_REQUIRED_PHRASES[0],
        *vm.GOOSE_HOWTO_REQUIRED_PHRASES[1:],
    ]
    findings = vm._inventory_lock_consistency(
        dup_howto, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "goose_howto_required_phrases must be unique" in f.message for f in findings
    )

    blank_howto = dict(inventory)
    blank_howto["goose_howto_required_phrases"] = ["ok", "  "]
    findings = vm._inventory_lock_consistency(
        blank_howto, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
    ...
    missing_howto = dict(inventory)
    missing_howto["goose_howto_required_phrases"] = [
        "## How to Use These Recipes",
        "goose run ./agentic_flows/quantum_algorithm_design.yaml",
    ]
    findings = vm._inventory_lock_consistency(
        missing_howto, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
    ...
    empty_state = dict(inventory)
    empty_state["goose_state_machine_required_phrases"] = []
    findings = vm._inventory_lock_consistency(
        empty_state, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
    ...
    dup_state = dict(inventory)
    dup_state["goose_state_machine_required_phrases"] = [
        vm.GOOSE_STATE_MACHINE_REQUIRED_PHRASES[0],
        vm.GOOSE_STATE_MACHINE_REQUIRED_PHRASES[0],
        *vm.GOOSE_STATE_MACHINE_REQUIRED_PHRASES[1:],
    ]
    findings = vm._inventory_lock_consistency(
        dup_state, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
    ...
    blank_state = dict(inventory)
    blank_state["goose_state_machine_required_phrases"] = ["ok", "  "]
    findings = vm._inventory_lock_consistency(
        blank_state, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
    ...
    missing_state = dict(inventory)
    missing_state["goose_state_machine_required_phrases"] = [
        "## Scratchpad State Machine",
        "./agentic_flows/scratchpad.txt",
    ]
    findings = vm._inventory_lock_consistency(
        missing_state, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
    ...
    empty_naming = dict(inventory)
    empty_naming["goose_naming_required_phrases"] = []
    findings = vm._inventory_lock_consistency(
        empty_naming, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "goose_naming_required_phrases must not be empty" in f.message for f in findings
    )

    dup_naming = dict(inventory)
    dup_naming["goose_naming_required_phrases"] = [
        vm.GOOSE_NAMING_REQUIRED_PHRASES[0],
        vm.GOOSE_NAMING_REQUIRED_PHRASES[0],
        *vm.GOOSE_NAMING_REQUIRED_PHRASES[1:],
    ]
    findings = vm._inventory_lock_consistency(
        dup_naming, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "goose_naming_required_phrases must be unique" in f.message for f in findings
    )

    blank_naming = dict(inventory)
    blank_naming["goose_naming_required_phrases"] = ["ok", "  "]
    findings = vm._inventory_lock_consistency(
        blank_naming, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
    ...
    missing_naming = dict(inventory)
    missing_naming["goose_naming_required_phrases"] = [
        "## Recipe Naming Convention",
        "[domain]_[action]_[target].yaml",
    ]
    findings = vm._inventory_lock_consistency(
        missing_naming, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "goose_naming_required_phrases must include Naming Convention" in f.message
        for f in findings
    )
