...
def test_live_v41_readme_leftover_validators() -> None:
...
def test_v41_readme_leftover_edge_cases(tmp_path: Path) -> None:
    ...
    missing = _inventory_payload()
    missing["readme_lead_required_phrases"] = [
    ...
    assert any(
        "readme_blurbs_required_phrases must not be empty" in f.message
        for f in findings
    )
    ...
    missing_b["readme_blurbs_required_phrases"] = [
    ...
    assert any(
        "readme_bootstrap_required_phrases must be unique" in f.message
        for f in findings
    )
