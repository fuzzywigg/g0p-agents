...
def _inventory_payload(**overrides: object) -> dict:
    payload = {
        ...
        "constitution_specialists_required_phrases": list(
            vm.CONSTITUTION_SPECIALISTS_REQUIRED_PHRASES
        ),
        "constitution_crypto_required_phrases": list(
            vm.CONSTITUTION_CRYPTO_REQUIRED_PHRASES
        ),
        "constitution_risk_required_phrases": list(
            vm.CONSTITUTION_RISK_REQUIRED_PHRASES
        ),
...
def test_live_v30_constitution_validators() -> None:
    assert vm.validate_constitution_specialists(REPO_ROOT) == []
    assert vm.validate_constitution_crypto(REPO_ROOT) == []
    assert vm.validate_constitution_risk(REPO_ROOT) == []
    assert vm.INVENTORY_VERSION == 30
    assert len(vm.VALIDATORS) == 103
    assert vm.MIN_VALIDATOR_COUNT == 103
    assert "constitution-specialists" in vm.VALIDATORS
    assert "constitution-crypto" in vm.VALIDATORS
    assert "constitution-risk" in vm.VALIDATORS
    inventory = json.loads(
        (REPO_ROOT / "schemas" / "packaging-inventory.json").read_text(encoding="utf-8")
    )
    assert inventory["version"] == 30
    assert inventory["constitution_specialists_required_phrases"] == list(
        vm.CONSTITUTION_SPECIALISTS_REQUIRED_PHRASES
    )
    assert inventory["constitution_crypto_required_phrases"] == list(
        vm.CONSTITUTION_CRYPTO_REQUIRED_PHRASES
    )
    assert inventory["constitution_risk_required_phrases"] == list(
        vm.CONSTITUTION_RISK_REQUIRED_PHRASES
    )
    assert inventory["min_validator_count"] == 103
    assert sorted(vm.VALIDATORS) == inventory["validator_names"]
...
def test_v30_constitution_edge_cases(tmp_path: Path) -> None:
    assert any(
        "missing" in f.message for f in vm.validate_constitution_specialists(tmp_path)
    )
    assert any("missing" in f.message for f in vm.validate_constitution_crypto(tmp_path))
    assert any("missing" in f.message for f in vm.validate_constitution_risk(tmp_path))

    _write(tmp_path / "AGENTS-v2.2.md", "# Agents\n\nNo locked content.\n")
    findings = vm.validate_constitution_specialists(tmp_path)
    assert any("missing 6.4.1 Specialist Agents section" in f.message for f in findings)
    for phrase in vm.CONSTITUTION_SPECIALISTS_REQUIRED_PHRASES:
        assert any(phrase in f.message for f in findings)

    findings = vm.validate_constitution_crypto(tmp_path)
    assert any(
        "missing 22.1 Quantum-Safe Cryptography section" in f.message for f in findings
    )
    for phrase in vm.CONSTITUTION_CRYPTO_REQUIRED_PHRASES:
        assert any(phrase in f.message for f in findings)

    findings = vm.validate_constitution_risk(tmp_path)
    assert any(
        "missing Section 25 Risk Tolerance Review" in f.message for f in findings
    )
    for phrase in vm.CONSTITUTION_RISK_REQUIRED_PHRASES:
        assert any(phrase in f.message for f in findings)
    ...
    assert vm.validate_constitution_specialists(tmp_path) == []
    assert vm.validate_constitution_crypto(tmp_path) == []
    assert vm.validate_constitution_risk(tmp_path) == []

    empty = _inventory_payload()
    empty["constitution_specialists_required_phrases"] = []
    findings = vm._inventory_lock_consistency(
        empty, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "constitution_specialists_required_phrases must not be empty" in f.message
        for f in findings
    )

    dup = _inventory_payload()
    dup["constitution_specialists_required_phrases"] = [
        vm.CONSTITUTION_SPECIALISTS_REQUIRED_PHRASES[0],
        vm.CONSTITUTION_SPECIALISTS_REQUIRED_PHRASES[0],
        *vm.CONSTITUTION_SPECIALISTS_REQUIRED_PHRASES[1:],
    ]
    findings = vm._inventory_lock_consistency(
        dup, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "constitution_specialists_required_phrases must be unique" in f.message
        for f in findings
    )

    blank = _inventory_payload()
    blank["constitution_specialists_required_phrases"] = ["ok", "  "]
    findings = vm._inventory_lock_consistency(
        blank, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "constitution_specialists_required_phrases entries must be non-empty strings"
        in f.message
        for f in findings
    )

    missing = _inventory_payload()
    missing["constitution_specialists_required_phrases"] = [
    ...
    assert any(
        "constitution_specialists_required_phrases must include" in f.message
        for f in findings
    )

    empty_c = _inventory_payload()
    empty_c["constitution_crypto_required_phrases"] = []
    findings = vm._inventory_lock_consistency(
    ...
    dup_c["constitution_crypto_required_phrases"] = [
    ...
    blank_c["constitution_crypto_required_phrases"] = ["ok", "  "]
    findings = vm._inventory_lock_consistency(
    ...
    missing_c["constitution_crypto_required_phrases"] = [
    ...
    assert any(
        "constitution_crypto_required_phrases must include 22.1" in f.message
        for f in findings
    )

    empty_r = _inventory_payload()
    empty_r["constitution_risk_required_phrases"] = []
    findings = vm._inventory_lock_consistency(
        empty_r, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "constitution_risk_required_phrases must not be empty" in f.message
        for f in findings
    )

    dup_r = _inventory_payload()
    dup_r["constitution_risk_required_phrases"] = [
        vm.CONSTITUTION_RISK_REQUIRED_PHRASES[0],
        vm.CONSTITUTION_RISK_REQUIRED_PHRASES[0],
        *vm.CONSTITUTION_RISK_REQUIRED_PHRASES[1:],
    ]
    findings = vm._inventory_lock_consistency(
        dup_r, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "constitution_risk_required_phrases must be unique" in f.message
        for f in findings
    )

    blank_r = _inventory_payload()
    blank_r["constitution_risk_required_phrases"] = ["ok", "  "]
    findings = vm._inventory_lock_consistency(
        blank_r, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "constitution_risk_required_phrases entries must be non-empty strings"
        in f.message
        for f in findings
    )

    missing_r = _inventory_payload()
    missing_r["constitution_risk_required_phrases"] = [
    ...
    assert any(
        "constitution_risk_required_phrases must include Section 25" in f.message
        for f in findings
    )
    ...
    findings = vm.validate_packaging_inventory(tmp_path)
    assert any(
        "constitution_specialists_required_phrases" in f.message for f in findings
    )
    assert any("constitution_crypto_required_phrases" in f.message for f in findings)
    assert any("constitution_risk_required_phrases" in f.message for f in findings)
