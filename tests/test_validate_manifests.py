...
def _inventory_payload(**overrides: object) -> dict:
    payload = {
        ...
        "postmortem_decision_required_phrases": list(
            vm.POSTMORTEM_DECISION_REQUIRED_PHRASES
        ),
        "postmortem_context_detail_required_phrases": list(
            vm.POSTMORTEM_CONTEXT_DETAIL_REQUIRED_PHRASES
        ),
        "postmortem_blocked_required_phrases": list(
            vm.POSTMORTEM_BLOCKED_REQUIRED_PHRASES
        ),
        ...
        ("markdownlint_default", False),
            ...
            (
                ...
                (
                    ...
                    list(vm.POSTMORTEM_DECISION_REQUIRED_PHRASES)[:-1] + ["invented"],
                ),
                (
                    "postmortem_context_detail_required_phrases",
                    list(vm.POSTMORTEM_CONTEXT_DETAIL_REQUIRED_PHRASES)[:-1]
                    + ["invented"],
                ),
                (
                    "postmortem_blocked_required_phrases",
                    list(vm.POSTMORTEM_BLOCKED_REQUIRED_PHRASES)[:-1] + ["invented"],
                ),
                (
...
def test_live_v5_validators() -> None:
    assert vm.validate_bug_report_template(REPO_ROOT) == []
    assert vm.validate_feature_request_template(REPO_ROOT) == []
    assert vm.INVENTORY_VERSION == 40
    assert len(vm.VALIDATORS) == 133
...
def test_live_v8_validators() -> None:
    ...
    assert vm.INVENTORY_VERSION == 40
    assert len(vm.VALIDATORS) == 133
    assert vm.MIN_VALIDATOR_COUNT == 133
...
def test_live_v12_validators() -> None:
    ...
    assert inventory["min_validator_count"] == 133
...
def test_v38_contributing_deepener_edge_cases(tmp_path: Path) -> None:
    ...
    assert any(
        "missing Packaging inventory v40 honesty lock" in f.message for f in findings
    )
...
def test_live_v40_postmortem_deepener_validators() -> None:
    assert vm.validate_postmortem_decision(REPO_ROOT) == []
    assert vm.validate_postmortem_context_detail(REPO_ROOT) == []
    assert vm.validate_postmortem_blocked(REPO_ROOT) == []
    assert vm.INVENTORY_VERSION == 40
    assert len(vm.VALIDATORS) == 133
    assert vm.MIN_VALIDATOR_COUNT == 133
    assert "postmortem-decision" in vm.VALIDATORS
    assert "postmortem-context-detail" in vm.VALIDATORS
    assert "postmortem-blocked" in vm.VALIDATORS
    inventory = json.loads(
        (REPO_ROOT / "schemas" / "packaging-inventory.json").read_text(encoding="utf-8")
    )
    assert inventory["version"] == 40
    assert inventory["postmortem_decision_required_phrases"] == list(
        vm.POSTMORTEM_DECISION_REQUIRED_PHRASES
    )
    assert inventory["postmortem_context_detail_required_phrases"] == list(
        vm.POSTMORTEM_CONTEXT_DETAIL_REQUIRED_PHRASES
    )
    assert inventory["postmortem_blocked_required_phrases"] == list(
        vm.POSTMORTEM_BLOCKED_REQUIRED_PHRASES
    )
    assert inventory["min_validator_count"] == 133
    assert sorted(vm.VALIDATORS) == inventory["validator_names"]
...
def test_v40_postmortem_deepener_edge_cases(tmp_path: Path) -> None:
    assert any("missing" in f.message for f in vm.validate_postmortem_decision(tmp_path))
    assert any(
        "missing" in f.message for f in vm.validate_postmortem_context_detail(tmp_path)
    )
    assert any("missing" in f.message for f in vm.validate_postmortem_blocked(tmp_path))

    _write(tmp_path / "postmortem.md", "# postmortem\n\nNo locked content.\n")
    findings = vm.validate_postmortem_decision(tmp_path)
    assert any("missing Repo Hydration Decision heading" in f.message for f in findings)
    for phrase in vm.POSTMORTEM_DECISION_REQUIRED_PHRASES:
        assert any(phrase in f.message for f in findings)

    findings = vm.validate_postmortem_context_detail(tmp_path)
    assert any("missing postmortem risk-level leftover" in f.message for f in findings)
    for phrase in vm.POSTMORTEM_CONTEXT_DETAIL_REQUIRED_PHRASES:
        assert any(phrase in f.message for f in findings)

    findings = vm.validate_postmortem_blocked(tmp_path)
    assert any(
        "missing postmortem blocked gh-CLI leftover" in f.message for f in findings
    )
    for phrase in vm.POSTMORTEM_BLOCKED_REQUIRED_PHRASES:
        assert any(phrase in f.message for f in findings)
    ...
    assert vm.validate_postmortem_decision(tmp_path) == []
    assert vm.validate_postmortem_context_detail(tmp_path) == []
    assert vm.validate_postmortem_blocked(tmp_path) == []

    empty = _inventory_payload()
    empty["postmortem_decision_required_phrases"] = []
    findings = vm._inventory_lock_consistency(
        empty, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "postmortem_decision_required_phrases must not be empty" in f.message
        for f in findings
    )

    dup = _inventory_payload()
    dup["postmortem_decision_required_phrases"] = [
        vm.POSTMORTEM_DECISION_REQUIRED_PHRASES[0],
        vm.POSTMORTEM_DECISION_REQUIRED_PHRASES[0],
        *vm.POSTMORTEM_DECISION_REQUIRED_PHRASES[1:],
    ]
    findings = vm._inventory_lock_consistency(
        dup, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "postmortem_decision_required_phrases must be unique" in f.message
        for f in findings
    )

    blank = _inventory_payload()
    blank["postmortem_decision_required_phrases"] = ["ok", "  "]
    findings = vm._inventory_lock_consistency(
        blank, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "postmortem_decision_required_phrases entries must be non-empty strings"
        in f.message
        for f in findings
    )

    missing = _inventory_payload()
    missing["postmortem_decision_required_phrases"] = [
        "## Decision: Repo Hydration — 2026-04-13"
    ]
    findings = vm._inventory_lock_consistency(
        missing, schema_path="schemas/packaging-inventory.json"
    ...
    assert any(
        "postmortem_decision_required_phrases must include" in f.message
        for f in findings
    )

    empty_c = _inventory_payload()
    empty_c["postmortem_context_detail_required_phrases"] = []
    findings = vm._inventory_lock_consistency(
        empty_c, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "postmortem_context_detail_required_phrases must not be empty" in f.message
        for f in findings
    )

    dup_c = _inventory_payload()
    dup_c["postmortem_context_detail_required_phrases"] = [
        vm.POSTMORTEM_CONTEXT_DETAIL_REQUIRED_PHRASES[0],
        vm.POSTMORTEM_CONTEXT_DETAIL_REQUIRED_PHRASES[0],
        *vm.POSTMORTEM_CONTEXT_DETAIL_REQUIRED_PHRASES[1:],
    ]
    findings = vm._inventory_lock_consistency(
        dup_c, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "postmortem_context_detail_required_phrases must be unique" in f.message
        for f in findings
    )

    blank_c = _inventory_payload()
    blank_c["postmortem_context_detail_required_phrases"] = ["ok", "  "]
    findings = vm._inventory_lock_consistency(
        blank_c, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "postmortem_context_detail_required_phrases entries must be "
        "non-empty strings" in f.message
        for f in findings
    )

    missing_c = _inventory_payload()
    missing_c["postmortem_context_detail_required_phrases"] = [
    ...
    assert any(
        "postmortem_context_detail_required_phrases must include" in f.message
        for f in findings
    )

    empty_b = _inventory_payload()
    empty_b["postmortem_blocked_required_phrases"] = []
    findings = vm._inventory_lock_consistency(
        empty_b, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "postmortem_blocked_required_phrases must not be empty" in f.message
        for f in findings
    )

    dup_b = _inventory_payload()
    dup_b["postmortem_blocked_required_phrases"] = [
        vm.POSTMORTEM_BLOCKED_REQUIRED_PHRASES[0],
        vm.POSTMORTEM_BLOCKED_REQUIRED_PHRASES[0],
        *vm.POSTMORTEM_BLOCKED_REQUIRED_PHRASES[1:],
    ]
    findings = vm._inventory_lock_consistency(
        dup_b, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "postmortem_blocked_required_phrases must be unique" in f.message
        for f in findings
    )

    blank_b = _inventory_payload()
    blank_b["postmortem_blocked_required_phrases"] = ["ok", "  "]
    findings = vm._inventory_lock_consistency(
        blank_b, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "postmortem_blocked_required_phrases entries must be non-empty strings"
        in f.message
        for f in findings
    )

    missing_b = _inventory_payload()
    missing_b["postmortem_blocked_required_phrases"] = ["**Next Steps**:"]
    findings = vm._inventory_lock_consistency(
        missing_b, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "postmortem_blocked_required_phrases must include" in f.message
        for f in findings
    )
    ...
    findings = vm.validate_packaging_inventory(tmp_path)
    assert any("postmortem_decision_required_phrases" in f.message for f in findings)
    assert any(
        "postmortem_context_detail_required_phrases" in f.message for f in findings
    )
    assert any("postmortem_blocked_required_phrases" in f.message for f in findings)
