...
def _inventory_payload(**overrides: object) -> dict:
    payload = {
        ...
        "contributing_branches_required_phrases": list(
            vm.CONTRIBUTING_BRANCHES_REQUIRED_PHRASES
        ),
...
def test_live_v22_contributing_validators() -> None:
    ...
    assert tuple(vm.CONTRIBUTING_BRANCHES_REQUIRED_PHRASES) == (
        "## Branch Strategy",
        "| Branch | Purpose |",
        "`alpha`",
        "`copilot/<task>`",
    ...
    assert inventory["contributing_branches_required_phrases"] == list(
        vm.CONTRIBUTING_BRANCHES_REQUIRED_PHRASES
    )
...
def test_v22_contributing_edge_cases(tmp_path: Path) -> None:
    ...
    assert any("missing Who Can Contribute section" in f.message for f in findings)
    ...
    for phrase in vm.CONTRIBUTING_BRANCHES_REQUIRED_PHRASES:
        assert any(phrase in f.message for f in findings)
    ...
    body = "\n".join(
        [
            ...
            "- **Andrew Pappas** (owner)",
            "- **Ecosystem collaborators** — PikoClaw",
            "",
            "## Branch Strategy",
            "",
            "| Branch | Purpose |",
            "| --- | --- |",
            "| `alpha` | Default. Never push directly. |",
            ...
            "## PR Requirements",
            "",
            "1. Branch off from `alpha`",
            "2. Fill in the PR template completely",
            "3. All CI checks must pass before merge (and",
            "   manifest validate on Python 3.11/3.12/3.13)",
            "4. One approval required",
            "",
        ]
    )
    _write(tmp_path / "CONTRIBUTING.md", body)
    ...
    blank_who = _inventory_payload()
    blank_who["contributing_who_required_phrases"] = ["## Who Can Contribute", "  "]
    findings = vm._inventory_lock_consistency(
    ...
    missing_who = _inventory_payload()
    ...
    empty_branches = _inventory_payload()
    empty_branches["contributing_branches_required_phrases"] = []
    findings = vm._inventory_lock_consistency(
        empty_branches, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "contributing_branches_required_phrases must not be empty" in f.message
        for f in findings
    )

    dup_branches = _inventory_payload()
    dup_branches["contributing_branches_required_phrases"] = [
        vm.CONTRIBUTING_BRANCHES_REQUIRED_PHRASES[0],
        vm.CONTRIBUTING_BRANCHES_REQUIRED_PHRASES[0],
        *vm.CONTRIBUTING_BRANCHES_REQUIRED_PHRASES[1:],
    ]
    findings = vm._inventory_lock_consistency(
        dup_branches, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "contributing_branches_required_phrases must be unique" in f.message
        for f in findings
    )

    blank_branches = _inventory_payload()
    blank_branches["contributing_branches_required_phrases"] = [
        "## Branch Strategy",
        " ",
    ]
    findings = vm._inventory_lock_consistency(
    ...
    assert any(
        "contributing_branches_required_phrases entries must be non-empty strings"
        in f.message
        for f in findings
    )

    missing_branches = _inventory_payload()
    missing_branches["contributing_branches_required_phrases"] = [
        "## Branch Strategy",
        "`alpha`",
    ]
    findings = vm._inventory_lock_consistency(
    ...
    blank_pr = _inventory_payload()
    blank_pr["contributing_pr_required_phrases"] = ["## PR Requirements", "  "]
    findings = vm._inventory_lock_consistency(
        blank_pr, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
    ...
    assert any(
        "contributing_branches_required_phrases" in f.message for f in findings
    )
