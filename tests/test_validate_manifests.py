...
def _inventory_payload(**overrides: object) -> dict:
    payload = {
        ...
        "prompt_quantum_hw_metrics_required_phrases": list(
            vm.PROMPT_QUANTUM_HW_METRICS_REQUIRED_PHRASES
        ),
        "prompt_bc_metrics_required_phrases": list(
            vm.PROMPT_BC_METRICS_REQUIRED_PHRASES
        ),
        "prompt_edge_review_metrics_required_phrases": list(
            vm.PROMPT_EDGE_REVIEW_METRICS_REQUIRED_PHRASES
        ),
        "prompt_orch_delivery_metrics_required_phrases": list(
            vm.PROMPT_ORCH_DELIVERY_METRICS_REQUIRED_PHRASES
        ),
        "prompt_quantum_comms_residual_required_phrases": list(
            vm.PROMPT_QUANTUM_COMMS_RESIDUAL_REQUIRED_PHRASES
        ),
        "prompt_bc_edge_comms_required_phrases": list(
            vm.PROMPT_BC_EDGE_COMMS_REQUIRED_PHRASES
        ),
        "prompt_orch_comms_residual_required_phrases": list(
            vm.PROMPT_ORCH_COMMS_RESIDUAL_REQUIRED_PHRASES
        ),
        "prompt_quantum_principles_residual_required_phrases": list(
            vm.PROMPT_QUANTUM_PRINCIPLES_RESIDUAL_REQUIRED_PHRASES
        ),
        "prompt_bc_edge_principles_required_phrases": list(
            vm.PROMPT_BC_EDGE_PRINCIPLES_REQUIRED_PHRASES
        ),
        "prompt_escalate_closures_required_phrases": list(
            vm.PROMPT_ESCALATE_CLOSURES_REQUIRED_PHRASES
        ),
        "prompt_qualtran_expertise_residual_required_phrases": list(
            vm.PROMPT_QUALTRAN_EXPERTISE_RESIDUAL_REQUIRED_PHRASES
        ),
        "prompt_vision_matrix_residual_required_phrases": list(
            vm.PROMPT_VISION_MATRIX_RESIDUAL_REQUIRED_PHRASES
        ),
...
def test_validators_registry_covers_all_checks() -> None:
    expected = {
        ...
        "prompt-quantum-hw-metrics",
        "prompt-bc-metrics",
        "prompt-edge-review-metrics",
        "prompt-orch-delivery-metrics",
        "prompt-quantum-comms-residual",
        "prompt-bc-edge-comms",
        "prompt-orch-comms-residual",
        "prompt-quantum-principles-residual",
        "prompt-bc-edge-principles",
        "prompt-escalate-closures",
        "prompt-qualtran-expertise-residual",
        "prompt-vision-matrix-residual",
        "hydration-phase2",
        "hydration-phase5",
        "license-mit",
        "link-check",
...
def test_live_v52_prompt_b_residual_validators() -> None:
    assert vm.validate_prompt_quantum_hw_metrics(REPO_ROOT) == []
    assert vm.validate_prompt_bc_metrics(REPO_ROOT) == []
    assert vm.validate_prompt_edge_review_metrics(REPO_ROOT) == []
    assert vm.validate_prompt_orch_delivery_metrics(REPO_ROOT) == []
    assert vm.validate_prompt_quantum_comms_residual(REPO_ROOT) == []
    assert vm.validate_prompt_bc_edge_comms(REPO_ROOT) == []
    assert vm.validate_prompt_orch_comms_residual(REPO_ROOT) == []
    assert vm.validate_prompt_quantum_principles_residual(REPO_ROOT) == []
    assert vm.validate_prompt_bc_edge_principles(REPO_ROOT) == []
    assert vm.validate_prompt_escalate_closures(REPO_ROOT) == []
    assert vm.validate_prompt_qualtran_expertise_residual(REPO_ROOT) == []
    assert vm.validate_prompt_vision_matrix_residual(REPO_ROOT) == []
    assert vm.INVENTORY_VERSION == 52
    assert len(vm.VALIDATORS) == 196
    assert vm.MIN_VALIDATOR_COUNT == 196
    for name in [
    ...
    assert inventory["version"] == 52
    assert inventory["min_validator_count"] == 196
    assert inventory["prompt_quantum_hw_metrics_required_phrases"] == list(
        vm.PROMPT_QUANTUM_HW_METRICS_REQUIRED_PHRASES
    )
    assert inventory["prompt_bc_metrics_required_phrases"] == list(
        vm.PROMPT_BC_METRICS_REQUIRED_PHRASES
    )
    assert inventory["prompt_edge_review_metrics_required_phrases"] == list(
        vm.PROMPT_EDGE_REVIEW_METRICS_REQUIRED_PHRASES
    )
    assert inventory["prompt_orch_delivery_metrics_required_phrases"] == list(
        vm.PROMPT_ORCH_DELIVERY_METRICS_REQUIRED_PHRASES
    )
    assert inventory["prompt_quantum_comms_residual_required_phrases"] == list(
        vm.PROMPT_QUANTUM_COMMS_RESIDUAL_REQUIRED_PHRASES
    )
    assert inventory["prompt_bc_edge_comms_required_phrases"] == list(
        vm.PROMPT_BC_EDGE_COMMS_REQUIRED_PHRASES
    )
    assert inventory["prompt_orch_comms_residual_required_phrases"] == list(
        vm.PROMPT_ORCH_COMMS_RESIDUAL_REQUIRED_PHRASES
    )
    assert inventory["prompt_quantum_principles_residual_required_phrases"] == list(
        vm.PROMPT_QUANTUM_PRINCIPLES_RESIDUAL_REQUIRED_PHRASES
    )
    assert inventory["prompt_bc_edge_principles_required_phrases"] == list(
        vm.PROMPT_BC_EDGE_PRINCIPLES_REQUIRED_PHRASES
    )
    assert inventory["prompt_escalate_closures_required_phrases"] == list(
        vm.PROMPT_ESCALATE_CLOSURES_REQUIRED_PHRASES
    )
    assert inventory["prompt_qualtran_expertise_residual_required_phrases"] == list(
        vm.PROMPT_QUALTRAN_EXPERTISE_RESIDUAL_REQUIRED_PHRASES
    )
    assert inventory["prompt_vision_matrix_residual_required_phrases"] == list(
        vm.PROMPT_VISION_MATRIX_RESIDUAL_REQUIRED_PHRASES
    )
    assert sorted(vm.VALIDATORS) == inventory["validator_names"]
    # prior prompt leftovers remain green
...
def test_v52_prompt_b_residual_edge_cases(tmp_path: Path) -> None:
    for fn in [
        vm.validate_prompt_quantum_hw_metrics,
        vm.validate_prompt_bc_metrics,
        vm.validate_prompt_edge_review_metrics,
        vm.validate_prompt_orch_delivery_metrics,
        vm.validate_prompt_quantum_comms_residual,
        vm.validate_prompt_bc_edge_comms,
        vm.validate_prompt_orch_comms_residual,
        vm.validate_prompt_quantum_principles_residual,
        vm.validate_prompt_bc_edge_principles,
        vm.validate_prompt_escalate_closures,
        vm.validate_prompt_qualtran_expertise_residual,
        vm.validate_prompt_vision_matrix_residual,
    ]:
        assert any("missing" in f.message for f in fn(tmp_path))
    ...
    cases = [
        (
            vm.validate_prompt_quantum_hw_metrics,
            "missing quantum-hw-metrics Gate count lock",
            vm.PROMPT_QUANTUM_HW_METRICS_REQUIRED_PHRASES,
        ),
        (
            vm.validate_prompt_bc_metrics,
            "missing bc-metrics Multi-chain sync lock",
            vm.PROMPT_BC_METRICS_REQUIRED_PHRASES,
        ),
        (
            vm.validate_prompt_edge_review_metrics,
            "missing edge-review-metrics Apple security review lock",
            vm.PROMPT_EDGE_REVIEW_METRICS_REQUIRED_PHRASES,
        ),
        (
            vm.validate_prompt_orch_delivery_metrics,
            "missing orch-delivery-metrics On-time delivery lock",
            vm.PROMPT_ORCH_DELIVERY_METRICS_REQUIRED_PHRASES,
        ),
        (
            vm.validate_prompt_quantum_comms_residual,
            "missing quantum-comms-residual Flag quantum advantage lock",
            vm.PROMPT_QUANTUM_COMMS_RESIDUAL_REQUIRED_PHRASES,
        ),
        (
            vm.validate_prompt_bc_edge_comms,
            "missing bc-edge-comms architecture diagrams lock",
            vm.PROMPT_BC_EDGE_COMMS_REQUIRED_PHRASES,
        ),
        (
            vm.validate_prompt_orch_comms_residual,
            "missing orch-comms-residual trade-off reasoning lock",
            vm.PROMPT_ORCH_COMMS_RESIDUAL_REQUIRED_PHRASES,
        ),
        (
            vm.validate_prompt_quantum_principles_residual,
            "missing quantum-principles-residual multiple backends lock",
            vm.PROMPT_QUANTUM_PRINCIPLES_RESIDUAL_REQUIRED_PHRASES,
        ),
        (
            vm.validate_prompt_bc_edge_principles,
            "missing bc-edge-principles quantum-resistant lock",
            vm.PROMPT_BC_EDGE_PRINCIPLES_REQUIRED_PHRASES,
        ),
        (
            vm.validate_prompt_escalate_closures,
            "missing escalate-closures Escalate when constraints lock",
            vm.PROMPT_ESCALATE_CLOSURES_REQUIRED_PHRASES,
        ),
        (
            vm.validate_prompt_qualtran_expertise_residual,
            "missing qualtran-expertise-residual Qualtran resource lock",
            vm.PROMPT_QUALTRAN_EXPERTISE_RESIDUAL_REQUIRED_PHRASES,
        ),
        (
            vm.validate_prompt_vision_matrix_residual,
            "missing vision-matrix-residual Current Project Vision lock",
            vm.PROMPT_VISION_MATRIX_RESIDUAL_REQUIRED_PHRASES,
        ),
    ]
    for fn, section_msg, phrases in cases:
    ...
    assert vm.validate_prompt_quantum_hw_metrics(tmp_path) == []
    assert vm.validate_prompt_bc_metrics(tmp_path) == []
    assert vm.validate_prompt_edge_review_metrics(tmp_path) == []
    assert vm.validate_prompt_orch_delivery_metrics(tmp_path) == []
    assert vm.validate_prompt_quantum_comms_residual(tmp_path) == []
    assert vm.validate_prompt_bc_edge_comms(tmp_path) == []
    assert vm.validate_prompt_orch_comms_residual(tmp_path) == []
    assert vm.validate_prompt_quantum_principles_residual(tmp_path) == []
    assert vm.validate_prompt_bc_edge_principles(tmp_path) == []
    assert vm.validate_prompt_escalate_closures(tmp_path) == []
    assert vm.validate_prompt_qualtran_expertise_residual(tmp_path) == []
    assert vm.validate_prompt_vision_matrix_residual(tmp_path) == []
    ...
    (tmp_path / "schemas" / "packaging-inventory.json").write_text(
        json.dumps(
            _inventory_payload(
                ...
                + ["invented edge review metrics"],
                prompt_orch_delivery_metrics_required_phrases=list(
                    vm.PROMPT_ORCH_DELIVERY_METRICS_REQUIRED_PHRASES
                )[:-1]
                + ["invented orch delivery metrics"],
                prompt_quantum_comms_residual_required_phrases=list(
                    vm.PROMPT_QUANTUM_COMMS_RESIDUAL_REQUIRED_PHRASES
                )[:-1]
                + ["invented quantum comms residual"],
                prompt_bc_edge_comms_required_phrases=list(
                    vm.PROMPT_BC_EDGE_COMMS_REQUIRED_PHRASES
                )[:-1]
                + ["invented bc edge comms"],
                prompt_orch_comms_residual_required_phrases=list(
                    vm.PROMPT_ORCH_COMMS_RESIDUAL_REQUIRED_PHRASES
                )[:-1]
                + ["invented orch comms residual"],
                prompt_quantum_principles_residual_required_phrases=list(
                    vm.PROMPT_QUANTUM_PRINCIPLES_RESIDUAL_REQUIRED_PHRASES
                )[:-1]
                + ["invented quantum principles residual"],
                prompt_bc_edge_principles_required_phrases=list(
                    vm.PROMPT_BC_EDGE_PRINCIPLES_REQUIRED_PHRASES
                )[:-1]
                + ["invented bc edge principles"],
                prompt_escalate_closures_required_phrases=list(
                    vm.PROMPT_ESCALATE_CLOSURES_REQUIRED_PHRASES
                )[:-1]
                + ["invented escalate closures"],
                prompt_qualtran_expertise_residual_required_phrases=list(
                    vm.PROMPT_QUALTRAN_EXPERTISE_RESIDUAL_REQUIRED_PHRASES
                )[:-1]
                + ["invented qualtran expertise residual"],
                prompt_vision_matrix_residual_required_phrases=list(
                    vm.PROMPT_VISION_MATRIX_RESIDUAL_REQUIRED_PHRASES
                )[:-1]
                + ["invented vision matrix residual"],
            )
        ),
        encoding="utf-8",
    )
...
def test_v52_inventory_lock_mismatch_and_consistency_matrix(tmp_path: Path) -> None:
    """Large self-test: every v52 B residual phrase key empty/dup/blank/seed + live registry."""
    assert vm.INVENTORY_VERSION == 52
    assert vm.MIN_VALIDATOR_COUNT == 196
    assert len(vm.VALIDATORS) == 196
    assert sorted(vm.VALIDATORS) == json.loads(
    ...
    for key, phrases, include_token in keys:
        ...
        payload = _inventory_payload()
        payload[key] = [phrases[0]] if include_token else ["seed only"]
        # seed that is missing required include set when truncated wrongly:
        payload[key] = ["unrelated seed phrase"]
        findings = vm._inventory_lock_consistency(
            payload, schema_path="schemas/packaging-inventory.json"
        )
        assert any(f"{key} must include" in f.message for f in findings)
