...
def _inventory_payload(**overrides: object) -> dict:
    payload = {
        ...
        "constitution_specialists_overview_required_phrases": list(
            vm.CONSTITUTION_SPECIALISTS_OVERVIEW_REQUIRED_PHRASES
        ),
        "constitution_quantum_profile_required_phrases": list(
            vm.CONSTITUTION_QUANTUM_PROFILE_REQUIRED_PHRASES
        ),
        "constitution_blockchain_profile_required_phrases": list(
            vm.CONSTITUTION_BLOCKCHAIN_PROFILE_REQUIRED_PHRASES
        ),
        "constitution_edge_profile_required_phrases": list(
            vm.CONSTITUTION_EDGE_PROFILE_REQUIRED_PHRASES
        ),
        "constitution_orch_profile_required_phrases": list(
            vm.CONSTITUTION_ORCH_PROFILE_REQUIRED_PHRASES
        ),
        "constitution_crypto_residual_required_phrases": list(
            vm.CONSTITUTION_CRYPTO_RESIDUAL_REQUIRED_PHRASES
        ),
        "constitution_on_device_residual_required_phrases": list(
            vm.CONSTITUTION_ON_DEVICE_RESIDUAL_REQUIRED_PHRASES
        ),
        "constitution_multichain_residual_required_phrases": list(
            vm.CONSTITUTION_MULTICHAIN_RESIDUAL_REQUIRED_PHRASES
        ),
        "constitution_handoff_residual_required_phrases": list(
            vm.CONSTITUTION_HANDOFF_RESIDUAL_REQUIRED_PHRASES
        ),
        "constitution_recipe_yaml_residual_required_phrases": list(
            vm.CONSTITUTION_RECIPE_YAML_RESIDUAL_REQUIRED_PHRASES
        ),
        "constitution_install_residual_required_phrases": list(
            vm.CONSTITUTION_INSTALL_RESIDUAL_REQUIRED_PHRASES
        ),
        "constitution_risk_residual_required_phrases": list(
            vm.CONSTITUTION_RISK_RESIDUAL_REQUIRED_PHRASES
        ),
...
def test_validators_registry_covers_all_checks() -> None:
    expected = {
        ...
        "constitution-blockchain-profile",
        "constitution-edge-profile",
        "constitution-orch-profile",
        "constitution-crypto-residual",
        "constitution-on-device-residual",
        "constitution-multichain-residual",
        "constitution-handoff-residual",
        "constitution-recipe-yaml-residual",
        "constitution-install-residual",
        "constitution-risk-residual",
        "changelog-preamble",
        "changelog-changed",
        "changelog-initial",
        "prompt-expertise",
...
def test_live_v53_constitution_leftover_validators() -> None:
    assert vm.validate_constitution_specialists_overview(REPO_ROOT) == []
    assert vm.validate_constitution_quantum_profile(REPO_ROOT) == []
    assert vm.validate_constitution_blockchain_profile(REPO_ROOT) == []
    assert vm.validate_constitution_edge_profile(REPO_ROOT) == []
    assert vm.validate_constitution_orch_profile(REPO_ROOT) == []
    assert vm.validate_constitution_crypto_residual(REPO_ROOT) == []
    assert vm.validate_constitution_on_device_residual(REPO_ROOT) == []
    assert vm.validate_constitution_multichain_residual(REPO_ROOT) == []
    assert vm.validate_constitution_handoff_residual(REPO_ROOT) == []
    assert vm.validate_constitution_recipe_yaml_residual(REPO_ROOT) == []
    assert vm.validate_constitution_install_residual(REPO_ROOT) == []
    assert vm.validate_constitution_risk_residual(REPO_ROOT) == []
    assert vm.INVENTORY_VERSION == 53
    assert len(vm.VALIDATORS) == 208
    assert vm.MIN_VALIDATOR_COUNT == 208
    for name in [
    ...
    assert inventory["version"] == 53
    assert inventory["min_validator_count"] == 208
    assert inventory["constitution_specialists_overview_required_phrases"] == list(
        vm.CONSTITUTION_SPECIALISTS_OVERVIEW_REQUIRED_PHRASES
    )
    assert inventory["constitution_quantum_profile_required_phrases"] == list(
        vm.CONSTITUTION_QUANTUM_PROFILE_REQUIRED_PHRASES
    )
    assert inventory["constitution_blockchain_profile_required_phrases"] == list(
        vm.CONSTITUTION_BLOCKCHAIN_PROFILE_REQUIRED_PHRASES
    )
    assert inventory["constitution_edge_profile_required_phrases"] == list(
        vm.CONSTITUTION_EDGE_PROFILE_REQUIRED_PHRASES
    )
    assert inventory["constitution_orch_profile_required_phrases"] == list(
        vm.CONSTITUTION_ORCH_PROFILE_REQUIRED_PHRASES
    )
    assert inventory["constitution_crypto_residual_required_phrases"] == list(
        vm.CONSTITUTION_CRYPTO_RESIDUAL_REQUIRED_PHRASES
    )
    assert inventory["constitution_on_device_residual_required_phrases"] == list(
        vm.CONSTITUTION_ON_DEVICE_RESIDUAL_REQUIRED_PHRASES
    )
    assert inventory["constitution_multichain_residual_required_phrases"] == list(
        vm.CONSTITUTION_MULTICHAIN_RESIDUAL_REQUIRED_PHRASES
    )
    assert inventory["constitution_handoff_residual_required_phrases"] == list(
        vm.CONSTITUTION_HANDOFF_RESIDUAL_REQUIRED_PHRASES
    )
    assert inventory["constitution_recipe_yaml_residual_required_phrases"] == list(
        vm.CONSTITUTION_RECIPE_YAML_RESIDUAL_REQUIRED_PHRASES
    )
    assert inventory["constitution_install_residual_required_phrases"] == list(
        vm.CONSTITUTION_INSTALL_RESIDUAL_REQUIRED_PHRASES
    )
    assert inventory["constitution_risk_residual_required_phrases"] == list(
        vm.CONSTITUTION_RISK_RESIDUAL_REQUIRED_PHRASES
    )
    assert sorted(vm.VALIDATORS) == inventory["validator_names"]
    # prior constitution leftovers remain green
    assert vm.validate_constitution_ide_stack(REPO_ROOT) == []
    assert vm.validate_constitution_install_script(REPO_ROOT) == []
    assert vm.validate_constitution_vscode(REPO_ROOT) == []
    assert vm.validate_constitution_hard_constraints(REPO_ROOT) == []
    ...
    assert vm.validate_constitution_handoff(REPO_ROOT) == []
    assert vm.validate_constitution_recipe_orchestration(REPO_ROOT) == []
    # prior prompt v52 modules remain green
    assert vm.validate_prompt_metrics_detail(REPO_ROOT) == []
    assert vm.validate_prompt_usage_detail(REPO_ROOT) == []
    assert "Packaging inventory v53" in (
        REPO_ROOT / "CONTRIBUTING.md"
    ...
    assert "v53 HEAVY" in (REPO_ROOT / "CHANGELOG.md").read_text(encoding="utf-8")


def test_v53_constitution_leftover_edge_cases(tmp_path: Path) -> None:
    for fn in [
        vm.validate_constitution_specialists_overview,
        vm.validate_constitution_quantum_profile,
        vm.validate_constitution_blockchain_profile,
        vm.validate_constitution_edge_profile,
        vm.validate_constitution_orch_profile,
        vm.validate_constitution_crypto_residual,
        vm.validate_constitution_on_device_residual,
        vm.validate_constitution_multichain_residual,
        vm.validate_constitution_handoff_residual,
        vm.validate_constitution_recipe_yaml_residual,
        vm.validate_constitution_install_residual,
        vm.validate_constitution_risk_residual,
    ]:
        assert any("missing" in f.message for f in fn(tmp_path))
    ...
    cases = [
        (
            vm.validate_constitution_specialists_overview,
            "missing specialists-overview 6.4.1 section lock",
            vm.CONSTITUTION_SPECIALISTS_OVERVIEW_REQUIRED_PHRASES,
        ),
        (
            vm.validate_constitution_quantum_profile,
            "missing quantum-profile Theorist heading lock",
            vm.CONSTITUTION_QUANTUM_PROFILE_REQUIRED_PHRASES,
        ),
        (
            vm.validate_constitution_blockchain_profile,
            "missing blockchain-profile System Designer heading lock",
            vm.CONSTITUTION_BLOCKCHAIN_PROFILE_REQUIRED_PHRASES,
        ),
        (
            vm.validate_constitution_edge_profile,
            "missing edge-profile Guardian heading lock",
            vm.CONSTITUTION_EDGE_PROFILE_REQUIRED_PHRASES,
        ),
        (
            vm.validate_constitution_orch_profile,
            "missing orch-profile Conductor heading lock",
            vm.CONSTITUTION_ORCH_PROFILE_REQUIRED_PHRASES,
        ),
        (
            vm.validate_constitution_crypto_residual,
            "missing crypto-residual Integration Standards heading lock",
            vm.CONSTITUTION_CRYPTO_RESIDUAL_REQUIRED_PHRASES,
        ),
        (
            vm.validate_constitution_on_device_residual,
            "missing on-device-residual mobile computation lock",
            vm.CONSTITUTION_ON_DEVICE_RESIDUAL_REQUIRED_PHRASES,
        ),
        (
            vm.validate_constitution_multichain_residual,
            "missing multichain-residual span multiple chains lock",
            vm.CONSTITUTION_MULTICHAIN_RESIDUAL_REQUIRED_PHRASES,
        ),
        (
            vm.validate_constitution_handoff_residual,
            "missing handoff-residual Coordination Protocol lock",
            vm.CONSTITUTION_HANDOFF_RESIDUAL_REQUIRED_PHRASES,
        ),
        (
            vm.validate_constitution_recipe_yaml_residual,
            "missing recipe-yaml-residual Orchestrate NFT mint lock",
            vm.CONSTITUTION_RECIPE_YAML_RESIDUAL_REQUIRED_PHRASES,
        ),
        (
            vm.validate_constitution_install_residual,
            "missing install-residual WSL2 Ubuntu lock",
            vm.CONSTITUTION_INSTALL_RESIDUAL_REQUIRED_PHRASES,
        ),
        (
            vm.validate_constitution_risk_residual,
            "missing risk-residual Section 12 Security Invariants lock",
            vm.CONSTITUTION_RISK_RESIDUAL_REQUIRED_PHRASES,
        ),
    ]
    for fn, section_msg, phrases in cases:
    ...
    assert vm.validate_constitution_specialists_overview(tmp_path) == []
    assert vm.validate_constitution_quantum_profile(tmp_path) == []
    assert vm.validate_constitution_blockchain_profile(tmp_path) == []
    assert vm.validate_constitution_edge_profile(tmp_path) == []
    assert vm.validate_constitution_orch_profile(tmp_path) == []
    assert vm.validate_constitution_crypto_residual(tmp_path) == []
    assert vm.validate_constitution_on_device_residual(tmp_path) == []
    assert vm.validate_constitution_multichain_residual(tmp_path) == []
    assert vm.validate_constitution_handoff_residual(tmp_path) == []
    assert vm.validate_constitution_recipe_yaml_residual(tmp_path) == []
    assert vm.validate_constitution_install_residual(tmp_path) == []
    assert vm.validate_constitution_risk_residual(tmp_path) == []
    ...
    assert vm.run_all_validations(REPO_ROOT) == []
