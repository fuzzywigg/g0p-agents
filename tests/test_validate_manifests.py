...
def _inventory_payload(**overrides: object) -> dict:
    payload = {
        ...
        "goose_blockchain_task_required_phrases": list(
            vm.GOOSE_BLOCKCHAIN_TASK_REQUIRED_PHRASES
        ),
        "goose_edge_task_required_phrases": list(vm.GOOSE_EDGE_TASK_REQUIRED_PHRASES),
        "goose_adding_recipes_required_phrases": list(
            vm.GOOSE_ADDING_RECIPES_REQUIRED_PHRASES
        ),
...
def test_live_v5_validators() -> None:
    assert vm.validate_bug_report_template(REPO_ROOT) == []
    assert vm.validate_feature_request_template(REPO_ROOT) == []
    assert vm.INVENTORY_VERSION == 43
    assert len(vm.VALIDATORS) == 144
...
def test_live_v8_validators() -> None:
    ...
    assert vm.INVENTORY_VERSION == 43
    assert len(vm.VALIDATORS) == 144
    assert vm.MIN_VALIDATOR_COUNT == 144
...
def test_live_v12_validators() -> None:
    ...
    assert inventory["min_validator_count"] == 144
...
def test_v38_contributing_deepener_edge_cases(tmp_path: Path) -> None:
    ...
    assert any(
        "missing Packaging inventory v43 honesty lock" in f.message for f in findings
    )
...
def test_live_v43_goose_recipes_leftover_validators() -> None:
    assert vm.validate_goose_blockchain_task(REPO_ROOT) == []
    assert vm.validate_goose_edge_task(REPO_ROOT) == []
    assert vm.validate_goose_adding_recipes(REPO_ROOT) == []
    assert vm.INVENTORY_VERSION == 43
    assert len(vm.VALIDATORS) == 144
    assert vm.MIN_VALIDATOR_COUNT == 144
    assert "goose-blockchain-task" in vm.VALIDATORS
    assert "goose-edge-task" in vm.VALIDATORS
    assert "goose-adding-recipes" in vm.VALIDATORS
    inventory = json.loads(
        (REPO_ROOT / "schemas" / "packaging-inventory.json").read_text(encoding="utf-8")
    )
    assert inventory["version"] == 43
    assert inventory["goose_blockchain_task_required_phrases"] == list(
        vm.GOOSE_BLOCKCHAIN_TASK_REQUIRED_PHRASES
    )
    assert inventory["goose_edge_task_required_phrases"] == list(
        vm.GOOSE_EDGE_TASK_REQUIRED_PHRASES
    )
    assert inventory["goose_adding_recipes_required_phrases"] == list(
        vm.GOOSE_ADDING_RECIPES_REQUIRED_PHRASES
    )
    assert inventory["min_validator_count"] == 144
    assert sorted(vm.VALIDATORS) == inventory["validator_names"]
...
def test_v43_goose_recipes_leftover_edge_cases(tmp_path: Path) -> None:
    assert any(
        "missing" in f.message for f in vm.validate_goose_blockchain_task(tmp_path)
    )
    assert any("missing" in f.message for f in vm.validate_goose_edge_task(tmp_path))
    assert any(
        "missing" in f.message for f in vm.validate_goose_adding_recipes(tmp_path)
    )

    _write(tmp_path / "GOOSE-RECIPES.md", "# Goose\n\nNo locked content.\n")
    findings = vm.validate_goose_blockchain_task(tmp_path)
    assert any(
        "missing Smart Contract Design Task section" in f.message for f in findings
    )
    for phrase in vm.GOOSE_BLOCKCHAIN_TASK_REQUIRED_PHRASES:
        assert any(phrase in f.message for f in findings)

    findings = vm.validate_goose_edge_task(tmp_path)
    assert any(
        "missing On-Device Security Implementation Task section" in f.message
        for f in findings
    )
    for phrase in vm.GOOSE_EDGE_TASK_REQUIRED_PHRASES:
        assert any(phrase in f.message for f in findings)

    findings = vm.validate_goose_adding_recipes(tmp_path)
    assert any("missing Adding New Recipes section" in f.message for f in findings)
    for phrase in vm.GOOSE_ADDING_RECIPES_REQUIRED_PHRASES:
        assert any(phrase in f.message for f in findings)
    ...
    assert vm.validate_goose_blockchain_task(tmp_path) == []
    assert vm.validate_goose_edge_task(tmp_path) == []
    assert vm.validate_goose_adding_recipes(tmp_path) == []

    empty = _inventory_payload()
    empty["goose_blockchain_task_required_phrases"] = []
    findings = vm._inventory_lock_consistency(
        empty, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "goose_blockchain_task_required_phrases must not be empty" in f.message
        for f in findings
    )

    dup = _inventory_payload()
    dup["goose_blockchain_task_required_phrases"] = [
        vm.GOOSE_BLOCKCHAIN_TASK_REQUIRED_PHRASES[0],
        vm.GOOSE_BLOCKCHAIN_TASK_REQUIRED_PHRASES[0],
        *vm.GOOSE_BLOCKCHAIN_TASK_REQUIRED_PHRASES[1:],
    ]
    findings = vm._inventory_lock_consistency(
        dup, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "goose_blockchain_task_required_phrases must be unique" in f.message
        for f in findings
    )

    blank = _inventory_payload()
    blank["goose_blockchain_task_required_phrases"] = ["ok", "  "]
    findings = vm._inventory_lock_consistency(
        blank, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "goose_blockchain_task_required_phrases entries must be non-empty strings"
        in f.message
        for f in findings
    )

    missing = _inventory_payload()
    missing["goose_blockchain_task_required_phrases"] = [
    ...
    assert any(
        "goose_blockchain_task_required_phrases must include Smart" in f.message
        for f in findings
    )

    empty_e = _inventory_payload()
    empty_e["goose_edge_task_required_phrases"] = []
    findings = vm._inventory_lock_consistency(
        empty_e, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "goose_edge_task_required_phrases must not be empty" in f.message
        for f in findings
    )

    dup_e = _inventory_payload()
    dup_e["goose_edge_task_required_phrases"] = [
        vm.GOOSE_EDGE_TASK_REQUIRED_PHRASES[0],
        vm.GOOSE_EDGE_TASK_REQUIRED_PHRASES[0],
        *vm.GOOSE_EDGE_TASK_REQUIRED_PHRASES[1:],
    ]
    findings = vm._inventory_lock_consistency(
        dup_e, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "goose_edge_task_required_phrases must be unique" in f.message for f in findings
    )

    blank_e = _inventory_payload()
    blank_e["goose_edge_task_required_phrases"] = ["ok", "  "]
    findings = vm._inventory_lock_consistency(
        blank_e, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "goose_edge_task_required_phrases entries must be non-empty strings"
        in f.message
        for f in findings
    )

    missing_e = _inventory_payload()
    missing_e["goose_edge_task_required_phrases"] = [
    ...
    assert any(
        "goose_edge_task_required_phrases must include On-Device" in f.message
        for f in findings
    )

    empty_a = _inventory_payload()
    empty_a["goose_adding_recipes_required_phrases"] = []
    findings = vm._inventory_lock_consistency(
        empty_a, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "goose_adding_recipes_required_phrases must not be empty" in f.message
        for f in findings
    )

    dup_a = _inventory_payload()
    dup_a["goose_adding_recipes_required_phrases"] = [
        vm.GOOSE_ADDING_RECIPES_REQUIRED_PHRASES[0],
        vm.GOOSE_ADDING_RECIPES_REQUIRED_PHRASES[0],
        *vm.GOOSE_ADDING_RECIPES_REQUIRED_PHRASES[1:],
    ]
    findings = vm._inventory_lock_consistency(
        dup_a, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "goose_adding_recipes_required_phrases must be unique" in f.message
        for f in findings
    )

    blank_a = _inventory_payload()
    blank_a["goose_adding_recipes_required_phrases"] = ["ok", "  "]
    findings = vm._inventory_lock_consistency(
        blank_a, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "goose_adding_recipes_required_phrases entries must be non-empty strings"
        in f.message
        for f in findings
    )

    missing_a = _inventory_payload()
    missing_a["goose_adding_recipes_required_phrases"] = ["## Adding New Recipes"]
    findings = vm._inventory_lock_consistency(
        missing_a, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "goose_adding_recipes_required_phrases must include Adding" in f.message
        for f in findings
    )
    ...
    (tmp_path / "schemas" / "packaging-inventory.json").write_text(
        json.dumps(
            _inventory_payload(
                ...
                + ["invented blockchain"],
                goose_edge_task_required_phrases=list(
                    vm.GOOSE_EDGE_TASK_REQUIRED_PHRASES
                )[:-1]
                + ["invented edge"],
                goose_adding_recipes_required_phrases=list(
    ...
    findings = vm.validate_packaging_inventory(tmp_path)
    assert any("goose_blockchain_task_required_phrases" in f.message for f in findings)
    assert any("goose_edge_task_required_phrases" in f.message for f in findings)
    assert any("goose_adding_recipes_required_phrases" in f.message for f in findings)
