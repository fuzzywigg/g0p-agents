...
def _inventory_payload(**overrides: object) -> dict:
    payload = {
        ...
        "goose_timeout_detail_required_phrases": list(vm.GOOSE_TIMEOUT_DETAIL_REQUIRED_PHRASES),
        "goose_deadline_detail_required_phrases": list(vm.GOOSE_DEADLINE_DETAIL_REQUIRED_PHRASES),
        "constitution_deadline_detail_required_phrases": list(
            vm.CONSTITUTION_DEADLINE_DETAIL_REQUIRED_PHRASES
        ),
        "implementation_timeout_detail_required_phrases": list(
            vm.IMPLEMENTATION_TIMEOUT_DETAIL_REQUIRED_PHRASES
        ),
...
def test_live_v53_orchestration_timeout_validators() -> None:
    assert vm.validate_goose_timeout_detail(REPO_ROOT) == []
    assert vm.validate_goose_deadline_detail(REPO_ROOT) == []
    assert vm.validate_constitution_deadline_detail(REPO_ROOT) == []
    assert vm.validate_implementation_timeout_detail(REPO_ROOT) == []
    assert vm.INVENTORY_VERSION == 53
    assert len(vm.VALIDATORS) == 200
    assert vm.MIN_VALIDATOR_COUNT == 200
    for name in [
    ...
    assert inventory["version"] == 53
    assert inventory["min_validator_count"] == 200
    assert inventory["goose_timeout_detail_required_phrases"] == list(
        vm.GOOSE_TIMEOUT_DETAIL_REQUIRED_PHRASES
    )
    assert inventory["goose_deadline_detail_required_phrases"] == list(
        vm.GOOSE_DEADLINE_DETAIL_REQUIRED_PHRASES
    )
    assert inventory["constitution_deadline_detail_required_phrases"] == list(
        vm.CONSTITUTION_DEADLINE_DETAIL_REQUIRED_PHRASES
    )
    assert inventory["implementation_timeout_detail_required_phrases"] == list(
        vm.IMPLEMENTATION_TIMEOUT_DETAIL_REQUIRED_PHRASES
    )
    assert sorted(vm.VALIDATORS) == inventory["validator_names"]
    assert vm.validate_goose_extensions(REPO_ROOT) == []
    ...
    assert "inventory v53 locks" in (REPO_ROOT / "README.md").read_text(encoding="utf-8")


def test_v53_orchestration_timeout_edge_cases(tmp_path: Path) -> None:
    for fn in [
        vm.validate_goose_timeout_detail,
        vm.validate_goose_deadline_detail,
        vm.validate_constitution_deadline_detail,
        vm.validate_implementation_timeout_detail,
    ]:
        assert any("missing" in f.message for f in fn(tmp_path))
    ...
    findings = vm.validate_goose_timeout_detail(tmp_path)
    assert any("Long timeout for multi-step orchestration" in f.message for f in findings)
    for phrase in vm.GOOSE_TIMEOUT_DETAIL_REQUIRED_PHRASES:
        assert any(phrase in f.message for f in findings)

    findings = vm.validate_goose_deadline_detail(tmp_path)
    assert any("Deadline inheritance placeholder" in f.message for f in findings)
    for phrase in vm.GOOSE_DEADLINE_DETAIL_REQUIRED_PHRASES:
        assert any(phrase in f.message for f in findings)

    findings = vm.validate_constitution_deadline_detail(tmp_path)
    assert any("Nested checkboxes deadline-cancel" in f.message for f in findings)
    for phrase in vm.CONSTITUTION_DEADLINE_DETAIL_REQUIRED_PHRASES:
        assert any(phrase in f.message for f in findings)

    findings = vm.validate_implementation_timeout_detail(tmp_path)
    assert any("Adjust timeouts hardware" in f.message for f in findings)
    for phrase in vm.IMPLEMENTATION_TIMEOUT_DETAIL_REQUIRED_PHRASES:
        assert any(phrase in f.message for f in findings)
    ...
    _write(
        tmp_path / "AGENTS-v2.2.md",
        "\n".join([*vm.CONSTITUTION_DEADLINE_DETAIL_REQUIRED_PHRASES, ""]),
    )
    _write(
        tmp_path / "IMPLEMENTATION-GUIDE.md",
        "\n".join([*vm.IMPLEMENTATION_TIMEOUT_DETAIL_REQUIRED_PHRASES, ""]),
    )
    assert vm.validate_goose_timeout_detail(tmp_path) == []
    assert vm.validate_goose_deadline_detail(tmp_path) == []
    assert vm.validate_constitution_deadline_detail(tmp_path) == []
    assert vm.validate_implementation_timeout_detail(tmp_path) == []
    ...
    for key, phrases, include_token in [
        ...
        assert any(f"{key} entries must be non-empty strings" in f.message for f in findings)
...
@pytest.mark.parametrize("timeout_value", [0, -1, 0.5, None, "0", ""])
def test_goose_recipe_schema_zero_timeout_stubs_already_handled(timeout_value: object) -> None:
    schema = vm.load_schema("goose-recipe.schema.json")
    bad = json.loads(json.dumps(MINIMAL_RECIPE))
...
def test_concurrent_abort_cancel_in_progress_deepen(tmp_path: Path) -> None:
    ...
    findings = vm.validate_ci_workflow(tmp_path)
    assert any(
        "concurrency.cancel-in-progress must be" in f.message for f in findings
    )


def test_v53_orchestration_timeout_large_consistency_matrix() -> None:
    assert vm.INVENTORY_VERSION == 53
    assert len(vm.VALIDATORS) == 200
    assert vm.MIN_VALIDATOR_COUNT == 200
    inventory = json.loads(
