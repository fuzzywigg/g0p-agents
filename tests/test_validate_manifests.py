...
def _inventory_payload(**overrides: object) -> dict:
    payload = {
        ...
        "prompt_escalate_when_required_phrases": list(
            vm.PROMPT_ESCALATE_WHEN_REQUIRED_PHRASES
        ),
        "prompt_qualtran_tools_required_phrases": list(
            vm.PROMPT_QUALTRAN_TOOLS_REQUIRED_PHRASES
        ),
        "prompt_interpolation_required_phrases": list(
            vm.PROMPT_INTERPOLATION_REQUIRED_PHRASES
        ),
        "prompt_checklist_detail_required_phrases": list(
            vm.PROMPT_CHECKLIST_DETAIL_REQUIRED_PHRASES
        ),
        "prompt_postmortem_refs_required_phrases": list(
            vm.PROMPT_POSTMORTEM_REFS_REQUIRED_PHRASES
        ),
...
def test_live_v8_validators() -> None:
    ...
    assert vm.INVENTORY_VERSION == 52
    assert len(vm.VALIDATORS) == 192
    assert vm.MIN_VALIDATOR_COUNT == 192
...
def test_live_v12_validators() -> None:
    ...
    assert inventory["min_validator_count"] == 192
...
def test_live_v52_prompt_template_locks_validators() -> None:
    ...
    assert vm.validate_prompt_escalate_when(REPO_ROOT) == []
    assert vm.validate_prompt_qualtran_tools(REPO_ROOT) == []
    assert vm.validate_prompt_interpolation(REPO_ROOT) == []
    assert vm.validate_prompt_checklist_detail(REPO_ROOT) == []
    assert vm.validate_prompt_postmortem_refs(REPO_ROOT) == []
    assert vm.INVENTORY_VERSION == 52
    assert len(vm.VALIDATORS) == 192
    assert vm.MIN_VALIDATOR_COUNT == 192
    for name in [
    ...
    assert inventory["prompt_escalate_when_required_phrases"] == list(
        vm.PROMPT_ESCALATE_WHEN_REQUIRED_PHRASES
    )
    assert inventory["prompt_qualtran_tools_required_phrases"] == list(
        vm.PROMPT_QUALTRAN_TOOLS_REQUIRED_PHRASES
    )
    assert inventory["prompt_interpolation_required_phrases"] == list(
        vm.PROMPT_INTERPOLATION_REQUIRED_PHRASES
    )
    assert inventory["prompt_checklist_detail_required_phrases"] == list(
        vm.PROMPT_CHECKLIST_DETAIL_REQUIRED_PHRASES
    )
    assert inventory["prompt_postmortem_refs_required_phrases"] == list(
        vm.PROMPT_POSTMORTEM_REFS_REQUIRED_PHRASES
    )
    assert sorted(vm.VALIDATORS) == inventory["validator_names"]
    # prior prompt leftovers remain green
...
def test_v52_prompt_template_locks_edge_cases(tmp_path: Path) -> None:
    for fn in [
        vm.validate_prompt_metrics_detail,
        vm.validate_prompt_principles_detail,
        vm.validate_prompt_communication_detail,
        vm.validate_prompt_escalate_when,
        vm.validate_prompt_qualtran_tools,
        vm.validate_prompt_interpolation,
        vm.validate_prompt_checklist_detail,
        vm.validate_prompt_postmortem_refs,
    ]:
        assert any("missing" in f.message for f in fn(tmp_path))
    ...
    cases = [
        ...
        (
            vm.validate_prompt_escalate_when,
            "missing escalate-when constraints cannot be met lock",
            vm.PROMPT_ESCALATE_WHEN_REQUIRED_PHRASES,
        ),
        (
            vm.validate_prompt_qualtran_tools,
            "missing qualtran-tools Qualtran resource analysis lock",
            vm.PROMPT_QUALTRAN_TOOLS_REQUIRED_PHRASES,
        ),
        (
            vm.validate_prompt_interpolation,
            "missing interpolation Error tolerance lock",
            vm.PROMPT_INTERPOLATION_REQUIRED_PHRASES,
        ),
        (
            vm.validate_prompt_checklist_detail,
            "missing checklist-detail Update risk register lock",
            vm.PROMPT_CHECKLIST_DETAIL_REQUIRED_PHRASES,
        ),
        (
            vm.validate_prompt_postmortem_refs,
            "missing postmortem-refs See postmortem.md lock",
            vm.PROMPT_POSTMORTEM_REFS_REQUIRED_PHRASES,
        ),
    ]
    for fn, section_msg, phrases in cases:
    ...
    assert vm.validate_prompt_escalate_when(tmp_path) == []
    assert vm.validate_prompt_qualtran_tools(tmp_path) == []
    assert vm.validate_prompt_interpolation(tmp_path) == []
    assert vm.validate_prompt_checklist_detail(tmp_path) == []
    assert vm.validate_prompt_postmortem_refs(tmp_path) == []
    ...
    (tmp_path / "schemas" / "packaging-inventory.json").write_text(
        json.dumps(
            _inventory_payload(
                ...
                + ["invented metrics detail"],
                prompt_principles_detail_required_phrases=list(
                    vm.PROMPT_PRINCIPLES_DETAIL_REQUIRED_PHRASES
                )[:-1]
                + ["invented principles detail"],
                prompt_communication_detail_required_phrases=list(
                    vm.PROMPT_COMMUNICATION_DETAIL_REQUIRED_PHRASES
                )[:-1]
                + ["invented communication detail"],
                prompt_escalate_when_required_phrases=list(
                    vm.PROMPT_ESCALATE_WHEN_REQUIRED_PHRASES
                )[:-1]
                + ["invented escalate when"],
                prompt_qualtran_tools_required_phrases=list(
                    vm.PROMPT_QUALTRAN_TOOLS_REQUIRED_PHRASES
                )[:-1]
                + ["invented qualtran tools"],
                prompt_interpolation_required_phrases=list(
                    vm.PROMPT_INTERPOLATION_REQUIRED_PHRASES
                )[:-1]
                ...
                + ["invented checklist detail"],
                prompt_postmortem_refs_required_phrases=list(
                    vm.PROMPT_POSTMORTEM_REFS_REQUIRED_PHRASES
                )[:-1]
                + ["invented postmortem refs"],
            )
        ),
        encoding="utf-8",
    )
...
def test_v52_inventory_lock_mismatch_and_consistency_matrix(tmp_path: Path) -> None:
    """Large self-test: every v52 template-lock phrase key empty/dup/blank/seed + live registry."""
    assert vm.INVENTORY_VERSION == 52
    assert vm.MIN_VALIDATOR_COUNT == 192
    assert len(vm.VALIDATORS) == 192
    assert sorted(vm.VALIDATORS) == json.loads(
    ...
    keys = [
        (
            "prompt_metrics_detail_required_phrases",
            vm.PROMPT_METRICS_DETAIL_REQUIRED_PHRASES,
            "Gate count/NIST/Hardware estimates/Multi-chain sync/Battery drain/Apple/Google PASS",
        ),
        (
            "prompt_principles_detail_required_phrases",
            vm.PROMPT_PRINCIPLES_DETAIL_REQUIRED_PHRASES,
        ...
        (
            "prompt_communication_detail_required_phrases",
            vm.PROMPT_COMMUNICATION_DETAIL_REQUIRED_PHRASES,
            "Conservative on claims/Flag quantum/architecture diagrams/trade-off reasoning",
        ),
        (
            "prompt_escalate_when_required_phrases",
            vm.PROMPT_ESCALATE_WHEN_REQUIRED_PHRASES,
            "Escalate when constraints/security or performance/device constraints",
        ),
        (
            "prompt_qualtran_tools_required_phrases",
            vm.PROMPT_QUALTRAN_TOOLS_REQUIRED_PHRASES,
        ...
        (
            "prompt_interpolation_required_phrases",
            vm.PROMPT_INTERPOLATION_REQUIRED_PHRASES,
            "Error tolerance/Gas budget/Deadline/Current Project Vision/Timeline/Budget",
        ),
        (
            "prompt_checklist_detail_required_phrases",
            vm.PROMPT_CHECKLIST_DETAIL_REQUIRED_PHRASES,
