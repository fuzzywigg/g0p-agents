...
def _inventory_payload(**overrides: object) -> dict:
    payload = {
        ...
        "prompt_human_escalation_required_phrases": list(
            vm.PROMPT_HUMAN_ESCALATION_REQUIRED_PHRASES
        ),
        ...
        ("markdownlint_default", False),
            ...
            (
                ...
                (
                    ...
                    list(vm.PROMPT_HUMAN_ESCALATION_REQUIRED_PHRASES)[:-1]
                    + ["invented"],
                ),
                (
                    "prompt_role_blurbs_required_phrases",
                    list(vm.PROMPT_ROLE_BLURBS_REQUIRED_PHRASES)[:-1]
                    + ["invented"],
                ),
                (
                    "prompt_escalation_format_required_phrases",
                    list(vm.PROMPT_ESCALATION_FORMAT_REQUIRED_PHRASES)[:-1]
                    + ["invented"],
                ),
                (
                    "prompt_living_docs_required_phrases",
                    list(vm.PROMPT_LIVING_DOCS_REQUIRED_PHRASES)[:-1]
                    + ["invented"],
                ),
                (
...
def test_live_v8_validators() -> None:
    ...
    assert vm.INVENTORY_VERSION == 51
    assert len(vm.VALIDATORS) == 175
    assert vm.MIN_VALIDATOR_COUNT == 175
...
def test_live_v12_validators() -> None:
    ...
    assert inventory["min_validator_count"] == 175
...
def test_live_v48_prompt_matrix_monthly_usage_example_validators() -> None:
...
def test_v48_prompt_matrix_monthly_usage_example_edge_cases(tmp_path: Path) -> None:
    for fn in [
    ...
    assert any("prompt_usage_example_required_phrases" in f.message for f in findings)
...
def test_live_v49_prompt_context_decision_escalation_authority_validators() -> None:
...
def test_v49_prompt_context_decision_escalation_authority_edge_cases(
    tmp_path: Path,
) -> None:
    for fn in [
...
def test_live_v50_prompt_responsibilities_cannot_delegate_human_escalation_validators() -> None:
    assert vm.validate_prompt_responsibilities(REPO_ROOT) == []
    assert vm.validate_prompt_cannot_delegate(REPO_ROOT) == []
    assert vm.validate_prompt_human_escalation(REPO_ROOT) == []
    assert vm.INVENTORY_VERSION == 51
    assert len(vm.VALIDATORS) == 175
    assert vm.MIN_VALIDATOR_COUNT == 175
    for name in [
    ...
    assert inventory["prompt_human_escalation_required_phrases"] == list(
        vm.PROMPT_HUMAN_ESCALATION_REQUIRED_PHRASES
    )
    assert sorted(vm.VALIDATORS) == inventory["validator_names"]
    assert vm.validate_prompt_context(REPO_ROOT) == []
...
def test_v50_prompt_responsibilities_cannot_delegate_human_escalation_edge_cases(
    tmp_path: Path,
) -> None:
    for fn in [
        vm.validate_prompt_responsibilities,
        vm.validate_prompt_cannot_delegate,
        vm.validate_prompt_human_escalation,
    ]:
        assert any("missing" in f.message for f in fn(tmp_path))
    ...
    cases = [
        ...
        (
            vm.validate_prompt_human_escalation,
            "missing When You Escalate to Human section",
            vm.PROMPT_HUMAN_ESCALATION_REQUIRED_PHRASES,
        ),
    ]
    for fn, section_msg, phrases in cases:
    ...
    assert vm.validate_prompt_human_escalation(tmp_path) == []
    ...
    (tmp_path / "schemas" / "packaging-inventory.json").write_text(
        json.dumps(
            _inventory_payload(
                ...
                + ["invented human escalation"],
            )
        ),
        encoding="utf-8",
    )
    findings = vm.validate_packaging_inventory(tmp_path)
    assert any("prompt_responsibilities_required_phrases" in f.message for f in findings)
    assert any(
        "prompt_cannot_delegate_required_phrases" in f.message for f in findings
    )
    assert any(
        "prompt_human_escalation_required_phrases" in f.message for f in findings
    )
...
def test_v50_inventory_lock_mismatch_and_consistency_matrix(tmp_path: Path) -> None:
    ...
    keys = [
        (
            "prompt_responsibilities_required_phrases",
            vm.PROMPT_RESPONSIBILITIES_REQUIRED_PHRASES,
            "Core Responsibilities/quantum algorithms/on-device/go/no-go",
        ),
        (
            "prompt_cannot_delegate_required_phrases",
            vm.PROMPT_CANNOT_DELEGATE_REQUIRED_PHRASES,
...
def test_live_v51_prompt_role_blurbs_escalation_format_living_docs_validators() -> None:
...
def test_v51_prompt_role_blurbs_escalation_format_living_docs_edge_cases(
    tmp_path: Path,
) -> None:
    for fn in [
    ...
    cases = [
        (
            vm.validate_prompt_role_blurbs,
            "missing QuantumArchitectAgent System Prompt",
            vm.PROMPT_ROLE_BLURBS_REQUIRED_PHRASES,
        ),
        (
            vm.validate_prompt_escalation_format,
        ...
        (
            vm.validate_prompt_living_docs,
            "missing Agent Prompt Templates heading",
            vm.PROMPT_LIVING_DOCS_REQUIRED_PHRASES,
        ),
    ]
    for fn, section_msg, phrases in cases:
    ...
    findings = vm.validate_packaging_inventory(tmp_path)
    assert any("prompt_role_blurbs_required_phrases" in f.message for f in findings)
    assert any(
        "prompt_escalation_format_required_phrases" in f.message for f in findings
    )
    assert any("prompt_living_docs_required_phrases" in f.message for f in findings)


def test_v51_inventory_lock_mismatch_and_consistency_matrix(tmp_path: Path) -> None:
    """Large self-test: every v51 phrase key empty/dup/blank/seed + live registry."""
    assert vm.INVENTORY_VERSION == 51
    assert vm.MIN_VALIDATOR_COUNT == 175
    assert len(vm.VALIDATORS) == 175
    assert sorted(vm.VALIDATORS) == json.loads(
    ...
    keys = [
        (
            "prompt_role_blurbs_required_phrases",
            vm.PROMPT_ROLE_BLURBS_REQUIRED_PHRASES,
            "quantum circuits/orchestrate specialists/System Prompt",
        ),
        (
            "prompt_escalation_format_required_phrases",
            vm.PROMPT_ESCALATION_FORMAT_REQUIRED_PHRASES,
        ...
        (
            "prompt_living_docs_required_phrases",
            vm.PROMPT_LIVING_DOCS_REQUIRED_PHRASES,
            "Agent Prompt Templates/living documents/AGENTS.md constraints/Task.md",
        ),
    ]
    for key, phrases, include_token in keys:
        payload = _inventory_payload()
