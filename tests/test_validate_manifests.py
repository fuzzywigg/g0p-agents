...
def _inventory_payload(**overrides: object) -> dict:
    payload = {
        ...
        "pr_changes_required_phrases": list(vm.PR_CHANGES_REQUIRED_PHRASES),
...
def test_live_v5_validators() -> None:
    assert vm.validate_bug_report_template(REPO_ROOT) == []
    assert vm.validate_feature_request_template(REPO_ROOT) == []
    assert vm.INVENTORY_VERSION == 23
    assert len(vm.VALIDATORS) == 82
...
def test_live_v8_validators() -> None:
    ...
    assert vm.INVENTORY_VERSION == 23
    assert len(vm.VALIDATORS) == 82
    assert vm.MIN_VALIDATOR_COUNT == 82
...
def test_live_v12_validators() -> None:
    ...
    assert inventory["min_validator_count"] == 82
...
def test_live_v23_pr_template_validators() -> None:
    ...
    assert vm.validate_pr_changes(REPO_ROOT) == []
    ...
    assert tuple(vm.PR_CHANGES_REQUIRED_PHRASES) == (
        "## Changes",
    ...
    assert inventory["pr_changes_required_phrases"] == list(vm.PR_CHANGES_REQUIRED_PHRASES)
    ...
    assert "pr-changes" in vm.VALIDATORS
...
def test_v23_pr_template_edge_cases(tmp_path: Path) -> None:
    ...
    assert any("missing" in f.message for f in vm.validate_pr_changes(tmp_path))
    ...
    assert any("missing Summary heading" in f.message for f in findings)
    assert any("missing Problem section" in f.message for f in findings)
    for phrase in vm.PR_SUMMARY_REQUIRED_PHRASES:
        assert any(phrase in f.message for f in findings)

    findings = vm.validate_pr_changes(tmp_path)
    assert any("missing Changes section" in f.message for f in findings)
    for phrase in vm.PR_CHANGES_REQUIRED_PHRASES:
        assert any(phrase in f.message for f in findings)
    ...
    assert vm.validate_pr_changes(tmp_path) == []
    ...
    empty_changes = dict(inventory)
    empty_changes["pr_changes_required_phrases"] = []
    findings = vm._inventory_lock_consistency(
        empty_changes, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "pr_changes_required_phrases must not be empty" in f.message for f in findings
    )

    dup_changes = dict(inventory)
    dup_changes["pr_changes_required_phrases"] = [
        vm.PR_CHANGES_REQUIRED_PHRASES[0],
        vm.PR_CHANGES_REQUIRED_PHRASES[0],
        *vm.PR_CHANGES_REQUIRED_PHRASES[1:],
    ]
    findings = vm._inventory_lock_consistency(
        dup_changes, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "pr_changes_required_phrases must be unique" in f.message for f in findings
    )

    blank_changes = dict(inventory)
    blank_changes["pr_changes_required_phrases"] = ["ok", "  "]
    findings = vm._inventory_lock_consistency(
        blank_changes, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "pr_changes_required_phrases entries must be non-empty strings" in f.message
        for f in findings
    )

    missing_changes = dict(inventory)
    missing_changes["pr_changes_required_phrases"] = ["## Changes"]
    findings = vm._inventory_lock_consistency(
        missing_changes, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "pr_changes_required_phrases must include Changes heading" in f.message
        for f in findings
    )

    empty_acceptance = dict(inventory)
    empty_acceptance["pr_acceptance_required_phrases"] = []
    findings = vm._inventory_lock_consistency(
        empty_acceptance, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "pr_acceptance_required_phrases must not be empty" in f.message for f in findings
    )

    dup_acceptance = dict(inventory)
    dup_acceptance["pr_acceptance_required_phrases"] = [
        vm.PR_ACCEPTANCE_REQUIRED_PHRASES[0],
        vm.PR_ACCEPTANCE_REQUIRED_PHRASES[0],
        *vm.PR_ACCEPTANCE_REQUIRED_PHRASES[1:],
    ]
    findings = vm._inventory_lock_consistency(
        dup_acceptance, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "pr_acceptance_required_phrases must be unique" in f.message for f in findings
    )

    blank_acceptance = dict(inventory)
    blank_acceptance["pr_acceptance_required_phrases"] = ["ok", "  "]
    findings = vm._inventory_lock_consistency(
        blank_acceptance, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
    ...
    missing_acceptance = dict(inventory)
    missing_acceptance["pr_acceptance_required_phrases"] = ["## Acceptance Criteria"]
    findings = vm._inventory_lock_consistency(
        missing_acceptance, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "pr_acceptance_required_phrases must include Acceptance Criteria" in f.message
        for f in findings
    )
    ...
    (tmp_path / "schemas" / "packaging-inventory.json").write_text(
        json.dumps(
            _inventory_payload(
                ...
                + ["invented summary"],
                pr_changes_required_phrases=list(vm.PR_CHANGES_REQUIRED_PHRASES)[:-1]
                + ["invented changes"],
                pr_acceptance_required_phrases=list(vm.PR_ACCEPTANCE_REQUIRED_PHRASES)[
                    :-1
                ]
    ...
    assert any("pr_changes_required_phrases" in f.message for f in findings)
