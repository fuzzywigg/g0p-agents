...
def test_live_v42_changelog_leftover_validators() -> None:
    ...
    assert len(vm.VALIDATORS) == 139
    assert vm.MIN_VALIDATOR_COUNT == 139
    assert "changelog-preamble" in vm.VALIDATORS
    assert "changelog-changed" in vm.VALIDATORS
    assert "changelog-initial" in vm.VALIDATORS
    inventory = json.loads(
        (REPO_ROOT / "schemas" / "packaging-inventory.json").read_text(encoding="utf-8")
    )
    assert inventory["version"] == 42
...
def test_v42_changelog_leftover_edge_cases(tmp_path: Path) -> None:
    assert any(
        "missing" in f.message for f in vm.validate_changelog_preamble(tmp_path)
    )
    assert any(
        "missing" in f.message for f in vm.validate_changelog_changed(tmp_path)
    )
    assert any(
        "missing" in f.message for f in vm.validate_changelog_initial(tmp_path)
    )
    ...
    empty = _inventory_payload()
    empty["changelog_preamble_required_phrases"] = []
    findings = vm._inventory_lock_consistency(
        empty, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "changelog_preamble_required_phrases must not be empty" in f.message
        for f in findings
    )

    dup = _inventory_payload()
    dup["changelog_preamble_required_phrases"] = [
        vm.CHANGELOG_PREAMBLE_REQUIRED_PHRASES[0],
        vm.CHANGELOG_PREAMBLE_REQUIRED_PHRASES[0],
        *vm.CHANGELOG_PREAMBLE_REQUIRED_PHRASES[1:],
    ]
    findings = vm._inventory_lock_consistency(
        dup, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "changelog_preamble_required_phrases must be unique" in f.message
        for f in findings
    )

    blank = _inventory_payload()
    blank["changelog_preamble_required_phrases"] = ["ok", "  "]
    findings = vm._inventory_lock_consistency(
        blank, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
    ...
    missing = _inventory_payload()
    missing["changelog_preamble_required_phrases"] = ["---"]
    findings = vm._inventory_lock_consistency(
        missing, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "changelog_preamble_required_phrases must include" in f.message
        for f in findings
    )

    empty_c = _inventory_payload()
    empty_c["changelog_changed_required_phrases"] = []
    findings = vm._inventory_lock_consistency(
        empty_c, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "changelog_changed_required_phrases must not be empty" in f.message
        for f in findings
    )

    dup_c = _inventory_payload()
    dup_c["changelog_changed_required_phrases"] = [
        vm.CHANGELOG_CHANGED_REQUIRED_PHRASES[0],
        vm.CHANGELOG_CHANGED_REQUIRED_PHRASES[0],
        *vm.CHANGELOG_CHANGED_REQUIRED_PHRASES[1:],
    ]
    findings = vm._inventory_lock_consistency(
        dup_c, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "changelog_changed_required_phrases must be unique" in f.message
        for f in findings
    )

    blank_c = _inventory_payload()
    blank_c["changelog_changed_required_phrases"] = ["ok", "  "]
    findings = vm._inventory_lock_consistency(
        blank_c, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
    ...
    missing_c = _inventory_payload()
    missing_c["changelog_changed_required_phrases"] = ["### Changed"]
    findings = vm._inventory_lock_consistency(
        missing_c, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "changelog_changed_required_phrases must include" in f.message
        for f in findings
    )

    empty_i = _inventory_payload()
    empty_i["changelog_initial_required_phrases"] = []
    findings = vm._inventory_lock_consistency(
        empty_i, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "changelog_initial_required_phrases must not be empty" in f.message
        for f in findings
    )

    dup_i = _inventory_payload()
    dup_i["changelog_initial_required_phrases"] = [
        vm.CHANGELOG_INITIAL_REQUIRED_PHRASES[0],
        vm.CHANGELOG_INITIAL_REQUIRED_PHRASES[0],
        *vm.CHANGELOG_INITIAL_REQUIRED_PHRASES[1:],
    ]
    findings = vm._inventory_lock_consistency(
        dup_i, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "changelog_initial_required_phrases must be unique" in f.message
        for f in findings
    )

    blank_i = _inventory_payload()
    blank_i["changelog_initial_required_phrases"] = ["ok", "  "]
    findings = vm._inventory_lock_consistency(
        blank_i, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
    ...
    missing_i = _inventory_payload()
    missing_i["changelog_initial_required_phrases"] = ["## [0.1.0] — 2025-12-13"]
    findings = vm._inventory_lock_consistency(
        missing_i, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "changelog_initial_required_phrases must include" in f.message
        for f in findings
    )
