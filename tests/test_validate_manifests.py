        ...
        ("markdownlint_default", False),
            ...
            (
                ...
                (
                    ...
                    list(vm.PROMPT_ROLE_BLURBS_REQUIRED_PHRASES)[:-1] + ["invented"],
                ),
                (
                ...
                (
                    ...
                    list(vm.PROMPT_LIVING_DOCS_REQUIRED_PHRASES)[:-1] + ["invented"],
                ),
                (
...
def test_live_v51_prompt_role_blurbs_escalation_format_living_docs_validators() -> None:
    ...
    assert sorted(vm.VALIDATORS) == inventory["validator_names"]
    # prior prompt leftovers remain green (v50 + earlier)
    assert vm.validate_prompt_context(REPO_ROOT) == []
    assert vm.validate_prompt_cannot_delegate(REPO_ROOT) == []
    assert vm.validate_prompt_escalation_authority(REPO_ROOT) == []
    assert vm.validate_prompt_responsibilities(REPO_ROOT) == []
    ...
    assert "**v51** deepeners" in (REPO_ROOT / "CHANGELOG.md").read_text(
        encoding="utf-8"
    )
...
def test_v51_inventory_lock_mismatch_and_consistency_matrix(tmp_path: Path) -> None:
    ...
    for key, phrases, include_token in keys:
        ...
        payload = _inventory_payload()
        payload[key] = list(phrases)[:-1] + [f"invented-{key}"]
        findings = vm._inventory_lock_consistency(
            payload, schema_path="schemas/packaging-inventory.json"
        )
        assert any(key in f.message for f in findings)
    ...
    # Use once-only phrases so a single replace removes the lock target.
    for fn, drop in [
        (vm.validate_prompt_role_blurbs, vm.PROMPT_ROLE_BLURBS_REQUIRED_PHRASES[0]),
        (
            vm.validate_prompt_escalation_format,
            "Timeline: [How long until decision needed?]",
        ),
        (
            vm.validate_prompt_living_docs,
            "Use these prompts when instantiating each specialist agent. "
            "Customize with project-specific context.",
        ),
    ]:
        assert body.count(drop) >= 1
        # remove all occurrences so the lock fails even for repeated headings
        truncated = body.replace(drop, "")
        _write(tmp_path / "AGENT-PROMPTS.md", truncated)
        findings = fn(tmp_path)
        assert findings
        assert any(drop in f.message or "missing" in f.message for f in findings)
