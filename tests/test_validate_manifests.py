...
def test_live_v40_hydration_deepener_validators() -> None:
    ...
    assert vm.VALIDATORS["hydration-list-b"](REPO_ROOT) == []


def test_v40_hydration_deepener_edge_cases(tmp_path: Path) -> None:
    ...
    assert any("missing PHASE 1 FINDINGS REPORT section" in f.message for f in findings)
    ...
    missing = _inventory_payload()
    missing["hydration_findings_required_phrases"] = ["## PHASE 1: FINDINGS REPORT"]
    findings = vm._inventory_lock_consistency(
        missing, schema_path="schemas/packaging-inventory.json"
    )
    assert any(
        "hydration_findings_required_phrases must include" in f.message
        for f in findings
    )
    ...
    assert any(
        "hydration_list_a_required_phrases must be unique" in f.message for f in findings
    )
    ...
    assert any(
        "hydration_list_a_required_phrases must include" in f.message for f in findings
    )
    ...
    assert any(
        "hydration_issues_required_phrases must be unique" in f.message for f in findings
    )
    ...
    assert any(
        "hydration_issues_required_phrases must include" in f.message for f in findings
    )
