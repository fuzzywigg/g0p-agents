...
def test_live_v5_validators() -> None:
    assert vm.validate_bug_report_template(REPO_ROOT) == []
    assert vm.validate_feature_request_template(REPO_ROOT) == []
    assert vm.INVENTORY_VERSION == 36
    assert len(vm.VALIDATORS) == 121
...
def test_live_v8_validators() -> None:
    ...
    assert vm.INVENTORY_VERSION == 36
    assert len(vm.VALIDATORS) == 121
    assert vm.MIN_VALIDATOR_COUNT == 121
...
def test_live_v12_validators() -> None:
    ...
    assert inventory["min_validator_count"] == 121
...
def test_live_v36_contributing_deepener_validators() -> None:
...
def test_v36_contributing_deepener_edge_cases(tmp_path: Path) -> None:
    assert any(
    ...
    _write(tmp_path / "CONTRIBUTING.md", "# Other\n\nNo locked content.\n")
    ...
    assert any(
        "missing Packaging inventory v36 honesty lock" in f.message for f in findings
    )
    ...
    missing = _inventory_payload()
    missing["contributing_metadata_required_phrases"] = [
        "# Contributing to g0p-agents"
    ]
    findings = vm._inventory_lock_consistency(
        missing, schema_path="schemas/packaging-inventory.json"
    ...
    assert any(
        "contributing_metadata_required_phrases must include title/Status/Edit policy"
        in f.message
        for f in findings
    )
    ...
    assert any(
        "contributing_surfaces_required_phrases must include surface list" in f.message
        for f in findings
    )
    ...
    assert any(
        "contributing_ci_honesty_required_phrases must include CI checks" in f.message
        for f in findings
    )
