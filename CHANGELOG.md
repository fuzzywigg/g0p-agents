...
- Goose-recipe schema leftover **tests-only** deepeners after tip through **#270**
  (goose-schema leftover after #265) / **#265** (Claude/routing leftover after #264) /
  **#264** (goose-schema leftover after #258) / **#258** / **#253** / **#248** /
  ...
  **#202** / **#200** / **#195** / **#188** / **#187** / **#186** / **#184**
  ...
  inventory bump): unsaturated reject leftovers beyond #270 (`name`
  empty/space/dot/hyphen/`False`/null/list, root null/list/string/empty/extra,
  `title` empty/null/bool/list, `instructions` null/bool/object, `prompt`
  null/list/bool, `version` empty/bool/list/padded/v-prefix, provider/model
  bool/list/empty/int/wrong-const, settings empty/extra, `recipe` empty/list/extra,
  missing recipe required fields, timeout zero/null/float/string, extension
  missing type/name, name empty/null, type empty/int/`STDIO`, item null/string/list,
  extensions non-array), invent-key KeyError / `run_all --only` ValueError, ACCEPT
  leftovers (name min 1, title max 256, instructions/prompt min 1, omit-timeout,
  per-enum types, mixed uniqueItems), missing-doc / directory-not-file / duplicate
  names / `.yml`-**File** inventory / extra-**File** / docs-phrase drop / first-run
  drop / invent-run leftovers, on-disk `.yml` invent / non-yaml allow-list /
  historic provider+version / wrong historic extension type / YAML parse leftovers,
  title-type exact + orchestration missing-agent + invent-agent +
  `expected_recipe_files` / `recipe_primary_agents` / `historic_recipe_version` /
  `historic_goose_settings` / `agentic_flows_allowed_files` inventory leftovers, CI
  `--list-validators` / `--only` goose-trio + required manifest step-marker smoke,
  concurrent races, and tip isolation against #270/#265/#264/#258/#253/#248/#242/
  #236/#229/#224/#219 siblings. Historic four only; Packaging inventory unchanged
  (v52 / 196 validators). Distinct from merged #270 (prior goose-schema leftover
  after #265), merged #265 (Claude/routing leftover), closed CONFLICTING
  #273/#268/#263/#262/#261/#259/#255/#254/#249/#246/#245/#239/#234/#228/#216/#212
  (same leftover on stale tips), #264 (prior goose-schema leftover after #258),
  #258 (hydration/security+handoff), #253 (prompts+flows), #248
  (actionlint/linkcheck), #242 (prompts+flows), #236 (actionlint/linkcheck), #229
