...
- CI/markdownlint residual **tests-only** leftover deepeners after tip through
  ...
  **#188** / **#187** / **#186** (EXISTING six fixtures — `markdownlint` /
  ...
  `.markdownlint.yaml` ↔ `schemas/markdownlint.schema.json` required-key and
  const lock parity, per-lock schema const mismatch, YAML structural and type
  ...
  concurrent races, tip-after-#248 deepeners (exact CI fixture Finding equality,
  ZWSP/soft-hyphen/CRLF lookalikes, simultaneous multi-lock exact drops,
  triple-surface races, quoted bool + cache-dep lookalikes, empty MD013/MD024
  maps + pytest marker reorder/junit lookalike), tip-after-#264 deepeners
  (isolation vs #264 goose-schema / #258 hydration / #253 prompts+flows,
  fullwidth/tab/ideographic lookalikes, schema additionalProperties +
  wrong-type roots, partial pytest marker drop + `--only` subset, dual-surface
  exact + inventory seed parity, quad-surface concurrent races), tip-after-#265
  deepeners (isolation vs #265 Claude/routing + #264 goose/#258 hydration,
  `--only` subset vs siblings, live-green), and tip isolation against
  #265/#264/#258/#253/#248/#242/#236/#229/#224/#219 siblings. Historic four
  only; inventory unchanged (v52 / 196). Distinct from merged #265
